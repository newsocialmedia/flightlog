...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    // Check for existing roster for same month/year
    const {data:existing} = await sb.from("rosters")
      ...
      if(existing?.id) {
        // Update existing — omit updated_at since column may not exist yet
    ...
    const {data,error} = await sb.from("rosters")
      .insert({user_id:userId, period_label:roster.periodLabel, year:roster.year, month_num:roster.monthNum, calendar:roster.calendar})
      .select()
      .single();
