...
function toIdent(flightNum) {
  const s = flightNum.trim().toUpperCase().replace(/\s+/,"");
  const m = s.match(/^([A-Z0-9]{2,3})(\d+)$/);
  if(!m) return s;
  return (IATA_TO_ICAO[m[1]] || m[1]) + m[2];
}
...
export default async function handler(req, res) {
  ...
  const ident = toIdent(flightNum);
  const rawMatch = flightNum.trim().toUpperCase().replace(/\s+/,"").match(/^([A-Z0-9]{2,3})(\d+)$/);
  const num = rawMatch?.[2] || "";
  const carrier = rawMatch?.[1] || "";

  // Try multiple idents — GoJet appears under GJS, UAL, and G7 in FlightAware
  const identsToTry = [...new Set([
    ident,
    (carrier==="G7"||carrier==="GJS") ? `UAL${num}` : null,
    (carrier==="G7") ? `GJS${num}` : null,
  ].filter(Boolean))];
  ...
  console.log(`[lookup-flight] ${tryIdent || usedIdent} → tail:${best.registration} dep:${depUtc} arr:${arrUtc}`);
