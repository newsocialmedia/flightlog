...
function getAirportUtcOffsetMins(airportCode) {
  ...
  if(!tz) {
    // Unknown airport — use longitude-based estimate
    const coords = AIRPORT_COORDS[airportCode];
    if(coords) return Math.round(coords[1] / 15) * 60;
    return 0;
  }
  try {
    // Compute actual UTC offset (DST-aware) by formatting a known UTC time
    // in the target timezone and comparing the result to the UTC time.
    // This works in all modern browsers including Safari on iOS.
    const now = new Date();
    // Format the date in the target timezone to get local H and M
    ...
    // offset = local - UTC (in minutes), handle day boundary
    let offsetMins = (localH * 60 + localM) - (utcH * 60 + utcM);
    if(offsetMins > 720)  offsetMins -= 1440; // crossed midnight forward
    if(offsetMins < -720) offsetMins += 1440; // crossed midnight backward
    return offsetMins;
  } catch {
    const coords = AIRPORT_COORDS[airportCode];
    if(coords) return Math.round(coords[1] / 15) * 60;
...
function schedMinsFromAirports(depTime, arrTime, depCode, arrCode) {
  ...
  const depLocalMins = dh * 60 + dm;
  const arrLocalMins = ah * 60 + am;

  const depOffset = getAirportUtcOffsetMins(depCode); // e.g. -240 for EDT
  const arrOffset = getAirportUtcOffsetMins(arrCode); // e.g. -300 for CDT

  // Block time = (arr local - arr UTC offset) - (dep local - dep UTC offset)
  // = arrLocal - depLocal + depOffset - arrOffset
  // e.g. LEX 14:30 EDT → ORD 15:45 CDT:
  // = (15:45 - 14:30) + (-240 - -300) = 75 + 60 = 135 mins = 2:15 ✓
  const naiveDiff = arrLocalMins - depLocalMins;
  const tzAdjustment = depOffset - arrOffset; // positive when flying west (gaining time)
  let blockMins = naiveDiff + tzAdjustment;

  // Handle overnight
  if(blockMins < 0) blockMins += 1440;
  if(blockMins > 1440) blockMins -= 1440;

  if(blockMins >= 15 && blockMins <= 600) return blockMins;

  // Fallback: naive same-timezone
...
// Flight route map using Leaflet tiles + RainViewer weather radar (free, no key)
function FlightRouteMap({dep, arr, showRadar}) {
  ...
  useEffect(()=>{
    ...
    const initMap = () => {
      ...
      const latSpan = Math.abs(c2[0]-c1[0]);
      const lonSpan = Math.abs(c2[1]-c1[1]);

      // Start at center with placeholder zoom — fitBounds below will correct it
      const map = L.map(mapRef.current, {
        center:[midLat,midLon], zoom:5, maxZoom:19,
        zoomControl:false, attributionControl:false,
        scrollWheelZoom:false, dragging:true,
      });
      leafletRef.current = map;

      // Base tile layer — dark aviation-style
      ...
      for(let i=0;i<=40;i++){
        ...
        const arc=Math.sin(t*Math.PI)*Math.max(latSpan,lonSpan)*0.06;
        pts.push([lat+arc,lon]);
      }
      L.polyline(pts,{color:"#3B8EEA",weight:2.5,dashArray:"8,5",opacity:0.85}).addTo(map);

      // Auto-fit zoom so both airports are visible with comfortable padding
      const bounds = L.latLngBounds([c1[0],c1[1]], [c2[0],c2[1]]);
      map.fitBounds(bounds, {padding:[36,36], maxZoom:8});

      // Airport markers
      const icon = (code) => L.divIcon({
        html:`<div style="background:#3B8EEA;color:#fff;font-size:9px;font-weight:700;padding:2px 5px;border-radius:4px;white-space:nowrap;border:1px solid rgba(255,255,255,.3)">${code}</div>`,
      ...
      L.marker(mid, {icon: L.divIcon({
        html:`<div style="font-size:18px;transform:rotate(${angle}deg);line-height:1">✈</div>`,
        className:"",iconAnchor:[10,10],
      })}).addTo(map);

      // Weather radar via RainViewer (free, no API key)
      if(showRadar) {
        const loadRadar = async () => {
          try {
            ...
            radarRef.current = L.tileLayer(
              ...
              {opacity:0.5, maxZoom:19, maxNativeZoom:18}
            ).addTo(map);
          } catch {}
        };
        loadRadar();
        radarTimerRef.current = setInterval(loadRadar, 5*60*1000); // refresh every 5 min
      }
    };
  ...
  return (
    <div className="card" style={{marginBottom:14,padding:0,overflow:"hidden"}}>
      <div style={{padding:"10px 16px 6px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {showRadar&&<div style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:C.teal}}>
...
function FlightDetailPage({flight:f, tail, solar, dist, blockMins, day, roster, hasActual, dep, arr, isXC, onBack, onAutoLookup, lkStatus, lkError, tmp, onTmpChange, onSaveTail, saving, onTailSaved, editingTimes, setEditingTimes, timeEdits, setTimeEdits, di, fi, userId}) {
  ...
  // Cache key for this specific flight briefing
  const briefingCacheKey = `fl_briefing_${f.flightNum}_${roster.year}_${(roster.monthNum||0)+1}_${day.day}`;
  ...
  useEffect(()=>{
    // Load cached briefing if available and less than 1 hour old
    try {
  ...
  async function loadBriefing(forceRefresh=false) {
    ...
    try {
      ...
      // Cache for 1 hour
      try {
        localStorage.setItem(briefingCacheKey, JSON.stringify({text:result.briefing, timestamp:now}));
      } catch {}
  ...
  async function saveTimesEdits() {
    ...
    try {
      ...
      setTimesError(e.message || "Failed to save times. Please try again.");
    } finally {
  ...
  return (
    <div style={{maxWidth:600,paddingBottom:32}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <button onClick={onAutoLookup} disabled={lkStatus==="loading"||lkStatus==="ratelimit"} style={{padding:"8px 14px",borderRadius:8,background:C.teal+"11",border:`1px solid ${C.teal}44`,color:C.teal,fontSize:13,fontWeight:600,cursor:"pointer"}}>
            {lkStatus==="loading"?<span className="spinner">⟳</span>
            :lkStatus==="notfound"?"Not found"
            :lkStatus==="ratelimit"?"⏳ Limit"
            :"🔍 Auto"}
          </button>
        </div>
        {lkError&&(lkStatus==="error"||lkStatus==="notfound")&&(
          <div style={{fontSize:11,color:lkStatus==="error"?C.red:C.muted,marginTop:8,padding:"6px 10px",background:C.panel,borderRadius:6,wordBreak:"break-word"}}>
            {lkStatus==="notfound"?"Flight not found in FlightAware: ":""}{lkError}
          </div>
        )}
        {tail.finalSynced&&<div style={{fontSize:11,color:C.green,marginTop:8}}>✓ Synced — actual times locked</div>}
      </div>

      {/* Route Map — with live radar for upcoming flights */}
      {(()=>{
        const flightDt2 = new Date(roster.year,(roster.monthNum||roster.month_num||0),day.day,...(f.depTime||"00:00").split(":").map(Number));
        const hoursUntil2 = (flightDt2 - new Date()) / 3600000;
        const isUpcoming2 = hoursUntil2 > -1 && hoursUntil2 < 24;
        return <FlightRouteMap dep={f.dep} arr={f.arr} showRadar={isUpcoming2}/>;
      })()}
      ...
      {(()=>{
        ...
        const canRefresh = briefingTimestamp && briefingAgeMs > 3600000; // only after 1 hour
        return (
          <div className="card" style={{marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <div>
                ...
                {briefingTimestamp&&(
                  <div style={{fontSize:10,color:C.muted,marginTop:1}}>
                    {briefingAgeMin===0?"Just now":briefingAgeMin===1?"1 min ago":`${briefingAgeMin} min ago`}
                    {briefingAgeMs<3600000&&<span style={{marginLeft:6,color:C.teal}}>• Valid for {60-briefingAgeMin}m</span>}
                  </div>
                )}
              ...
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                {briefing&&canRefresh&&(
                  <button onClick={()=>loadBriefing(true)} disabled={briefingLoading} style={{padding:"5px 10px",borderRadius:7,background:C.teal+"18",border:`1px solid ${C.teal}44`,color:C.teal,fontSize:11,fontWeight:600,cursor:"pointer"}}>
                    {briefingLoading?<span className="spinner">⟳</span>:"↻ Refresh"}
                  </button>
                )}
                {!briefing&&(
                  <button onClick={()=>loadBriefing(false)} disabled={briefingLoading} style={{padding:"6px 14px",borderRadius:8,background:C.teal,border:"none",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer"}}>
