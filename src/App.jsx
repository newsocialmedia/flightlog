...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    if(Array.isArray(roster.carryForwardDays) && roster.carryForwardDays.length > 0) {
      ...
      // Ensure carry-forward days don't bleed into this month's calendar
      // by removing any day from cal that also appears in carryForwardDays
      const carryDayNums = new Set(carryForwardDays.map((d: any) => d.day));
      // Keep only days that are NOT in carryForwardDays AND are valid for this month
      // We use array position to disambiguate: if day:1 appears twice (Feb day 1 AND Mar day 1),
      // the carry-forward one should be at the END after the month-end days
      let seenDays = new Set<number>();
      thisDays = [];
      // Walk calendar in order — for duplicate day numbers, keep the FIRST occurrence
      // (which should be the real February day, not the carry-forward March day)
      for(const d of cal) {
        if(carryDayNums.has(d.day)) {
          // Could be Feb day 1 OR Mar day 1 — skip if we've already found a Feb day 1
          if(seenDays.has(d.day)) continue; // duplicate, skip
          // If this is early in the array AND day is small, it might be a real Feb day
          // We use the carryForwardDays reference to check: if d is IN carryForwardDays exactly, skip
          const isCarry = carryForwardDays.some((c: any) => c.day === d.day && c.flights?.length === d.flights?.length);
          if(isCarry) continue;
        }
