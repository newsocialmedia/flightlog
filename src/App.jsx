...
async function fetchProfile(userId, accessToken) {
  try {
    ...
    let token = accessToken || sb.auth._token || SUPA_ANON;
...
function LandingPage({onLogin, onSignup}) {
  // Ensure styles are injected — runs synchronously before browser paint
  useEffect(()=>{
    let el = document.getElementById("fl-styles");
    if(!el || !el.textContent) {
      if(!el) {
...
function SubscriptionWall({user, onSubscribed, onLogout}) {
  const [status, setStatus] = useState("checking"); // "checking" | "waiting" | "error"
  ...
  async function checkSubscription() {
    try {
      ...
      if(!sb.auth._token) {
        try {
          const t = localStorage.getItem("fl_token");
          if(t) sb.auth._token = t;
        } catch {}
      }
      const token = sb.auth._token || SUPA_ANON;
      const isAnon = !sb.auth._token;
      console.log("[Wall] token:", isAnon ? "ANON — RLS will block" : "USER ✓", "| userId:", user.id);

      const res = await fetch(
        `${SUPA_URL}/rest/v1/profiles?id=eq.${user.id}&select=subscription_status,plan&limit=1`,
        {headers:{
          "apikey": SUPA_ANON,
          "Authorization": `Bearer ${token}`,
      ...
      console.log("[Wall] profile:", JSON.stringify(profile));
      ...
      console.error("[Wall] error:", e.message);
      return false;
    }
  }

  // Auto-poll every 3 seconds for up to 3 minutes
  useEffect(()=>{
    let count = 0;
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:C.base}}>
      ...
      <div style={{padding:"16px 20px",borderBottom:`1px solid ${C.border}`,background:C.surface,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={async()=>{
            // Force check and navigate regardless — for paid users stuck on wall
            const found = await checkSubscription();
            if(!found) {
              // Force-navigate if user claims to have paid
              const token = sb.auth._token || SUPA_ANON;
              const res = await fetch(`${SUPA_URL}/rest/v1/profiles?id=eq.${user.id}&select=*&limit=1`,
                {headers:{"apikey":SUPA_ANON,"Authorization":`Bearer ${token}`}});
              const data = await res.json();
              const profile = Array.isArray(data)?data[0]:data;
              alert(`Your status: ${profile?.subscription_status||"not found"}\n\nIf you paid, email support@flightlog.app with your receipt and we'll activate manually.`);
            }
          }} style={{fontSize:12,color:C.teal,background:C.teal+"12",border:`1px solid ${C.teal}33`,borderRadius:7,padding:"6px 12px",cursor:"pointer",fontWeight:600}}>
            Check status
          </button>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",alignItems:"center",padding:"28px 16px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:28,maxWidth:360}}>
          ...
          <div style={{fontSize:22,fontWeight:800,color:C.ink,marginBottom:8,letterSpacing:"-.5px"}}>
            Welcome, {user.name?.split(" ")[0]||"Pilot"}!
          </div>
          <div style={{fontSize:14,color:C.muted,lineHeight:1.65}}>
            Subscribe below to unlock FlightLog. The app will open <strong>automatically</strong> the moment your payment is confirmed.
          </div>
        </div>
        ...
        <div style={{
          display:"flex",alignItems:"center",gap:10,
          padding:"10px 18px",borderRadius:20,marginBottom:24,
          background:C.teal+"10",border:`1px solid ${C.teal}22`,
        }}>
          <div style={{width:8,height:8,borderRadius:"50%",background:C.teal,flexShrink:0,animation:"pulse 1.5s infinite"}}/>
          <span style={{fontSize:12,color:C.teal,fontWeight:600}}>
            {status==="waiting"
              ? "Waiting for payment — tap Subscribe below"
              : `Listening for payment confirmation${dots}`}
          </span>
        </div>
        ...
        <div style={{width:"100%",maxWidth:400,marginBottom:20}}>
          {[
            ["📄","AI roster parsing — any airline PDF"],
            ["✈","Auto tail number & block time sync"],
            ["📊","FAR 61.57 & 117 currency tracking"],
            ["📋","Jeppesen & ASA export"],
            ["🗺️","Route maps with live weather radar"],
          ].map(([icon,text])=>(
            <div key={text} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",marginBottom:8,background:C.surface,borderRadius:10,border:`1px solid ${C.border}`}}>
    ...
    <div key={themeKey}>
      ...
      {screen==="app"&&user&&(
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base}}>
          ...
          {/* ── SUBSCRIPTION WALL ── */}
          {user.role!=="admin"&&user.subscription_status!=="active"&&user.subscription_status!=="past_due"?(
