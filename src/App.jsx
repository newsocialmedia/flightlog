  ...
  for(const r of rosters){
    ...
    (r.calendar||[]).forEach((d,di)=>{
      (d.flights||[]).forEach((f,fi)=>{
        ...
        bm.legs+=1;
        bm.night+=nightMins;
        if(isXC) bm.xc+=mins; // XC shown as TIME, not leg count
        bm.landings+=1;
        bm.dist+=dist;
        if(isPIC) bm.pic+=mins;
        if(isSIC) bm.sic+=mins;
        if(isMulti) bm.multi+=mins;
        if(isTurbine) bm.turbine+=mins;
        if(isSingle) bm.single+=mins;
        if(solar?.nightArr){ bm.nightLandings+=1; bm.nightTo+=1; }
        else { bm.dayLdg+=1; bm.dayTo+=1; }

        totalMins+=mins; totalLegs+=1; totalNight+=nightMins; totalDist+=dist;
        if(isXC) totalXC+=mins;
        totalLandings+=1;
        if(isPIC) totalPIC+=mins;
        if(isSIC) totalSIC+=mins;
        ...
        if(flightDate>=cutoff30)  { last30.mins+=mins;  last30.legs+=1;  last30.night+=nightMins;  if(isXC)last30.xc+=mins;  last30.landings+=1; }
        if(flightDate>=cutoff6mo) { last6mo.mins+=mins; last6mo.legs+=1; last6mo.night+=nightMins; if(isXC)last6mo.xc+=mins; last6mo.landings+=1; }
        if(flightDate>=cutoff12mo){ last12mo.mins+=mins;last12mo.legs+=1;last12mo.night+=nightMins;if(isXC)last12mo.xc+=mins;last12mo.landings+=1; }
      });
    });
  }
  return{
...
function csvExport(rosters, tails) {
  const rows=[["Date","Day","Flight","Dep","SchedDep","ActualDep","Arr","SchedArr","ActualArr","AircraftType","Tail","SchedBlock","ActualBlock","Period","Deadhead","Remarks"]];
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    ...
    const mNum=r.monthNum??r.month_num??0;
    const dateStr=`${r.year}-${String(mNum+1).padStart(2,"0")}-${String(d.day).padStart(2,"0")}`;
    rows.push([dateStr,d.dow,f.flightNum,f.dep,f.depTime,t.actualDep||"",f.arr,f.arrTime,t.actualArr||"",f.acType||"",t.tail||"",schedBlock,actualBlock,r.periodLabel||"",isDH?"DH":"",t.remarks||""]);
  })));
...
function jeppesenExport(rosters, tails) {
  ...
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    ...
    rows.push([dateStr,f.flightNum,f.dep,f.arr,t.actualDep||f.depTime,t.actualArr||f.arrTime,f.acType||"","N/"+  (t.tail||""),isDH?"":block,"","","","","",isDH?"":block,"","",remarks]);
  })));
...
function asaExport(rosters, tails) {
  ...
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    ...
    const block=isDH?"":(t.actualBlockMins!=null?fmtMins(t.actualBlockMins):schedMins(f)?fmtMins(schedMins(f)):"");
    const remarks=isDH?`DEADHEAD${t.remarks?" -- "+t.remarks:""}`:(t.remarks||"");
    rows.push([dateStr,f.acType||"","N/"+(t.tail||""),f.dep,f.arr,block,"","","","",isDH?"":block,"","","",remarks]);
  })));
      ...
      <div style={{padding:"20px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",zIndex:1,maxWidth:1200,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        <div>
          ...
          <p style={{fontSize:13,color:S.muted,margin:"3px 0 0"}}>
            {nextFlight?`Next duty in ${formatDutyCountdown(Math.round((nextFlight.dt-now)/60000))}`:"Welcome back to your logbook"}
          </p>
        </div>
...
function LogbookHubPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, onOpenFlight, pendingFlight, onPendingFlightConsumed, setPage, initialTab}) {
  ...
  const [hubTab, setHubTab] = useState(()=>{
    ...
    try{ return sessionStorage.getItem("fl_logbook_hub_tab") || "daily"; }catch{ return "daily"; }
  });
  ...
  const TABS = [
    ...
    {id:"logbookview", label:"Logbook View",icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="4" y="2" width="13" height="18" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M8 7h6M8 11h4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"roster",      label:"Roster View", icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="17" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M8 2V6M16 2V6M3 9H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
    ...
    return(
      <FlightDetailPage
        ...
        onSaveTail={async(val)=>{
          ...
          try{ await db_saveTail(user.id,rId,fk,val||""); onTailSaved(tk,{...(tail||{}),tail:val}); setHubSelectedFlight(p=>({...p,tail:{...p.tail,tail:val}})); }catch(e){alert(e.message);}
        }}
        saving={false}
        onTailSaved={async(v)=>{
          const parts=tk.split("-"); const rId=parts.slice(0,-2).join("-"); const fk=parts.slice(-2).join("-");
          try{ await db_saveTail(user.id,rId,fk,v); onTailSaved(tk,v); setHubSelectedFlight(p=>({...p,tail:{...p.tail,...v},hasActual:true})); }catch(e){alert(e.message);}
        }}
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      {/* Active tab content -- reuses existing fully-working components */}
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
        ...
        {hubTab==="roster"&&(
          <CalendarPage user={user} rosters={rosters} tails={tails} onRosterUpdated={onRosterUpdated} onOpenFlight={onOpenFlight}/>
        )}
        {hubTab==="activelogs"&&(
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed, setPage, forcedView, onOpenFlightExternal}) {
  ...
  useEffect(()=>{ if(forcedView && forcedView!==view) setView(forcedView); },[forcedView]);
  ...
  function openFlight(di,fi,f,day){
    ...
    try{sessionStorage.setItem("fl_open_flight",JSON.stringify({rosterIdx:selRoster,rosterId:roster.id,di,fi,flightData}));}catch{}
    ...
    for(const r of signedRosters){
      ...
      (r.calendar||[]).forEach((d,di)=>{
        (d.flights||[]).forEach((f,fi)=>{
          ...
          // item still exists for reference in Active Logs / Daily View.
          if(t.cancelled) return;
        ...
        {!roster?(
          ...
          <>
            {(()=>{
              ...
              const mNum2=roster.monthNum??roster.month_num??0;
              const now=new Date();
              // Flatten every flight across every day into one list, already
              // sorted most-recent-first (days desc, then within-day by time desc)
              const allDayFlights=[];
              (roster.calendar||[]).filter(d=>(d.flights||[]).length>0).sort((a,b)=>b.day-a.day).forEach(day=>{
                const di=(roster.calendar||[]).findIndex(d=>d.day===day.day);
                const sorted=[...(day.flights||[])].map((f,fi)=>({f,fi})).sort((a,b)=>(b.f.depTime||"00:00").localeCompare(a.f.depTime||"00:00"));
                sorted.forEach(({f,fi})=>allDayFlights.push({f,fi,di,day}));
              });
              ...
              return allDayFlights.map(({f,fi,di,day})=>{
                const tk=`${roster.id}-${di}-${fi}`;
                const tail=tails[tk]||{};
                ...
                const dist=calcDist(f.dep,f.arr);
                const blockMinsVal=tail.actualBlockMins!=null?tail.actualBlockMins:schedMins(f)||0;
                const isToday=day.day===now.getDate()&&mNum2===now.getMonth()&&roster.year===now.getFullYear();
                const depDisplay = hasActual?tail.actualDep:f.depTime;
                const arrDisplay = hasActual?tail.actualArr:f.arrTime;

                return(
                  <div key={`${di}-${fi}`}
                    onClick={()=>openFlight(di,fi,f,day)}
                    style={{
                  ...
                  >
                    ...
                    <div style={{flexShrink:0,width:56,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",borderRight:`1px solid ${C.border}`,paddingRight:14}}>
                      ...
                      <div style={{fontSize:10,fontWeight:700,color:C.muted,marginTop:3,letterSpacing:".5px"}}>{MONTH_ABBR[mNum2]}</div>
                      <div style={{fontSize:10,fontWeight:600,color:C.muted}}>{roster.year}</div>
                    </div>
...
// all rendered dark, unlike a plain dark-tinted background). When the flight
// departs within 24 hours, overlays the latest RainViewer radar frame so the
// pilot can see real precipitation along the route before departure.
function FlightRouteMap({dep, arr, dist, flightDateStr, flightDepTime, S}) {
  ...
  // Is this flight within 24 hours of departure? Radar overlay only makes
  // sense for near-term flights — historical/future-far flights don't need it.
  const within24h = useMemo(() => {
    if(!flightDateStr || !flightDepTime) return false;
    try {
      const [h,m] = flightDepTime.split(":").map(Number);
      const [y,mo,d] = flightDateStr.split("-").map(Number);
      const depDt = new Date(y, mo-1, d, h||0, m||0);
      const diffHrs = (depDt.getTime() - Date.now()) / 3600000;
      return diffHrs > -3 && diffHrs < 24; // departing soon, or just departed
    } catch { return false; }
  }, [flightDateStr, flightDepTime]);
  ...
  useEffect(() => {
    ...
    const line = L.polyline(pts, { color:"#3B82F6", weight:3.5, opacity:0.95, smoothFactor:1, dashArray:"10 6" }).addTo(map);
  ...
  // Overlay live weather radar (RainViewer) for near-term flights
  useEffect(() => {
    if(!mapReady || !within24h) return;
    const L = window.L;
    const map = leafletMapRef.current;
    if(!L || !map) return;
    ...
    async function loadRadar() {
      try {
        ...
        setRadarLoaded(true);
      } catch { /* radar is a nice-to-have, fail silently */ }
    }

    loadRadar();
    // Auto-refresh radar every 5 minutes so it stays current
    refreshInterval = setInterval(loadRadar, 5 * 60 * 1000);
  ...
  return (
    <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:18,padding:"16px 18px",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",width:"100%",height:220,borderRadius:12,overflow:"hidden",background:"#0B1120"}}>
        ...
        {within24h&&(
          <div style={{position:"absolute",top:8,right:8,zIndex:1000,display:"flex",alignItems:"center",gap:5,background:"rgba(15,23,42,0.75)",backdropFilter:"blur(6px)",padding:"4px 8px",borderRadius:100,border:"1px solid rgba(255,255,255,0.1)"}}>
            <span style={{width:7,height:7,borderRadius:"50%",background:radarLoaded?"#22C55E":"#94A3B8",display:"inline-block",animation:radarLoaded?"radarPulse 2s ease-in-out infinite":"none",flexShrink:0}}/>
            <span style={{fontSize:10,fontWeight:700,color:radarLoaded?"#4ADE80":"#94A3B8"}}>{radarLoaded?"Live":"Radar..."}</span>
          </div>
        )}
      </div>
      <style>{`
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{padding:"16px 16px 32px",maxWidth:600,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* Route map — dark Leaflet basemap + live weather radar overlay */}
        {dep&&arr&&<FlightRouteMap dep={dep} arr={arr} dist={dist2} flightDateStr={dateStr} flightDepTime={f?.depTime} S={S}/>}
...
function PostUploadVerifyScreen({parsedRoster, rosters, user, onRosterSaved, onReloadRosters, setParsedRoster, setStatus, setMsg, setPage}) {
  ...
  // Reload rosters from DB on mount so the carryover stub (a separate DB
  // insert that happens inside db_saveRoster) is visible. The parent's
  // handleRosterSaved does this too but may not settle before this screen
  // first renders — so we keep our own fresh copy.
  const [freshRosters, setFreshRosters] = useState(rosters);
  const [loadingCarryover, setLoadingCarryover] = useState(true);
  const [discarding, setDiscarding] = useState(false);

  useEffect(()=>{
    db_loadRosters(user.id)
      .then(r=>{ if(r?.length){ setFreshRosters(r); onReloadRosters?.(r); } })
      .catch(()=>{})
      .finally(()=>setLoadingCarryover(false));
  },[]);
    ...
    // Use freshRosters (reloaded on mount) so the carryover stub is visible
    const nextMonthCarryover = freshRosters.find(r=>
      (r.monthNum??r.month_num??0)===nextMNum && r.year===nextYear && r.id!==parsedRoster.id
    );
    const hasCarryover = !loadingCarryover && !!nextMonthCarryover;

    // Tab toggle between this month and the carryover month
    const [vTab, setVTab] = useState("current"); // "current" | "carryover"
    const activeRoster = vTab==="carryover" && nextMonthCarryover ? nextMonthCarryover : parsedRoster;
    const activeMNum   = activeRoster.monthNum ?? activeRoster.month_num ?? 0;

    // Build the full calendar grid for the active roster
    // Show ALL days in the month, not just duty days
    const daysInMonth = new Date(activeRoster.year, activeMNum+1, 0).getDate();
    const dayMap = {};
    (activeRoster.calendar||[]).forEach(d=>{ dayMap[d.day] = d; });
    const allDays = Array.from({length:daysInMonth},(_,i)=>({
      day: i+1,
      data: dayMap[i+1] || {day:i+1,dow:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(activeRoster.year,activeMNum,i+1).getDay()],isOff:true,dutyCode:null,flights:[]},
    }));

    const totalFlights = (activeRoster.calendar||[]).reduce((a,d)=>a+(d.flights||[]).length,0);
    const dutyDays     = (activeRoster.calendar||[]).filter(d=>(d.flights||[]).length>0||d.dutyCode).length;
    ...
    async function saveCalendar(newCal, roster=activeRoster) {
      const updated = {...roster, calendar:newCal};
      try {
        await db_saveRoster(user.id, updated);
        onRosterSaved(updated);
        if(vTab==="current") setParsedRoster(updated);
        // Update local fresh copy so the UI reflects the save immediately
        setFreshRosters(prev=>prev.map(r=>r.id===updated.id?updated:r));
      } catch(e) { alert(e.message); }
    }

    async function deleteDay(dayNum) {
      if(!window.confirm(`Remove day ${dayNum} (all flights) from the roster?`)) return;
      const newCal = (activeRoster.calendar||[]).filter(d=>d.day!==dayNum);
      await saveCalendar(newCal);
    }

    async function deleteFlight(dayNum, fi) {
      if(!window.confirm("Remove this flight?")) return;
      const newCal = (activeRoster.calendar||[]).map(d=>
        d.day===dayNum ? {...d, flights:(d.flights||[]).filter((_,i)=>i!==fi)} : d
      ).filter(d=>(d.flights||[]).length>0||d.dutyCode||!d.isOff);
      await saveCalendar(newCal);
    }

    async function addFlight(dayNum) {
      const f = flightDraft;
      if(!f.flightNum&&!f.dep) return;
      const nc = [...(activeRoster.calendar||[])];
      const existing = nc.find(d=>d.day===dayNum);
      const dow = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(activeRoster.year,activeMNum,dayNum).getDay()];
    ...
    return(
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        ...
        <div style={{background:`linear-gradient(135deg,#10B981,#059669)`,padding:"20px 18px 16px",color:"#fff",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:hasCarryover?12:0}}>
          ...
          {hasCarryover&&(
            <div style={{background:"rgba(255,255,255,0.15)",borderRadius:12,padding:"10px 14px",border:"1px solid rgba(255,255,255,0.25)"}}>
              <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:8}}>
                Carryover flights detected and moved to {MONTHS_FULL[nextMNum]} {nextYear}
              </div>
              <div style={{display:"flex",gap:6}}>
                {[["current",`${MONTH_ABBR[mNum]} ${parsedRoster.year}`],["carryover",`${MONTH_ABBR[nextMNum]} ${nextYear}`]].map(([tid,lbl])=>(
        ...
        <div style={{padding:"10px 16px 4px",flexShrink:0,background:S.surface,borderBottom:`1px solid ${S.border}`}}>
          <div style={{fontSize:11,color:S.muted,lineHeight:1.5}}>
            Review and edit below. Nothing is permanent until you <strong style={{color:S.ink}}>Verify & Sign</strong> in Active Logs. Add or remove flights as needed.
          </div>
        </div>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      ...
                      {!isOff&&(
                        <button onClick={()=>deleteDay(day)} title="Remove day"
                          style={{width:28,height:28,borderRadius:"50%",background:"#FEF2F2",border:"none",color:"#DC2626",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
                      )}
                    </div>
                  </div>
                    ...
                    return(
                      <div key={fi} style={{borderTop:`1px solid ${S.border}`,padding:"8px 12px",display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <button onClick={()=>deleteFlight(day,fi)} title="Remove flight"
                          style={{width:26,height:26,borderRadius:"50%",background:"#FEF2F2",border:"none",color:"#DC2626",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>×</button>
                      </div>
                    );
                  })}
                  ...
                  {isAdding&&(
                    <div style={{borderTop:`1px solid ${S.border}`,padding:"10px 12px",background:`${S.blue}06`}}>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:6}}>
                        <div><div style={{fontSize:10,color:S.muted,marginBottom:3}}>Flight #</div><input style={INPUT} placeholder="G7 4522" value={flightDraft.flightNum} onChange={e=>setFlightDraft(p=>({...p,flightNum:e.target.value}))}/></div>
                        <div><div style={{fontSize:10,color:S.muted,marginBottom:3}}>Aircraft</div><input style={INPUT} placeholder="CRJ7" value={flightDraft.acType} onChange={e=>setFlightDraft(p=>({...p,acType:e.target.value.toUpperCase()}))}/></div>
        ...
        <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"flex",gap:8}}>
            <button
              onClick={async()=>{
                if(!window.confirm(`Discard this roster and all parsed flights? This cannot be undone.`)) return;
                setDiscarding(true);
                try{
                  // Delete this month's roster
                  await sb.from("rosters").delete().eq("id",parsedRoster.id);
                  // Delete carryover stub if it was just created
                  if(nextMonthCarryover){
                    await sb.from("rosters").delete().eq("id",nextMonthCarryover.id);
                  }
                  // Reload parent rosters state
                  const fresh = await db_loadRosters(user.id);
                  onReloadRosters?.(fresh||[]);
                }catch(e){ alert("Discard failed: "+e.message); setDiscarding(false); return; }
                setParsedRoster(null); setStatus(null); setMsg("");
              }}
              disabled={discarding}
              style={{flex:1,padding:"12px",borderRadius:14,background:"#FEF2F2",border:"1.5px solid #FECACA",color:"#DC2626",fontSize:13,fontWeight:700,cursor:discarding?"not-allowed":"pointer",opacity:discarding?0.6:1}}
            >
              {discarding?"Discarding...":"Discard"}
            </button>
            <button
              onClick={()=>{setParsedRoster(null);setStatus(null);setMsg("");}}
              style={{flex:1,padding:"12px",borderRadius:14,background:"none",border:`1.5px solid ${S.border}`,color:S.muted,fontSize:13,fontWeight:700,cursor:"pointer"}}
            >
              Upload Another
            </button>
          </div>
          <button
            onClick={()=>setPage&&setPage("active-logs")}
            style={{width:"100%",padding:"13px",borderRadius:14,background:`linear-gradient(135deg,${S.blue},#7C3AED)`,border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}
          >
            Review & Sign in Active Logs
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M14 6l6 6-6 6" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </button>
        </div>
      </div>
    );
...
function UploadPage({user, rosters, onRosterSaved, onReloadRosters, setPage}) {
    ...
    try {
      ...
      try {
        ...
        function stripScore(items){
          ...
          const labels = p1.filter(i=>DAY_LBL.test(i.str.trim()));
          ...
          const ordered = best.slice().sort((a,b)=>a.x-b.x).map(i=>parseInt(i.str.trim().slice(3)));
          let asc=0, desc=0;
          for(let k=1;k<ordered.length;k++){ if(ordered[k]>ordered[k-1]) asc++; else desc++; }
          if(asc <= desc) return 0;
          const hasFld = p1.some(i=>i.str.trim()==='FlD' && i.y>bestY && i.y<bestY+60);
      ...
      if(!roster) {
        ...
        if(!response.ok) {
          ...
          // Layout attempt failed for another reason: retry ONCE with PDF only
          // (fresh request = fresh server time budget for the vision pipeline).
          if(layoutReliable) {
      ...
      setParsedRoster(saved||roster); // show post-upload verification screen
    } catch(e) {
      setStatus("error");
  ...
  // ── TIER 0a: Exact-coordinate NetLine parser ────────────────────────────────
  // Works directly on pdf.js positioned text items {x, y, str, page} in visual
  // coordinates (y increases downward). No fixed-width text reconstruction, so
  // no character-cell drift: FlD markers match day labels by real X proximity.
  //
  // Accuracy guarantees (self-validation — returns null to defer to AI unless):
  //   1. Every FlD-marked day yields at least one extracted flight
  //   2. Every day marked off has no C/I content in its column (catches
  //      unrecognized duty days)
  //   3. Non-FlD strip markers (LCR, SIM, TVL...) become dutyCode days
  //   4. Day-number reset in strip/body → carryover days for next month
  function parseNetlineFromItems(items) {
    const DAY_PREFIX = /^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)(\d{2})/;
    const MONTHS = {Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
    const HEADER_WORDS = new Set(["date","duty","dep","arr","info","AC","Hot","Rep"]);

    const joined = items.map(i=>i.str).join(' ');
    if(!/FlD/.test(joined) || !/G7\s*\d{3,4}/.test(joined)) return {_fail:"no FlD or G7 pattern found in raw item text — not this roster format"};

    // Period: may be split across items — the joined text handles both cases
    const pm = joined.match(/Period:\s*0?1\s*([A-Z][a-z]{2})\s*(\d{2})/);
    if(!pm || MONTHS[pm[1]]===undefined) return {_fail:"Period: line not found or month name unrecognized"};
    const monthNum = MONTHS[pm[1]];
    const year = 2000 + parseInt(pm[2]);

    // ── Day-header strip ───────────────────────────────────────────────────
    // Don't require a day label to be its own isolated item — PDF generators
    // often batch multiple labels, or a label + trailing content, into one
    // showText run. Scan INSIDE every item's string for embedded day-label
    // matches and record each match's real x position (item x + offset of the
    // match within the string, converted via the item's own char pitch).
    const p1 = items.filter(i=>i.page===1);
    const DAY_G = /(Mon|Tue|Wed|Thu|Fri|Sat|Sun)(\d{2})/g;
    function charPitch(i){
      // Prefer the item's own reported width if pdf.js provided one; else
      // fall back to a page-wide estimate from itself vs its length.
      if(i.width && i.str.length) return i.width / i.str.length;
      return 6; // last-resort default, only used if nothing else is available
    }
    function scanMatches(pool, re){
      const out=[];
      for(const it of pool){
        re.lastIndex=0;
        let m;
        while((m=re.exec(it.str))!==null){
          const pitch=charPitch(it);
          out.push({x: it.x + m.index*pitch, y: it.y, str: m[0], groups: m, host: it});
        }
      }
      return out;
    }
    const labelHits = scanMatches(p1, DAY_G);
    const bands = {};
    labelHits.forEach(h=>{const b=Math.round(h.y/5)*5;(bands[b]=bands[b]||[]).push(h);});
    let strip=[], stripY=0;
    Object.entries(bands).forEach(([b,arr])=>{
      // Dedupe by day number within the band (a batched item scanned for
      // multiple labels can't double-count the same day)
      const seen=new Set(), uniq=[];
      for(const h of arr.sort((a,b2)=>a.x-b2.x)){
        const d=parseInt(h.groups[2]);
        if(seen.has(d)) continue;
        seen.add(d); uniq.push({...h, day:d});
      }
      if(uniq.length>strip.length){strip=uniq;stripY=Number(b);}
    });
    if(strip.length<15) return {_fail:`day-header strip too short: found ${strip.length} labels in best y-band, need 15+ (expected ~28-31)`};

    const stripSorted=strip; // already x-sorted per-band above

    // ── Marker row: FlD / LCR / SIM / TVL... scanned the same way ────────────
    // A marker item may carry ONLY the code, or the code plus adjacent noise
    // from the same batched run — so again scan inside strings rather than
    // requiring an exact match. Codes are 2-5 letters, word-bounded.
    const MARKER_G = /\b([A-Z][a-zA-Z]{1,4})\b/g;
    const bandRows = p1.filter(i=>i.y>stripY && i.y<stripY+60);
    const markerHitsRaw = scanMatches(bandRows, MARKER_G).filter(h=>{
      const s=h.groups[1];
      return !HEADER_WORDS.has(s) && !DAY_PREFIX.test(s);
    });
    if(!markerHitsRaw.length) return {_fail:"no marker-row hits found within 60 units below the strip y-band"};
    const fldHits = markerHitsRaw.filter(h=>h.groups[1]==='FlD');
    if(!fldHits.length) return {_fail:`markers found (${markerHitsRaw.length}) but none equal "FlD" exactly`};
    const fldY = fldHits.map(h=>h.y).sort((a,b)=>a-b)[Math.floor(fldHits.length/2)]; // median
    const markers = markerHitsRaw
      .filter(h=>Math.abs(h.y-fldY)<10)
      .map(h=>({x:h.x, y:h.y, str:h.groups[1]}));

    // X tolerance from strip spacing
    let minGap=Infinity;
    for(let i=1;i<stripSorted.length;i++) minGap=Math.min(minGap, stripSorted[i].x-stripSorted[i-1].x);
    const tol=Math.max(8, minGap*0.55);

    // ── Classify each strip day: flight duty / dutyCode / off; detect carryover
    const dutyDays=new Set(), codeDays={}, carryDuty=new Set(), carryCodes={};
    let prev=0, inCarry=false;
    for(const lab of stripSorted){
      const d=lab.day;
      if(!inCarry && prev>=20 && d<prev && d<=14) inCarry=true;
      prev=d;
      const near=markers.filter(m=>Math.abs(m.x-lab.x)<tol);
      if(!near.length) continue; // off day
      const marker=near.sort((a,b)=>Math.abs(a.x-lab.x)-Math.abs(b.x-lab.x))[0].str;
      if(marker==='FlD'){ (inCarry?carryDuty:dutyDays).add(d); }
      else { (inCarry?carryCodes:codeDays)[d]=marker; }
    }
    if(dutyDays.size===0 && Object.keys(codeDays).length===0) return {_fail:`strip classified but zero days matched a marker within tolerance (tol computed, ${stripSorted.length} strip labels, ${markers.length} markers)`};

    // ── Body: cluster day-label X positions into columns, walk in column order
    const dayTexts={}, carryTexts={};
    const pages=[...new Set(items.map(i=>i.page))].sort((a,b)=>a-b);
    let seenReset=false, prevBody=0;

    for(const pg of pages){
      const pageItems=items.filter(i=>i.page===pg && !(pg===1 && i.y<=fldY+20));
      const bodyLabels=pageItems.filter(i=>DAY_PREFIX.test(i.str.trim()));
      if(!bodyLabels.length) continue;

      // Cluster label X positions (labels in one column share X within ~20 units)
      const xs=bodyLabels.map(i=>i.x).sort((a,b)=>a-b);
      const cols=[];
      xs.forEach(x=>{ if(cols.length && x-cols[cols.length-1].max<25) cols[cols.length-1].max=x; else cols.push({min:x,max:x}); });
      const starts=cols.map(c=>c.min);
      const bounds=starts.map((s,i)=>[s-5, i+1<starts.length?starts[i+1]-5:Infinity]);

      for(const [lo,hi] of bounds){
        const colItems=pageItems.filter(i=>i.x>=lo&&i.x<hi).sort((a,b)=>(a.y-b.y)||(a.x-b.x));
        let cur=null, curCarry=false;
        for(const it of colItems){
          const s=it.str.trim();
          const m=DAY_PREFIX.exec(s);
          // Day label starts a block when it sits at the column's left edge
          if(m && it.x < lo+45){
            const d=parseInt(m[2]);
            if(!seenReset && prevBody>=20 && d<prevBody && d<=14) seenReset=true;
            prevBody=d;
            cur=d; curCarry=seenReset;
            const bucket=curCarry?carryTexts:dayTexts;
            if(!bucket[d]) bucket[d]=[];
            const rest=s.slice(m[0].length); // handle concatenated "Wed03C/IORD0600"
            if(rest) bucket[d].push(rest);
          } else if(cur!=null){
            (curCarry?carryTexts:dayTexts)[cur].push(s);
          }
        }
      }
    }

    // ── Flight extraction — tolerant of missing spaces (concatenated items) ──
    function extractFlights(texts){
      const t=(texts||[]).join(' ');
      const out=[], seen=new Set();
      const rx=/(DH\s*\/?\s*)?G7\s*(\d{3,4})(?:\s*\/\s*\d+)?\s*([A-Z]{3})\s*!?\s*(\d{4})\s*!?\s*(\d{4})\s*([A-Z]{3})\s*([A-Z][A-Z0-9]{1,3})/g;
      let m;
      while((m=rx.exec(t))!==null){
        const key=m[2]+m[3]+m[6];
        if(seen.has(key)) continue;
        seen.add(key);
        out.push({
          flightNum:`G7 ${m[2]}`, dep:m[3],
          depTime:`${m[4].slice(0,2)}:${m[4].slice(2)}`,
          arr:m[6], arrTime:`${m[5].slice(0,2)}:${m[5].slice(2)}`,
          acType:m[7]||"", schedBlockMins:null, isDeadhead:!!m[1],
        });
      }
      return out;
    }
    ...
    for(let d=1;d<=dim;d++){
      ...
      if(dutyDays.has(d)){
        const flights=extractFlights(dayTexts[d]);
        calendar.push({day:d,dow,isOff:false,dutyCode:null,flights});
      } else if(codeDays[d]){
        const flights=extractFlights(dayTexts[d]);
        calendar.push({day:d,dow,isOff:false,dutyCode:codeDays[d],flights});
      } else {
        calendar.push({day:d,dow,isOff:true,dutyCode:null,flights:[]});
      }
    }

    // ── SELF-VALIDATION: complete or defer ────────────────────────────────────
    // 1. Every FlD duty day must have produced flights
    for(const d of dutyDays){
      const day=calendar.find(c=>c.day===d);
      if(!day || day.flights.length===0) return {_fail:`FlD duty day ${d} produced zero flights — extraction regex found nothing in its column text: ${JSON.stringify((dayTexts[d]||[]).slice(0,3))}`};
    }
    // 2. No off day may have C/I content lurking in its column (unrecognized duty)
    for(const day of calendar){
      if(day.isOff && dayTexts[day.day] && /C\/I/.test((dayTexts[day.day]||[]).join(' '))) return {_fail:`day ${day.day} marked off but has C/I content in its column — unrecognized duty day: ${JSON.stringify((dayTexts[day.day]||[]).slice(0,3))}`};
    }

    // ── Carryover days ────────────────────────────────────────────────────────
    const carryForwardDays=[];
    for(const d of carryDuty){
      const flights=extractFlights(carryTexts[d]);
      if(flights.length===0) return {_fail:`carryover duty day ${d} produced zero flights`};
      const nd=new Date(year,monthNum+1,d);
      carryForwardDays.push({day:d,dow:DOW[nd.getDay()],isOff:false,dutyCode:null,flights});
    }
    for(const d of Object.keys(carryCodes).map(Number)){
      const nd=new Date(year,monthNum+1,d);
      carryForwardDays.push({day:d,dow:DOW[nd.getDay()],isOff:false,dutyCode:carryCodes[d],flights:extractFlights(carryTexts[d])});
    }
    ...
    return {
      ...
      _layer:'netline_items', uploadedAt:new Date().toISOString(),
    };
  }
...
function ActiveLogsPage({user, rosters, tails, onRosterUpdated, onTailSaved, onDeleteRoster}) {
  ...
  // -- Carryover verification (next-month tab inside VerifyModal) --
  const [verifyTab, setVerifyTab] = useState("current"); // "current" | "nextmonth"
  const [currentConfirmed, setCurrentConfirmed] = useState(false);
  const [nextMonthConfirmed, setNextMonthConfirmed] = useState(false);
  const [nextEditEdits, setNextEditEdits] = useState({}); // edits for the next-month carryover tab
  ...
  function openVerify(r){
    setVerifyModal(r);
    setVerifyTab("current");
    setCurrentConfirmed(false);
    setNextMonthConfirmed(false);
    setNextEditEdits({});
    setEditEdits({});
    setAuditLog([]);
  }
  ...
  async function applyAndSignRoster(roster, edits){
    ...
    await db_saveRoster(user.id,{...roster,calendar:nc,signed:true,signedAt:new Date().toISOString()});
    onRosterUpdated(roster.id,nc);
  ...
  async function signMonth(){
    ...
    // Require both months confirmed before any signing happens when a
    // carryover exists -- neither roster is saved until both checkboxes
    // are ticked, so there's no partial/inconsistent state.
    if(!currentConfirmed){ setVerifyTab("current"); return; }
    if(hasCarryover && !nextMonthConfirmed){ setVerifyTab("nextmonth"); return; }

    setSigning(true);
    try {
      const currentAudit = await applyAndSignRoster(r, editEdits);
      let nextAudit=[];
      if(hasCarryover){
        nextAudit = await applyAndSignRoster(nextMonthRoster, nextEditEdits);
      }
      setAuditLog([...currentAudit,...nextAudit]);
      setVerifyModal(null);
      setEditEdits({});
      setNextEditEdits({});
      setVerifyTab("current");
  ...
  function VerifyModal(){
    ...
    // Build rows for whichever tab is active -- "current" uses the normal
    // editEdits/auditLog state, "nextmonth" uses the separate
    // nextEditEdits state so edits don't bleed between the two months.
    const activeRoster = verifyTab==="nextmonth" ? nextMonthRoster : r;
    const activeEdits = verifyTab==="nextmonth" ? nextEditEdits : editEdits;
    const setActiveEdits = verifyTab==="nextmonth" ? setNextEditEdits : setEditEdits;
    const activeMNum = activeRoster ? (activeRoster.monthNum??activeRoster.month_num??0) : mNum;

    const allRows=[];
    (activeRoster?.calendar||[]).forEach((d,di)=>{
      (d.flights||[]).forEach((f,fi)=>{
        const tk=`${activeRoster.id}-${di}-${fi}`;
        const t=tails[tk]||{};
        const dateStr=`${activeRoster.year}-${String(activeMNum+1).padStart(2,"0")}-${String(d.day).padStart(2,"0")}`;
    ...
    const totalBlock=allRows.reduce((acc,{tk,f})=>{
      const ed=activeEdits[tk];
      const mins=ed?.actualBlockMins!=null?ed.actualBlockMins:(tails[tk]?.actualBlockMins??schedMins(f)??0);
      return acc+mins;
    },0);

    const isConfirmed = verifyTab==="nextmonth" ? nextMonthConfirmed : currentConfirmed;
    const setIsConfirmed = verifyTab==="nextmonth" ? setNextMonthConfirmed : setCurrentConfirmed;
    const bothConfirmed = currentConfirmed && (!hasCarryover || nextMonthConfirmed);
    ...
    return(
      <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",background:"rgba(0,0,0,0.5)",backdropFilter:"blur(4px)",overflowY:"auto",padding:"24px 16px"}}>
        <div style={{background:S.surface,borderRadius:24,width:"100%",maxWidth:840,boxShadow:"0 32px 80px rgba(0,0,0,0.2)",overflow:"hidden",marginBottom:24}}>
          ...
          <div style={{background:"linear-gradient(135deg,#1D4ED8,#7C3AED)",padding:"24px 28px",color:"#fff"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                ...
                <h2 style={{fontSize:24,fontWeight:900,margin:0,letterSpacing:"-0.5px"}}>{MONTHS[activeMNum]} {activeRoster?.year} Flight Log</h2>
                <p style={{fontSize:13,color:"rgba(255,255,255,0.7)",marginTop:6}}>
                  {hasCarryover
                    ? "This roster includes flights carried into next month. Review and confirm both months before signing."
                    : "Review all flights, make corrections, then sign to lock this record permanently."}
                </p>
              </div>
              <button onClick={()=>{setVerifyModal(null);setEditEdits({});setNextEditEdits({});setAuditLog([]);}} style={{background:"rgba(255,255,255,0.15)",border:"none",color:"#fff",width:36,height:36,borderRadius:"50%",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>×</button>
            </div>

            {/* Carryover tab switcher -- only shown when this roster has a
                next-month stub created by the auto-carryover logic */}
            {hasCarryover&&(
              <div style={{display:"flex",gap:8,marginTop:16}}>
                {[
                  ["current", `${MONTHS[mNum]} ${r.year}`, currentConfirmed],
                  ["nextmonth", `${MONTHS[(mNum===11?0:mNum+1)]} ${mNum===11?r.year+1:r.year} (Carryover)`, nextMonthConfirmed],
                ].map(([tid,label,confirmed])=>(
                  <button key={tid} onClick={()=>setVerifyTab(tid)} style={{
                    display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,
                    border:"none",cursor:"pointer",fontSize:12,fontWeight:700,
                    background:verifyTab===tid?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.1)",
                    color:"#fff",
                  }}>
                    {confirmed&&<svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="#4ADE80" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div style={{display:"flex",gap:12,marginTop:18,flexWrap:"wrap"}}>
              {[
                ...
                [`${allRows.filter(({tk})=>tails[tk]?.tail||activeEdits[tk]?.tail).length}/${allRows.length}`,`Synced`],
              ].map(([val,lbl])=>(
                      ...
                      <td style={{padding:"6px 10px"}}>
                        <input
                          ...
                          onChange={e=>{
                            ...
                            setActiveEdits(p=>({...p,[tk]:{...p[tk],_blockRaw:raw}}));
                          }}
                          onBlur={e=>{
                            const raw=activeEdits[tk]?._blockRaw||e.target.value;
                            if(!raw) return;
                            ...
                            if(mins!=null){
                              ...
                              setActiveEdits(p=>({...p,[tk]:{...p[tk],actualBlockMins:mins,_blockRaw:undefined}}));
                            }
                          }}
                      ...
                      <td style={{padding:"6px 10px"}}>
                        <input
                          ...
                          onChange={e=>{
                            ...
                            setActiveEdits(p=>({...p,[tk]:{...p[tk],tail:val}}));
                          }}
                      ...
                      <td style={{padding:"6px 10px"}}>
                        <input
                          ...
                          onChange={e=>{
                            ...
                            setActiveEdits(p=>({...p,[tk]:{...p[tk],acType:val}}));
                          }}
          ...
          {/* Per-tab confirmation checkbox */}
          <div style={{padding:"16px 28px 0",background:S.surface}}>
            <label style={{display:"flex",alignItems:"flex-start",gap:10,cursor:"pointer",padding:"12px 14px",borderRadius:12,background:isConfirmed?"#ECFDF5":S.panel,border:`1px solid ${isConfirmed?"#A7F3D0":S.border}`}}>
              <input
                type="checkbox"
                checked={isConfirmed}
              ...
              />
              <span style={{fontSize:12,color:isConfirmed?"#065F46":S.ink,lineHeight:1.5}}>
                I confirm the {MONTHS[activeMNum]} {activeRoster?.year} flight data above is accurate and ready to sign.
                {verifyTab==="nextmonth"&&" This carryover month must also be confirmed before either record can be saved."}
              </span>
            </label>
          </div>
          ...
          <div style={{padding:"20px 28px",borderTop:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap",background:S.surface}}>
            <div style={{fontSize:12,color:S.muted,lineHeight:1.6,maxWidth:400}}>
              ...
              {hasCarryover&&!bothConfirmed&&(
                <div style={{marginTop:6,color:"#D97706",fontWeight:600}}>
                  Confirm both {MONTHS[mNum]} and {MONTHS[(mNum===11?0:mNum+1)]} before signing.
                </div>
              )}
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{setVerifyModal(null);setEditEdits({});setNextEditEdits({});setAuditLog([]);setVerifyTab("current");setCurrentConfirmed(false);setNextMonthConfirmed(false);}} style={{padding:"11px 22px",borderRadius:12,background:"none",border:`1.5px solid ${S.border}`,color:S.silver,fontSize:13,fontWeight:600,cursor:"pointer"}}>
                Cancel
              </button>
              <button onClick={signMonth} disabled={signing||!bothConfirmed} style={{padding:"11px 28px",borderRadius:12,background:(signing||!bothConfirmed)?"#94A3B8":"linear-gradient(135deg,#1D4ED8,#7C3AED)",border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:(signing||!bothConfirmed)?"not-allowed":"pointer",boxShadow:bothConfirmed?"0 4px 16px rgba(29,78,216,0.3)":"none",display:"flex",alignItems:"center",gap:8}}>
                {signing?(
                  ...
                  <><svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>{hasCarryover?"Sign & Lock Both Months":"Sign & Lock Record"}</>
                )}
              </button>
            </div>
          </div>
    ...
    return(
      <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        <div style={{padding:"14px 18px",background:S.surface,borderBottom:`1px solid ${S.border}`,display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10,backdropFilter:"blur(12px)"}}>
          ...
          {isSigned&&<span style={{fontSize:11,fontWeight:700,color:"#2563EB",background:"#EFF6FF",padding:"4px 10px",borderRadius:100,border:"1px solid #BFDBFE",flexShrink:0,marginRight:8}}>🔒 Verified</span>}
          {!isSigned&&onDeleteRoster&&(
            <button
              ...
              style={{width:36,height:36,borderRadius:"50%",background:"#FEF2F2",border:"1px solid #FECACA",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}
            >
        ...
        <div style={{padding:"16px 16px 80px"}}>
          <button onClick={()=>openVerify(r)} style={{width:"100%",padding:"13px",borderRadius:14,background:isSigned?"none":`linear-gradient(135deg,${S.blue},#7C3AED)`,border:isSigned?`1.5px solid ${S.border}`:"none",color:isSigned?S.muted:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:16,boxShadow:isSigned?"none":`0 4px 16px ${S.blue}30`}}>
...
const MORE_PAGES = ["map","profile","settings","membership","support","export","referral"];
...
function BottomTabBar({page, setPage, user}) {
  ...
  const pilotTabs = [
    ...
    },
    {
      ...
      icon:(active)=>(
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <rect x="3" y="12" width="4" height="9" rx="1" fill={active?BLUE:MUTED}/>
          <rect x="10" y="7" width="4" height="14" rx="1" fill={active?BLUE:MUTED}/>
          <rect x="17" y="3" width="4" height="18" rx="1" fill={active?BLUE:MUTED}/>
        </svg>
      ),
    },
    {
      ...
      isPopupTrigger: true,
      icon:(active)=>(
        <div style={{width:42,height:42,borderRadius:14,background:showAddPopup?BLUE:"linear-gradient(135deg,#1D4ED8,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:showAddPopup?"0 4px 14px rgba(29,78,216,0.4)":"0 4px 14px rgba(29,78,216,0.3)",marginBottom:-2,transition:"transform .15s",transform:showAddPopup?"rotate(45deg)":"none"}}>
...
export default function App() {
  ...
  function navigate(newPage){
    ...
    if(newPage==="calendar"){ setLogbookInitialTab("roster"); newPage="logbook"; }
    else if(newPage==="active-logs"){ setLogbookInitialTab("activelogs"); newPage="logbook"; }
