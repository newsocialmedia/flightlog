...
function UploadPage({user, onRosterSaved}) {
  ...
  async function handleFile(file) {
    ...
    setFileName(file.name); setPreview(""); setCorrections([]); setStatus("loading"); setMsg("Reading roster with AI…");
    try {
      ...
      const verified = !!roster._verified;
      ...
      const dutyDays = roster.calendar.filter(d=>d.flights.length>0).length;
      setMsg(verified
        ? `✓ Saved ${dutyDays} duty days for ${roster.periodLabel} — double-checked against the original PDF`
        : `✓ Saved ${dutyDays} duty days for ${roster.periodLabel}`);
      onRosterSaved(saved);
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        {corrections.length>0&&(
          <div className="notice" style={{marginTop:12}}>
            <div style={{fontWeight:600,marginBottom:6}}>🔍 The verification pass caught and fixed {corrections.length} issue{corrections.length!==1?"s":""}:</div>
