...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden",background:C.base,position:"relative"}}>
      ...
      {/* Backdrop */}
      {sheetOpen&&<div onClick={closeSheet} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.3)",zIndex:99}}/>}
            ...
            return(
              <div key={fi} onClick={()=>{
                ...
                <div style={{minWidth:50,flexShrink:0,marginRight:12}}>
                  <div style={{fontSize:15,fontWeight:700,color:C.ink}}>{hasActual?t.actualDep:f.depTime}</div>
                  <div style={{fontSize:11,color:C.muted,marginTop:2}}>{hasActual?t.actualArr:f.arrTime}</div>
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}>
                    <span style={{fontSize:12,fontWeight:700,color:C.teal}}>{f.flightNum}</span>
                    {f.acType&&<span style={{fontSize:10,color:C.muted,background:C.panel,padding:"1px 5px",borderRadius:3}}>{f.acType}</span>}
                    {(dist||0)>50&&<span style={{fontSize:9,color:C.teal,background:C.teal+"18",padding:"1px 5px",borderRadius:3,fontWeight:700}}>XC</span>}
                    {solar.nightMins>0&&<span style={{fontSize:9,color:"#7C3AED",background:"#EDE9FE22",padding:"1px 5px",borderRadius:3}}>🌙</span>}
                  </div>
                  <div style={{fontSize:15,fontWeight:700,color:C.ink}}>{f.dep} → {f.arr}</div>
                  {t.tail&&<div style={{fontSize:10,fontFamily:"monospace",color:C.muted,marginTop:2}}>{t.tail}{synced&&<span style={{color:"#00875A",marginLeft:4}}>✓</span>}</div>}
                </div>
                <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                  {bMins>0&&<div style={{fontSize:13,fontWeight:700,color:C.ink}}>{fmtMins(bMins)}</div>}
...
function ExportPage({rosters, tails}) {
  return (
    <div style={{flex:1,overflowY:"auto",background:C.base,padding:"24px 16px"}}>
      ...
      <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Download your logbook in standard formats</div>
      {[
        {fmt:"CSV",desc:"Spreadsheet format — works with Excel, Google Sheets",icon:"📊",action:()=>csvExport(rosters,tails)},
        {fmt:"Jeppesen",desc:"Standard pilot logbook format",icon:"📘",action:()=>alert("Coming soon")},
        {fmt:"ASA",desc:"ASA logbook format",icon:"📗",action:()=>alert("Coming soon")},
