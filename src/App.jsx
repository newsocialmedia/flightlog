...
(function injectStyles() {
  try {
    ...
    // Fallback — styles will be injected by App on mount
  }
})();
...
async function fetchProfile(userId, accessToken) {
  try {
    ...
    if(!token) {
      ...
      token = s?.session?.access_token || SUPA_ANON;
    }
...
function SubscriptionWall({user, onSubscribed, onLogout}) {
  ...
  // Auto-poll every 4 seconds for up to 2 minutes after page loads
  // Handles the case where Stripe webhook fires while pilot is reading the page
  useEffect(()=>{
    let attempts = 0;
    const poll = setInterval(async()=>{
      attempts++;
      if(attempts > 30) { clearInterval(poll); return; } // stop after 2 min
      try{
        const {data:s} = await sb.auth.getSession();
        const token = s?.session?.access_token || SUPA_ANON;
  ...
  async function manualRefresh(){
    ...
    try{
      ...
      if(profile?.subscription_status==="active"||profile?.subscription_status==="past_due"){
        ...
        setMsg("Payment not confirmed yet. Stripe webhooks can take up to 60 seconds. Please wait and try again.");
      }
  ...
  return(
    <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",background:C.base}}>
      {/* Header */}
      <div style={{padding:"16px 20px",borderBottom:`1px solid ${C.border}`,background:C.surface,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={manualRefresh} disabled={checking} style={{
            fontSize:12,color:C.teal,background:C.teal+"12",
            border:`1px solid ${C.teal}33`,borderRadius:7,
            padding:"6px 14px",cursor:"pointer",fontWeight:700,
            display:"flex",alignItems:"center",gap:6,
          }}>
            {checking?<span className="spinner">⟳</span>:"↻"} I've paid
          </button>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"28px 16px"}}>
        <div style={{textAlign:"center",marginBottom:24,maxWidth:360}}>
          ...
          <div style={{fontSize:14,color:C.muted,lineHeight:1.65}}>
            Subscribe to unlock your full logbook. After payment, the app will open automatically — or tap <strong>I've paid</strong> above.
          </div>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:20,padding:"8px 16px",borderRadius:20,background:C.teal+"12",border:`1px solid ${C.teal}22`}}>
          ...
          <span style={{fontSize:12,color:C.teal,fontWeight:600}}>Checking for payment automatically…</span>
        </div>

        {msg&&<div style={{marginBottom:16,padding:"10px 14px",borderRadius:8,background:C.gold+"15",border:`1px solid ${C.gold}44`,color:C.gold,fontSize:13,maxWidth:400,textAlign:"center"}}>{msg}</div>}
...
export default function App() {
  ...
  async function handleAuth(u){
    ...
    // Get fresh JWT token from current session
    const { data: sessionData } = await sb.auth.getSession();
    const token = sessionData?.session?.access_token || SUPA_ANON;
