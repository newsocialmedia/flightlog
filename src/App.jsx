        ...
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>
              ...
              {rosters.map((r,i)=>(
                <tr key={i}>
                  <td style={{fontWeight:500,color:C.white}}>{r.name||r.user_name||"—"}<br/><span style={{fontSize:11,color:C.muted}}>{r.email||""}</span></td>
...
export default function App() {
  ...
  async function handleAuth(u) {
    // Ensure role is set — fetchProfile sometimes fails due to RLS timing
    // Fallback: hardcode known admin emails
    const ADMIN_EMAILS = ["admin@flightlog.app"];
    if(!u.role || u.role === "authenticated") {
