...
function AddFlightPage({user, rosters, onRosterSaved, setPage}) {
  ...
  if(success) return(
    <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif",display:"flex",flexDirection:"column",alignItems:"center",padding:"32px 16px"}}>
    ...
    <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{position:"relative",width:"100%",height:260,background:"#0B1120",overflow:"hidden",flexShrink:0,zIndex:1}}>
        {form.dep&&form.arr
          ...
          : <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(255,255,255,0.5)",fontSize:13,textAlign:"center",padding:"0 40px"}}>Enter departure and arrival to see the route</div>
        }
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed, setPage, forcedView, onOpenFlightExternal}) {
    ...
    if(dvFiltersActive){
      dailyRosters.forEach(r=>{
        ...
        (r.calendar||[]).filter(d=>(d.flights||[]).length>0).forEach(day=>{
          ...
          sorted.forEach(({f,fi})=>allDayFlights.push({f,fi,di,day,roster:r,mNum:rMNum,tk:`${r.id}-${di}-${fi}`}));
        });
      });
  ...
  },[roster,dvFiltersActive,dailyRosters,dvFilterMonth,dvFilterYear,dvFilterEquip]);
                ...
                {dvSelectMode&&(
                  <div style={{position:"sticky",top:0,zIndex:15,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"10px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                    <button onClick={()=>setDvSelected(dvSelected.size===allDayFlights.length?new Set():new Set(allDayFlights.map(r=>r.tk)))} style={{fontSize:12,fontWeight:700,color:C.teal,background:"none",border:"none",cursor:"pointer",padding:0}}>
                      {dvSelected.size===allDayFlights.length?"Deselect all":`Select all${dvFiltersActive?" (this view)":" (this month)"}`}
                    </button>
