...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    if(Array.isArray(roster.carryForwardDays) && roster.carryForwardDays.length > 0) {
      ...
      // Remove carry-forward days from this month's calendar
      // Use flight data comparison to identify which day:1, day:2 etc are carry-forward
      const carryDayNums = new Set(carryForwardDays.map((d:any) => d.day));
      // Filter: keep days that are (a) valid for this month AND (b) not carry-forward
      // For duplicate day numbers, prefer the one with matching flight data to the carry list
      thisDays = cal.filter((d:any) => {
        if(d.day < 1 || d.day > daysInMonth) return false;
        if(!carryDayNums.has(d.day)) return true;
        // Same day number exists in carry list — check if this IS the carry day
        const carryVersion = carryForwardDays.find((c:any) => c.day === d.day);
        if(!carryVersion) return true;
        // If flights match carry version exactly, this IS the carry day — exclude it
        const sameFlights = JSON.stringify(d.flights) === JSON.stringify(carryVersion.flights);
        return !sameFlights; // keep if different (real Feb day 1), exclude if same (carry Mar day 1)
      });
    ...
    if(carryForwardDays.length > 0) {
      ...
      if(normalizedCarry.length > 0) {
        ...
        if(nextRoster?.id) {
          // Merge: carry-forward days fill in any gaps in the existing roster.
          // If a day already exists in the next month's roster, keep the roster
          // version (it's from the actual monthly upload — more authoritative).
          // Only add carry-forward days that don't already exist.
          const existingCal = nextRoster.calendar || [];
          const existingDayNums = new Set(existingCal.map((d:any) => d.day));
          const newDays = normalizedCarry.filter((d:any) => !existingDayNums.has(d.day));
          if(newDays.length > 0) {
            const merged = [...existingCal, ...newDays].sort((a:any,b:any)=>a.day-b.day);
            await sb.from("rosters").update({calendar:merged}).eq("id",nextRoster.id);
          }
          // If all carry-forward days already exist in the roster — nothing to do
        } else {
    ...
    if(existing?.id) {
      // Merge: start with the new upload's days, then fill in any days from the
      // existing roster that the new upload doesn't cover (e.g. carry-forward days
      // from a previous month's upload that aren't in the new PDF).
      const existingCal = existing.calendar || [];
      const newDayNums = new Set(thisDays.map((d:any) => d.day));
      // Keep existing days that are NOT overridden by the new upload
      const preservedDays = existingCal.filter((d:any) => !newDayNums.has(d.day));
      const mergedCal = [...thisDays, ...preservedDays].sort((a:any,b:any)=>a.day-b.day);
