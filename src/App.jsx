...
function computeNightTime(dateStr, depCode, arrCode, depTime, arrTime) {
  ...
  const depOffMin = getAirportUtcOffsetMins(depCode); // minutes, e.g. -300 for EST
  const arrOffMin = getAirportUtcOffsetMins(arrCode);
...
function UploadPage({user, onRosterSaved, setPage}) {
  ...
  if(parsedRoster) {
    ...
    return(
      <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        {/* Success header */}
        <div style={{background:`linear-gradient(135deg,#10B981,#059669)`,padding:"28px 22px 24px",color:"#fff"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            <div style={{width:48,height:48,borderRadius:"50%",background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
            <div>
              <div style={{fontSize:12,fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.75)"}}>Roster Parsed</div>
              <h2 style={{fontSize:22,fontWeight:900,margin:"2px 0 0",letterSpacing:"-.5px"}}>{MONTHS_FULL[mNum]} {parsedRoster.year}</h2>
            </div>
          </div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {[[`${dutyDays}`,"Duty Days"],[`${allRows.length}`,"Flights"],[fmtMins(totalBlock),"Sched Block"]].map(([val,lbl])=>(
              <div key={lbl} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",padding:"7px 14px",borderRadius:100,border:"1px solid rgba(255,255,255,0.2)"}}>
                <span style={{fontSize:15,fontWeight:800,color:"#fff"}}>{val}</span>
                <span style={{fontSize:11,color:"rgba(255,255,255,0.75)",marginLeft:6}}>{lbl}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:"16px 16px 100px"}}>
          <div style={{fontSize:12,fontWeight:700,color:S.muted,textTransform:"uppercase",letterSpacing:".5px",marginBottom:10}}>
            Review extracted flights
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {allRows.map(({f,dateStr},idx)=>(
              <div key={idx} style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:14,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
          ...
          {/* Action buttons */}
          <div style={{position:"fixed",bottom:0,left:0,right:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"14px 16px",display:"flex",gap:10,zIndex:50}}>
            <button
