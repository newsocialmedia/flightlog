...
function FlightRouteMap({dep, arr, dist, flightDateStr, flightDepTime, S, compact, hero}) {
  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      // Shouldn't normally happen with a plain remount (a key change gives
      // React a genuinely new DOM node), but the theme toggle's app-wide
      // key={themeKey} bump forces this component to tear down and rebuild
      // in the same breath as everything else on the page, and that's
      // exactly the kind of rapid destroy/recreate churn that has, in
      // practice, occasionally left a stale _leaflet_id on the element
      // before this effect's own cleanup finished running. L.map() throws
      // "Map container is already initialized" on such a node -- and since
      // that throw happens inside a plain useEffect (not during render),
      // React's ErrorBoundary never sees it: the exception just aborts this
      // function silently, leafletMapRef.current is never set, mapReady
      // never flips true, and that ONE flight's map is permanently blank
      // while every other flight (mounting fresh, never hitting this) works
      // fine -- which matches "some maps break on theme toggle, others
      // don't" exactly. Clearing the marker first makes L.map() safe either
      // way.
      ...
      try {
        ...
        // rechecks it on its own. During the theme toggle's whole-app
        // remount, this container can commit into the DOM a moment before
        // the browser has finished settling its final layout box -- Leaflet
        // then renders its tile grid for whatever size it read at that
        // instant, which can leave the map visibly blank or wrongly tiled
        // even though initialization itself "succeeded" with no error.
        // invalidateSize() forces a fresh measurement; calling it once
        // immediately and once more shortly after covers both a same-tick
        // layout mismatch and any slightly slower settling.
        map.invalidateSize();
