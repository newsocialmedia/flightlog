  ...
  async function parseRoster() {
    ...
    try {
      ...
      try {
        ...
        for(let p=1; p<=pdfDoc.numPages; p++){
          ...
          for(const item of content.items){
            ...
            rawItems.push({x: Math.round(tx[4]), y: Math.round(tx[5]), str: item.str, page: p});
          }
        }
        ...
        function transformItems(items, mode){
          ...
          return items.map(i=>{
            ...
            return {x, y, str:i.str, page:i.page};
          });
        }
      ...
      if(!roster) {
        ...
        // If the layout-text attempt failed server-side, retry ONCE with the raw
        // PDF only. This gives the server a fresh time budget for the vision
        // pipeline instead of chaining both pipelines into one timed-out request.
        if(!response.ok && layoutReliable) {
