...
function AppLandingPage({onLogin, onSignup}) {
  ...
  useEffect(()=>{
    isWebAuthnAvailable().then(setBioAvailable);
  },[]);
  ...
  async function bioSignIn() {
    ...
    try {
      const session = await authenticateWithBiometric(savedUserId);
      if(!session?.user) throw new Error("Could not restore session.");
      // Reload full profile
      const {data:profile} = await sb.from("profiles").select("*").eq("id", session.user.id).single();
      const fullUser = {...session.user,...(profile||{})};
      window.dispatchEvent(new CustomEvent("fl-bio-auth", {detail: fullUser}));
    } catch(e) {
      setBioErr(e.message||"Biometric sign-in failed. Try signing in manually.");
    } finally { setBioLoading(false); }
  }
