...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    // ── Extract carry-forward days FIRST, before checking if roster exists ──
    // This runs regardless of whether we're inserting or updating, so March days
    // in a February roster always get routed to the March roster.
    ...
    if(Array.isArray(roster.carryForwardDays) && roster.carryForwardDays.length > 0) {
      ...
      // Detect carry-forward by two patterns:
      // Pattern A: day counter resets (28 → 1 means March 1 after Feb 28)
      ...
      // Pattern B: days explicitly numbered beyond month end (day 29-31 in Feb)
      ...
      carryForwardDays = rawCarry.filter(d => d.day >= 1 && d.day <= 5);
