    ...
    for(const roster of rosters){
      ...
      for(let di=0;di<(roster.calendar||[]).length;di++){
        ...
        for(let fi=0;fi<(day.flights||[]).length;fi++){
          ...
          dt.setHours(h,m,0,0);
          if(dt>now) upcoming.push({f,day,roster,di,fi,tk,tail,dateStr,dt});
        }
      }
    }
        ...
        {nextFlight&&(()=>{
          ...
          return(
            <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14,marginBottom:20}}>
              {/* Flight card */}
              <div style={{background:S.surface,borderRadius:24,padding:"20px 22px",border:`1px solid ${S.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.06)",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{position:"relative",zIndex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      ...
                      <div style={{fontSize:14,fontWeight:700,color:S.ink,background:S.panel,padding:"6px 12px",borderRadius:8,border:`1px solid ${S.border}`,fontFamily:"monospace"}}>{tail.tail||"Pending sync"}</div>
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        onAutoLookup={async()=>{
          ...
          try{
            ...
            if(d.tail||d.actualDep||d.actualArr){
              const updates={tail:d.tail||tail.tail,actualDep:d.actualDep||tail.actualDep,actualArr:d.actualArr||tail.actualArr,actualBlockMins:d.actualBlockMins??tail.actualBlockMins};
              const nc=[...(r.calendar||[])];
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.base}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,background:C.surface,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,flexShrink:0}}>
        <div style={{display:"flex",gap:8,overflowX:"auto",flexWrap:"nowrap"}}>
          {rosters.map((r,i)=>(
            <button key={r.id} onClick={()=>setSelRoster(i)} style={{padding:"5px 14px",borderRadius:100,fontSize:12,fontWeight:600,border:"none",cursor:"pointer",flexShrink:0,background:selRoster===i?C.teal:C.panel,color:selRoster===i?"#fff":C.muted}}>
              {r.periodLabel}
            </button>
          ))}
        </div>
        {/* View toggle */}
        <div style={{display:"flex",background:C.panel,borderRadius:10,padding:3,gap:2,flexShrink:0}}>
          {[["daily","Daily"],["logbook","Logbook"]].map(([v,label])=>(
        ...
        {!roster?(
          ...
          <>
            ...
            <div style={{margin:"4px 0 16px"}}>
              {!lbAddForm.show?(
                <button onClick={()=>setLbAddForm(f=>({...f,show:true}))} style={{width:"100%",padding:"12px",borderRadius:10,background:"none",border:`1.5px dashed ${C.border}`,color:C.muted,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  const S={
    bg:"#F8FAFC",surface:"#FFFFFF",border:"#E2E8F0",
    ink:"#0F172A",muted:"#64748B",silver:"#475569",
    blue:"#1D4ED8",purple:"#7C3AED",panel:"#F1F5F9",
...
function UploadPage({user, onRosterSaved}) {
  ...
  async function parseRoster() {
    ...
    setParsing(true); setStatus(null); setMsg("");
    try {
    ...
    <div style={{flex:1,overflowY:"auto",background:C.base}}>
      ...
      <div style={{padding:16,maxWidth:560}}>
        ...
        <div
          ...
          style={{
            border:`2px dashed ${drag?C.teal:file?C.teal:C.border}`,
            borderRadius:20,padding:"36px 20px",textAlign:"center",cursor:"pointer",
            background:drag?C.teal+"08":file?C.teal+"05":C.surface,
        ...
        >
          ...
          {file?(
            <>
              <div style={{fontSize:15,fontWeight:700,color:C.teal,marginBottom:4}}>{file.name}</div>
              <div style={{fontSize:12,color:C.muted}}>{(file.size/1024).toFixed(1)} KB · Tap to change</div>
            </>
          ):(
            <>
              <div style={{fontSize:15,fontWeight:700,color:C.ink,marginBottom:6}}>Drop your roster PDF here</div>
              <div style={{fontSize:13,color:C.muted,lineHeight:1.5}}>or tap to browse · Any airline format</div>
            </>
          )}
        </div>
        ...
        {/* Status */}
        {status&&(
          <div style={{marginBottom:16,padding:"12px 14px",borderRadius:10,background:status==="success"?C.teal+"12":C.red+"12",border:`1px solid ${status==="success"?C.teal+"33":C.red+"33"}`,fontSize:13,color:status==="success"?C.teal:C.red,fontWeight:500}}>
            {msg}
          </div>
        )}
        ...
        <button
          ...
          style={{
            width:"100%",padding:"15px",borderRadius:14,
            background:!file||parsing?C.panel:C.teal,
            border:"none",color:!file||parsing?C.muted:"#fff",
        ...
        {/* Info */}
        <div style={{background:C.panel,borderRadius:12,padding:"14px 16px",border:`1px solid ${C.border}`}}>
          <div style={{fontSize:13,fontWeight:700,color:C.ink,marginBottom:8}}>How it works</div>
          {[
...
function MorePage({user, setPage, onLogout, isDark, onToggleTheme, rosters, tails}) {
  ...
  const S = {
    bg:"#F8FAFC", surface:"#FFFFFF", border:"#E2E8F0",
    ink:"#111827", muted:"#64748B", silver:"#475569",
    blue:"#1D4ED8", purple:"#7C3AED", panel:"#F1F5F9",
  };

  const pilotItems = [
    {id:"analytics",   label:"Stats",                  bg:"#EFF6FF", color:"#1D4ED8",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="12" width="4" height="9" rx="1" fill="#1D4ED8"/><rect x="10" y="7" width="4" height="14" rx="1" fill="#1D4ED8"/><rect x="17" y="3" width="4" height="18" rx="1" fill="#1D4ED8"/></svg>},
    {id:"map",         label:"Route Map",               bg:"#ECFDF5", color:"#10B981",
    ...
    {id:"profile",     label:"Profile",                 bg:"#FFF7ED", color:"#F97316",
    ...
    {id:"membership",  label:"Manage Subscription",     bg:"#EFF6FF", color:"#1D4ED8",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="2" y="5" width="20" height="14" rx="2" stroke="#1D4ED8" strokeWidth="2"/><path d="M2 10h20" stroke="#1D4ED8" strokeWidth="2"/><path d="M6 15h4" stroke="#1D4ED8" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"referral",    label:"Refer & Earn",            bg:"#FFF7ED", color:"#F97316",
    ...
    {id:"support",     label:"Support",                 bg:"#ECFDF5", color:"#10B981",
    ...
    {id:"export",      label:"Export Logbook",          bg:"#EFF6FF", color:"#1D4ED8",
    ...
    {id:"settings",    label:"Settings",                bg:"#F5F3FF", color:"#7C3AED",
  ...
  const adminItems = [
    {id:"admin-overview",  label:"Admin Overview",    bg:"#EFF6FF", color:"#1D4ED8",
    ...
    {id:"admin-users",     label:"User Management",   bg:"#FFF7ED", color:"#F97316",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" stroke="#F97316" strokeWidth="2" strokeLinecap="round"/><circle cx="9" cy="7" r="4" stroke="#F97316" strokeWidth="2"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" stroke="#F97316" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"admin-analysis",  label:"Analysis",          bg:"#EFF6FF", color:"#1D4ED8",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="12" width="4" height="9" rx="1" fill="#1D4ED8"/><rect x="10" y="7" width="4" height="14" rx="1" fill="#1D4ED8"/><rect x="17" y="3" width="4" height="18" rx="1" fill="#1D4ED8"/></svg>},
    {id:"admin-rosters",   label:"All Rosters",       bg:"#F5F3FF", color:"#7C3AED",
    ...
    {id:"admin-settings",  label:"Admin Settings",    bg:"#F5F3FF", color:"#7C3AED",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="3" stroke="#7C3AED" strokeWidth="2"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z" stroke="#7C3AED" strokeWidth="2"/></svg>},
    {id:"profile",         label:"Profile",           bg:"#FFF7ED", color:"#F97316",
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4" stroke="#F97316" strokeWidth="2"/><path d="M4 20c0-3 3.6-5 8-5s8 2 8 5" stroke="#F97316" strokeWidth="2" strokeLinecap="round"/></svg>},
  ];
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base}}>
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row"}}>
            ...
            <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",position:"relative",minHeight:0}}>
              ...
              {page==="more"&&<MorePage user={user} page={page} setPage={navigate} onLogout={handleLogout} isDark={isDark} onToggleTheme={handleToggleTheme} rosters={rosters} tails={tails}/>}
