    ...
    for(const roster of rosters) {
      for(let di=0; di<(roster.calendar||[]).length; di++) {
        ...
        for(let fi=0; fi<(day.flights||[]).length; fi++) {
          ...
          if(flightDt > now) return { f, day, roster, di, fi, flightDt };
        }
      }
    }
    return null;
    ...
    <div>
      ...
      {nextFlight&&(
        <div style={{
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:11,color:C.teal,fontWeight:700,letterSpacing:".8px",textTransform:"uppercase",marginBottom:2}}>Next Flight</div>
            <div style={{fontSize:18,fontWeight:700,color:C.ink}}>
              {nextFlight.f.flightNum} · {nextFlight.f.dep} → {nextFlight.f.arr}
            </div>
            <div style={{fontSize:13,color:C.silver,marginTop:2}}>
              {nextFlight.day.dow} {String(nextFlight.day.day).padStart(2,"0")} · Dep {nextFlight.f.depTime} · {nextFlight.f.acType}
            </div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:22,fontWeight:700,color:C.teal}}>{countdown(nextFlight.flightDt)}</div>
        ...
        <table style={{width:"max-content",minWidth:"100%",borderCollapse:"collapse",fontSize:12,fontFamily:"Inter,system-ui,sans-serif"}}>
          ...
          <tbody>
            ...
            {rows.map(({di,fi,f,tail,tk,solar,dist,blockMins,day},idx)=>{
              ...
              return (<>
                <tr key={tk} onClick={()=>setExpandedRow(isExp?null:tk)} style={{cursor:"pointer",transition:"background .1s"}}>
