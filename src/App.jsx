...
function SubscriptionWall({user, onSubscribed, onLogout}) {
  ...
  async function checkSubscription() {
    try {
      // Use current token directly — no need to refresh each poll
      const token = sb.auth._token || SUPA_ANON;

      // Fetch profile directly with cache-busting to avoid stale data
      const res = await fetch(
        `${SUPA_URL}/rest/v1/profiles?id=eq.${user.id}&select=subscription_status,plan,subscription_end&limit=1`,
        {headers:{
          "apikey": SUPA_ANON,
          "Authorization": `Bearer ${token}`,
