...
async function fetchProfile(userId, accessToken) {
  try {
    ...
    if(!token) {
      try {
        const refreshed = await sb.auth.refreshSession();
        token = refreshed?.access_token || sb.auth._token || SUPA_ANON;
      } catch {
        token = sb.auth._token || SUPA_ANON;
      }
    }
...
function SubscriptionWall({user, onSubscribed, onLogout}) {
  ...
  async function checkSubscription() {
    try {
      // Refresh token first to get latest session
      let token = sb.auth._token || SUPA_ANON;
      try {
        const refreshed = await sb.auth.refreshSession();
