...
function buildJeppesenRows(rosters, tails) {
  ...
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    ...
    const flightSimStr="";
    const dualStr="";
    rows.push([dateStr,f.acType||"","N/"+(t.tail||""),f.dep,f.arr,appCount,remarks,toCount,ldgCount,selStr,multiEngStr,nightStr,actualIfrStr,simIfrStr,xcStr,instructorStr,dualStr,sicStr,flightSimStr,picStr,isDH?"":block]);
  })));
  return rows;
}
...
function jeppesenExport(rosters, tails) {
  ...
  const header=["Date","Aircraft Type","Aircraft Ident","From","To","Nr. Inst. App.","Remarks and Endorsements","Nr T/O","Nr Ldg","SEL","MEL","Night","Actual Instr","Sim Instr","Cross Country","Instructor","Dual Received","SIC/Copilot","Flight Sim","PIC","Total Duration of Flight"];
...
async function jeppesenExportPdf(rosters, tails) {
  ...
  const dataRows = buildJeppesenRows(rosters, tails);
...
function buildAsaRows(rosters, tails) {
  ...
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    ...
    const appCount=f.loggedApproaches>0?String(f.loggedApproaches):"";
    const dayStr="";
    rows.push([dateStr,f.acType||"","N/"+(t.tail||""),f.dep,f.arr,selStr,multiEngStr,groundTrainerStr,dualStr,picStr,dayStr,nightStr,xcStr,actualIfrStr,simIfrStr,appCount,ldgStr,isDH?"":block,remarks]);
  })));
  return rows;
}
...
async function asaExportPdf(rosters, tails) {
  const jsPDF = await loadJsPdf();
  const doc = new jsPDF({orientation:"landscape", unit:"pt", format:"letter"});
  ...
  const dataRows = buildAsaRows(rosters, tails);
                  ...
                  <div style={{position:"sticky",top:0,zIndex:15,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"10px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                    ...
                    {dvSelected.size>0&&(
                      <div style={{display:"flex",gap:6,marginLeft:"auto",flexWrap:"wrap"}}>
                        ...
                        <button onClick={()=>setDvBulkPanel(dvBulkPanel==="acType"?null:"acType")} style={{padding:"6px 12px",borderRadius:8,border:`1.5px solid ${dvBulkPanel==="acType"?C.teal:C.border}`,background:dvBulkPanel==="acType"?`${C.teal}12`:"none",color:dvBulkPanel==="acType"?C.teal:C.ink,fontSize:12,fontWeight:600,cursor:"pointer"}}>Aircraft Type</button>
                      </div>
                    )}
          ...
          {!editingDetails?(()=>{
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              <div>
                ...
                <div style={{fontSize:10,color:S.muted,marginTop:5}}>Leave unset to classify automatically from Aircraft Type.</div>
              </div>
...
function OnboardingStartPage({setPage}) {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 20px",background:C.bg,textAlign:"center",minHeight:"70vh"}}>
      <div style={{width:64,height:64,borderRadius:20,background:`linear-gradient(135deg,${C.teal},#60A5FA)`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20,boxShadow:`0 8px 24px ${C.teal}40`}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:320}}>
        <button
          ...
          style={{padding:"16px 20px",borderRadius:16,background:`linear-gradient(135deg,${C.teal},#3B82F6)`,border:"none",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:12,textAlign:"left",boxShadow:`0 4px 16px ${C.teal}30`}}
        >
        ...
        >
          <span style={{width:38,height:38,borderRadius:11,background:C.panel,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke={C.teal} strokeWidth="2.4" strokeLinecap="round"/></svg>
          </span>
          <span>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:S.bg,minHeight:"70vh"}}>
      ...
      <div style={{padding:"0 24px 28px",display:"flex",flexDirection:"column",gap:16,alignItems:"center"}}>
        <div style={{display:"flex",gap:6}}>
          {slides.map((_,i)=>(
            <button key={i} onClick={()=>setSlide(i)} aria-label={`Go to slide ${i+1}`} style={{width:i===slide?20:6,height:6,borderRadius:100,background:i===slide?S.teal:S.border,border:"none",cursor:"pointer",transition:"all .2s",padding:0}}/>
          ))}
        </div>
        <button
          onClick={()=>isLast?finish():setSlide(s=>s+1)}
          style={{width:"100%",maxWidth:280,padding:"14px",borderRadius:16,background:`linear-gradient(135deg,${S.teal},#3B82F6)`,border:"none",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 16px ${S.teal}30`}}
        >
