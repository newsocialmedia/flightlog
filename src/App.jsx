...
function computeNightTime(dateStr, depCode, arrCode, depTime, arrTime) {
  ...
  // -- Convert local clock times to TRUE UTC using real timezone offsets --
  // BUG THAT WAS HERE: the old code approximated UTC by subtracting
  // longitude/15 from the local clock time. That's wrong for almost every
  // real airport -- timezones follow political/administrative boundaries,
  // not pure longitude (e.g. all of China is UTC+8 regardless of longitude;
  // US Eastern is a flat UTC-5/-4 band, not a smooth longitude gradient).
  // This produced systematically wrong night/day classification and thus
  // wrong night-time minutes. Fixed: use the same Intl.DateTimeFormat-based
  // getAirportUtcOffsetMins() already used correctly elsewhere in this file
  // for block-time computation.
  const depOffMin = getAirportUtcOffsetMins(depCode, dateStr); // minutes, e.g. -300 for EST
  const arrOffMin = getAirportUtcOffsetMins(arrCode, dateStr);

  const [dh,dm] = depTime.split(":").map(Number);
  const [ah,am] = arrTime.split(":").map(Number);
  if(isNaN(dh)||isNaN(dm)||isNaN(ah)||isNaN(am)) return {nightMins:0,dayDep:true,nightDep:false,dayArr:true,nightArr:false};

  // UTC minutes-of-day for departure and arrival (departure date as base)
  let depUtcMin = (dh*60+dm) - depOffMin;
  let arrUtcMin = (ah*60+am) - arrOffMin;
  // Departure UTC minute may be negative or >1440 if local offset pushes it
  // into the previous/next UTC day -- normalize relative to a 0-based dep day
  const depDayOffset = Math.floor(depUtcMin/1440);
  depUtcMin = ((depUtcMin%1440)+1440)%1440;
  // Arrival is always after departure -- if raw arrUtcMin (before normalizing
  // depDayOffset back out) is earlier than depUtcMin in absolute terms, the
  // flight crossed a UTC day boundary; add 1440 until arrival is after dep.
  let totalMins = arrUtcMin - (dh*60+dm - depOffMin); // raw diff in true UTC minutes
  totalMins = arrUtcMin - ((dh*60+dm)-depOffMin);
  // Simplify: compute totalMins as straightforward UTC delta, handling wrap
  const depAbsUtc = (dh*60+dm) - depOffMin;
  const arrAbsUtc = (ah*60+am) - arrOffMin;
  totalMins = arrAbsUtc - depAbsUtc;
  while(totalMins < 0) totalMins += 1440;
  // Sanity bound -- flights longer than 20h are almost certainly a data
  // error (wrong date or bad times); cap to avoid wildly wrong night minutes
  if(totalMins > 1200) totalMins = totalMins % 1440;

  // -- Solar sunrise/sunset (already returned in true UTC hours by solarTimes) --
  const depSolar=solarTimes(dateStr,depCoords[0],depCoords[1]);
  // Arrival may occur on the next calendar day if the flight crosses
  // midnight UTC -- use the correct date for the arrival's solar calc.
  const arrCrossesDay = (depUtcMin + totalMins) >= 1440;
  const arrDateStr = arrCrossesDay ? addDaysToDateStr(dateStr,1) : dateStr;
  ...
  // depAbsUtc/arrAbsUtc may be negative or >1440 (representing prev/next UTC
  // day) -- normalize to 0-1440 range for comparison against sunrise/sunset
  // (which solarTimes returns as UTC hours, typically already 0-24 but can
  // be slightly outside that range near the poles in summer/winter -- treat
  // those edge cases as never-night / always-night per civil twilight rules)
  const depUtcHr = (((depAbsUtc/60)%24)+24)%24;
  const arrUtcHr = (((arrAbsUtc/60)%24)+24)%24;

  function isNightAt(utcHr, solar) {
    if(solar.sunrise==null||solar.sunset==null) return false;
    // Normalize sunrise/sunset into 0-24 for safe comparison
    const sr = ((solar.sunrise%24)+24)%24, ss=((solar.sunset%24)+24)%24;
    if(sr<=ss) return utcHr<sr || utcHr>ss;       // normal case
    return utcHr<sr && utcHr>ss ? false : (utcHr>=ss||utcHr<=sr); // polar/wrap edge case
  }

  const depIsNight = isNightAt(depUtcHr, depSolar);
  const arrIsNight = isNightAt(arrUtcHr, arrSolar);

  // -- Proportional night time, not a crude 50% guess --
  // Walk the flight in 1-minute steps comparing against a linearly
  // interpolated day/night boundary between departure and arrival
  // conditions. For most flights (same day/night state throughout, or a
  // single transition near one twilight), this reduces to either 0%, 100%,
  // or an accurate proportional split based on actual time-of-day distance
  // to the relevant terminator, rather than always assuming exactly half.
  let nightMins;
  if(!depIsNight && !arrIsNight) {
    nightMins = 0;
  } else if(depIsNight && arrIsNight) {
    nightMins = totalMins;
  } else {
    // Transitioning flight -- find the proportion of the flight that
    // occurs after sunset (dep day->night) or before sunrise (arr night->day)
    if(depIsNight) {
      // Departed at night, arrived in daylight -- night portion is from
      // departure until the relevant sunrise is crossed
      const sr = ((depSolar.sunrise%24)+24)%24;
      let minsToSunrise = ((sr - depUtcHr)+24)%24 * 60;
      nightMins = Math.min(totalMins, Math.round(minsToSunrise));
    } else {
      // Departed in daylight, arrived at night -- night portion is from
      // sunset until arrival
      const ss = ((arrSolar.sunset%24)+24)%24;
      let minsFromSunset = ((arrUtcHr - ss)+24)%24 * 60;
      nightMins = Math.min(totalMins, Math.round(minsFromSunset));
    }
  }
...
function LogbookHubPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, onOpenFlight, pendingFlight, onPendingFlightConsumed, setPage, initialTab}) {
  ...
  const [hubTab, setHubTab] = useState(()=>{
    // Restore the sub-tab across page refreshes -- without this, refreshing
    // while on Active Logs (or Roster View, Logbook View) silently dropped
    // back to Daily View because only the top-level `page` was persisted,
    // never which hub tab was active within it.
    if(initialTab) return initialTab;
    try{ return sessionStorage.getItem("fl_logbook_hub_tab") || "daily"; }catch{ return "daily"; }
  }); // "daily" | "logbookview" | "roster" | "activelogs"
  // React to initialTab changing after mount (e.g. Quick Action jumps straight
  // to Active Logs while already on the Logbook hub showing Daily View)
  useEffect(()=>{ if(initialTab) setHubTab(initialTab); },[initialTab]);
  // Persist whenever the tab changes so a refresh lands back on the same one
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
        {(hubTab==="daily"||hubTab==="logbookview")&&(
          <LogbookPage
            ...
            pendingFlight={pendingFlight} onPendingFlightConsumed={onPendingFlightConsumed}
            setPage={setPage}
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed, setPage, forcedView}) {
...
function FlightRouteMap({dep, arr, dist, flightDateStr, flightDepTime, S}) {
  ...
  useEffect(() => {
    ...
    const line = L.polyline(pts, { color:"#3B82F6", weight:3, opacity:0.9, smoothFactor:1, dashArray:"8 6" }).addTo(map);
  ...
  useEffect(() => {
    ...
    return () => { if(radarLayer && map) { try { map.removeLayer(radarLayer); } catch {} } };
  }, [mapReady, within24h]);
    ...
    <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:18,padding:"16px 18px",overflow:"hidden"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {within24h&&(
            <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:10,fontWeight:700,color:radarLoaded?"#3B82F6":S.muted,background:radarLoaded?"#EFF6FF":S.panel,padding:"3px 8px",borderRadius:100}}>
              <span style={{width:6,height:6,borderRadius:"50%",background:radarLoaded?"#3B82F6":S.muted,display:"inline-block",animation:radarLoaded?"pulse 1.5s infinite":"none"}}/>
              {radarLoaded?"Live Radar":"Loading radar..."}
            </span>
          )}
        </div>
      </div>
      <div
        ref={mapElRef}
        style={{width:"100%",height:220,borderRadius:12,overflow:"hidden",background:"#0B1120"}}
      />
      <style>{`
...
function UploadPage({user, onRosterSaved, setPage}) {
  ...
  if(parsedRoster) {
    ...
    const MONTHS_FULL = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    const allRows = [];
    (parsedRoster.calendar||[]).forEach((d,di)=>{
      (d.flights||[]).forEach((f,fi)=>{
        const dateStr = `${parsedRoster.year}-${String(mNum+1).padStart(2,"0")}-${String(d.day).padStart(2,"0")}`;
        allRows.push({f,d,di,fi,dateStr});
      });
    });
    allRows.sort((a,b)=>a.dateStr.localeCompare(b.dateStr)||(a.f.depTime||"").localeCompare(b.f.depTime||""));
    const dutyDays = (parsedRoster.calendar||[]).filter(d=>(d.flights||[]).length>0).length;
    const totalBlock = allRows.reduce((acc,{f})=>acc+(schedMins(f)||0),0);

    async function deleteRow(di, fi){
      if(!window.confirm("Remove this flight from the roster?")) return;
      const nc=[...(parsedRoster.calendar||[])];
      nc[di]={...nc[di],flights:(nc[di].flights||[]).filter((_,i)=>i!==fi)};
      try{
        await db_saveRoster(user.id,{...parsedRoster,calendar:nc});
        const updated={...parsedRoster,calendar:nc};
        onRosterSaved(updated);
        setParsedRoster(updated);
      }catch(e){alert(e.message);}
    }
    ...
    return(
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        {/* Success header */}
        <div style={{background:`linear-gradient(135deg,#10B981,#059669)`,padding:"24px 22px 20px",color:"#fff",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            <div style={{width:44,height:44,borderRadius:"50%",background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
            <div>
              ...
              <h2 style={{fontSize:20,fontWeight:900,margin:"2px 0 0",letterSpacing:"-.5px"}}>{MONTHS_FULL[mNum]} {parsedRoster.year}</h2>
            </div>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[[`${dutyDays}`,"Duty Days"],[`${allRows.length}`,"Flights"],[fmtMins(totalBlock),"Sched Block"]].map(([val,lbl])=>(
              <div key={lbl} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",padding:"6px 12px",borderRadius:100,border:"1px solid rgba(255,255,255,0.2)"}}>
                <span style={{fontSize:14,fontWeight:800,color:"#fff"}}>{val}</span>
                <span style={{fontSize:10,color:"rgba(255,255,255,0.75)",marginLeft:5}}>{lbl}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Flight list -- scrollable, editable */}
        <div style={{flex:1,overflowY:"auto",padding:"14px 16px 16px"}}>
          <div style={{fontSize:12,fontWeight:700,color:S.muted,textTransform:"uppercase",letterSpacing:".5px",marginBottom:4}}>
            Review extracted flights
          </div>
          <div style={{fontSize:12,color:S.muted,marginBottom:12,lineHeight:1.5}}>
            This data is saved as a draft. Remove anything incorrect now, or make further
            corrections later in <strong style={{color:S.ink}}>Active Logs</strong> -- nothing
            is permanent until you <strong style={{color:S.ink}}>Verify & Sign</strong>.
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {allRows.map(({f,d,di,fi,dateStr},idx)=>(
              <div key={`${di}-${fi}`} style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:14,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,position:"relative"}}>
                <div style={{width:4,height:36,borderRadius:2,background:S.blue,flexShrink:0}}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:800,color:S.ink}}>{f.flightNum||"--"}</div>
                  <div style={{fontSize:11,color:S.muted,marginTop:2}}>{dateStr} · {f.dep}→{f.arr} · {f.depTime||"--"}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:12,fontWeight:700,color:S.ink}}>{schedMins(f)?fmtMins(schedMins(f)):"--"}</div>
                  {f.acType&&<div style={{fontSize:10,color:S.muted,marginTop:2}}>{f.acType}</div>}
                </div>
                <button
                  onClick={()=>deleteRow(di,fi)}
                  title="Remove this flight"
                  style={{width:26,height:26,borderRadius:"50%",background:"none",border:"none",color:S.muted,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,opacity:0.5,lineHeight:1}}
                >×</button>
              </div>
            ))}
            {allRows.length===0&&(
              <div style={{textAlign:"center",padding:"32px",color:S.muted,fontSize:13}}>No flights extracted from this roster.</div>
            )}
          </div>
        </div>

        {/* Action buttons -- inside the normal page flow, NOT position:fixed,
            so it sits naturally above the app-level bottom nav instead of
            being hidden underneath it. */}
        <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"14px 16px",display:"flex",gap:10}}>
          <button
            onClick={()=>{setParsedRoster(null);setStatus(null);setMsg("");}}
            style={{flex:1,padding:"13px",borderRadius:14,background:"none",border:`1.5px solid ${S.border}`,color:S.muted,fontSize:14,fontWeight:700,cursor:"pointer"}}
          >
            Upload Another
          </button>
          <button
            onClick={()=>setPage&&setPage("active-logs")}
            style={{flex:2,padding:"13px",borderRadius:14,background:`linear-gradient(135deg,${S.blue},#7C3AED)`,border:"none",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 16px ${S.blue}30`,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}
          >
            Review & Sign in Active Logs
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M14 6l6 6-6 6" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </button>
        </div>
      </div>
    );
...
export default function App() {
  ...
  function navigate(newPage){
    ...
    if(newPage===page){
      // Still re-apply the tab even if already on the logbook page (e.g.
      // jumping from Daily View straight to Active Logs via a Quick Action)
      return;
    }
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base,position:"fixed",top:0,left:0,right:0,bottom:0}} className="app-shell">
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row",minHeight:0}}>
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
              ...
              {page==="logbook"&&<LogbookHubPage user={user} rosters={rosters} tails={tails} onTailSaved={handleTailSaved} onDeleteRoster={handleDeleteRoster} onRosterUpdated={handleRosterCalendarUpdated} pendingFlight={pendingFlight} onPendingFlightConsumed={()=>setPendingFlight(null)} setPage={navigate} initialTab={logbookInitialTab} onOpenFlight={(flight)=>{try{sessionStorage.setItem("fl_open_flight",JSON.stringify({rosterId:flight.roster?.id,di:flight.di,fi:flight.fi,flightData:flight}));}catch{}setPendingFlight(flight);}}/>}
              {page==="upload"&&<UploadPage user={user} onRosterSaved={handleRosterSaved} setPage={navigate}/>}
