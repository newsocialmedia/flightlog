...
async function db_saveTail(userId, rosterId, flightKey, tail, actualDep="", actualArr="", actualBlockMins=null, lock=false) {
  if(isConfigured()) {
    ...
    const {error} = await sb.from("tail_logs").upsert(payload);
...
function UploadPage({user, onRosterSaved}) {
  ...
  const [expandedReviewDays,setExpandedReviewDays]=useState({}); // di -> bool, collapsed by default
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated}) {
  ...
  async function autoLookup(di,fi,f,day) {
    ...
    // Guard: don't allow manual lookup before the flight has plausibly landed.
    // arrTime is local time at the destination. We don't know the exact timezone
    // here, so we apply the same conservative 9hr UTC offset buffer the cron uses:
    // treat arrTime as if it were UTC and require 9hr+5min to have elapsed since
    // then. This guarantees we never query FlightAware before the flight has
    // actually landed even in UTC-8 (the westernmost US timezone), and matches
    // the Phase 1 capture window the cron uses.
    const [ah, am] = (f.arrTime || "00:00").split(":").map(Number);
    const dateStr = `${roster.year}-${String(roster.monthNum+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
    const naiveArrMs = new Date(`${dateStr}T${String(ah).padStart(2,"0")}:${String(am).padStart(2,"0")}:00Z`).getTime();
    const EARLIEST_LOOKUP_MS = naiveArrMs + (9 * 60 * 60 * 1000) + (5 * 60 * 1000);
    if (Date.now() < EARLIEST_LOOKUP_MS) {
      alert(`Flight hasn't landed yet — auto-lookup is available roughly 9 hours after the scheduled arrival time (${f.arrTime} local). Try again later or enter the tail number manually.`);
      return;
    }

    setLkStatus(p=>({...p,[tk]:"loading"}));
    try {
      const res=await lookupFlight(f.flightNum,dateStr,f.depTime);
      if(res.tailNumber) {
        const existing=tails[tk]||{};
        ...
        // Save to DB first — only update UI state after the write succeeds,
        // so a failed save shows an error rather than ghost data that vanishes on refresh.
