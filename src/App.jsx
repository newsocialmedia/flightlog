...
function jeppesenExport(rosters, tails) {
  ...
  //   ROUTE OF FLIGHT: Date, Aircraft Type, Aircraft Ident, From, To,
  //     Nr. Inst. App., Remarks and Endorsements, Nr T/O, Nr Ldg
  //   AIRCRAFT CATEGORY: SEL, MEL
  //   CONDITION OF FLIGHT: Night, Actual Instr, Sim Instr, Cross Country
  //   TYPE OF PILOTING TIME: Instructor, Dual Received, SIC/Copilot,
  //     Flight Sim, PIC, Total Duration of Flight
  const rows=[["Date","Aircraft Type","Aircraft Ident","From","To","Nr. Inst. App.","Remarks and Endorsements","Nr T/O","Nr Ldg","SEL","MEL","Night","Actual Instr","Sim Instr","Cross Country","Instructor","Dual Received","SIC/Copilot","Flight Sim","PIC","Total Duration of Flight"]];
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    const k=`${r.id}-${di}-${fi}`;
    const t=tails[k]||{};
    if(t.cancelled) return;
    ...
    // SEL (single-engine land) has no dedicated field anywhere in this
    // app's data model -- only Multi-Engine time is tracked explicitly,
    // and it's only ever undefined (never set) or a real duration (set),
    // with nothing in the app writing an explicit zero to distinguish
    // "confirmed single-engine" from "multi-engine status never recorded
    // either way." Genuinely always blank until a real SEL field exists.
    const selStr="";
    ...
    // Nr T/O is INFERRED, not a genuinely tracked field -- this app only
    // records landing counts (day/night), not takeoffs separately. Assumed
    // equal to 1 whenever landing data exists, following the standard
    // logbook convention for a normal single-leg flight (one takeoff, one
    // set of landings) -- an assumption, not a value this app independently
    // verifies the way it does for landings themselves.
    ...
    // Instructor-given and Flight-Sim time have no corresponding fields
    // anywhere in this app's data model -- no UI captures CFI-given time
    // or standalone flight-simulator sessions, so these stay genuinely
...
function asaExport(rosters, tails) {
  ...
  //
  // Four grouped header categories span these columns in the source doc:
  //   POINTS OF DEPARTURE & ARRIVAL: Date, Aircraft Make/Model,
  //     Aircraft Identification Number, From, To
  //   AIRCRAFT CATEGORY AND CLASSIFICATION: SEL, MEL
  //   TYPE OF PILOTING TIME: Ground Trainer, Dual Received,
  //     Pilot in Command, Day, Night
  //   CONDITIONS OF FLIGHT: Cross Country, Actual Instr, Sim Instr,
  //     No. Instr. Appr., No. Ldg. Day/Night, Total Duration of Flight,
  //     Remarks and Endorsements
  const rows=[["Date","Aircraft Make/Model","Aircraft Ident Number","From","To","SEL","MEL","Ground Trainer","Dual Received","Pilot in Command","Day","Night","Cross Country","Actual Instr","Sim Instr","No. Instr. Appr.","No. Ldg. Day/Night","Total Duration of Flight","Remarks and Endorsements"]];
  (rosters||[]).forEach(r=>(r.calendar||[]).forEach((d,di)=>d.flights.forEach((f,fi)=>{
    const k=`${r.id}-${di}-${fi}`;
    const t=tails[k]||{};
    if(t.cancelled) return;
    ...
    const xcStr=isDH?"":(f.loggedXcMins!=null?fmtMins(f.loggedXcMins):"");
    const multiEngStr=isDH?"":(f.loggedMultiEngMins!=null?fmtMins(f.loggedMultiEngMins):"");
    // SEL has no dedicated field anywhere in this app's data model, same
    // honest gap as the Jeppesen export -- only Multi-Engine time is
    // tracked explicitly, with nothing distinguishing "confirmed single-
    // engine" from "multi-engine status never recorded either way."
    const selStr="";
    const picStr=isDH?"":(f.loggedPicMins!=null&&f.loggedPicMins>0?block:"");
    // Dual Received (loggedDualMins) and Ground Trainer have no path in
    // this app's own Add Flight / Flight Details UI -- neither field is
    // ever written by manual entry. loggedDualMins DOES exist as a real,
    // named field populated when a pilot imports a prior logbook CSV that
    // had it (see buildImportRosters), so it's read here if present;
    // Ground Trainer has no corresponding field anywhere, imported or
    // manual, so it stays genuinely blank.
    ...
    const appCount=f.loggedApproaches>0?String(f.loggedApproaches):"";
    // "Day" (piloting time logged as day, distinct from Night) has no
    // dedicated field either -- this app tracks Night time explicitly but
    // infers "day" only implicitly as block-time-minus-night for display
    // purposes elsewhere, never as its own stored, exportable value. Since
    // that would be a computed approximation rather than a real logged
    // field, and this export is meant to reflect only what's genuinely on
    // record, Day is left blank rather than backed into a number that
                  ...
                  <div style={{position:"sticky",top:0,zIndex:15,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"10px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                    ...
                    {dvBulkPanel==="multiEng"&&(
                      <div style={{width:"100%",display:"flex",gap:8,alignItems:"center",paddingTop:8,borderTop:`1px solid ${C.border}`}}>
                        <input type="text" placeholder="e.g. 1:30" value={dvBulkInput} onChange={e=>setDvBulkInput(e.target.value.replace(/[^0-9:]/g,""))} style={{width:100,padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,fontSize:13,fontFamily:"monospace",background:C.panel,color:C.ink,outline:"none"}}/>
                        <button disabled={!dvBulkInput||dvBulkApplying} onClick={()=>applyDvBulkEdit({loggedMultiEngMins:parseHM(dvBulkInput)})} style={{padding:"8px 16px",borderRadius:8,background:C.teal,border:"none",color:"#fff",fontSize:12,fontWeight:700,cursor:(!dvBulkInput||dvBulkApplying)?"not-allowed":"pointer",opacity:(!dvBulkInput||dvBulkApplying)?0.6:1}}>{dvBulkApplying?"Applying...":`Apply to ${dvSelected.size}`}</button>
                      </div>
                    )}
                    {dvBulkPanel==="picSic"&&(
                      <div style={{width:"100%",display:"flex",gap:8,alignItems:"center",paddingTop:8,borderTop:`1px solid ${C.border}`}}>
