  ...
  function parseNetlineFromItems(items) {
    ...
    for(const pg of pages){
      ...
      if(stripArr.length >= 10){
        ...
        for(const h of strip){
          ...
          prev = h.day;
          if(inCarry) carryDayNums.add(h.day);
    ...
    for(const pg of pages){
      ...
      margins.forEach((mg, bi)=>{
        ...
        blk.forEach((l, k)=>{
          ...
          blkPrev = l.day;
          // A label is carryover if: the day sequence reset within THIS block,
          // OR the strip says this day number is post-reset AND either the
          // main-month occurrence was already bucketed (duplicate small day)
          // or the main month has no marker for it (main day is off, so any
          // body occurrence must be the carry one).
          const isCarry = blkReset
    ...
    for(const oc of overflowCarry){
      ...
      if(ex){
        ...
        for(const f of oc.flights) if(!seen.has(f.flightNum+f.dep+f.depTime)) ex.flights.push(f);
      } else {
