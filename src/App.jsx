...
async function db_saveTimeRule(userId, rule) {
  if(!isConfigured()) return;
  const payload = {...rule, user_id: userId};
  if(payload.id) { await sb.from("time_rules").update(payload).eq("id", payload.id); }
  else { await sb.from("time_rules").insert(payload); }
}
  ...
  for(const roster of rosters) {
    (roster.calendar||[]).forEach((day, di) => {
      (day.flights||[]).forEach((f, fi) => {
        ...
        const dateStr = `${roster.year}-${String(roster.month_num+1||roster.monthNum+1).padStart(2,"0")}-${String(day.day).padStart(2,"0")}`;
        const flightDate = new Date(dateStr);
        const actualMins = tail?.actualBlockMins;
        const schedMin = schedMins(f);
        const mins = actualMins ?? schedMin ?? 0;
        if(!mins) return;
        ...
        // Time period buckets (only actual flown)
        if(actualMins) {
          if(flightDate >= d30) results.last30.mins += actualMins;
          if(flightDate >= d6mo) results.last6mo.mins += actualMins;
          if(flightDate >= d12mo) results.last12mo.mins += actualMins;
        }
...
function AnalyticsPage({user, rosters, tails}) {
  ...
  return (
    <div style={{maxWidth:700}}>
      ...
      {tab==="far117"&&(
        <div className="card">
          <div style={{fontSize:14,fontWeight:600,color:C.ink,marginBottom:4}}>FAR Part 117 — Table B Simplified Limits</div>
          <div style={{fontSize:12,color:C.muted,marginBottom:16}}>Applies to Part 121 air carrier operations (scheduled). Consult your airline's ops specs for augmented crew rules.</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {far117.map(row=>(
              <div key={row.limit} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",background:C.panel,borderRadius:9}}>
                <span style={{fontSize:13,color:C.silver}}>{row.limit}</span>
                <span style={{fontFamily:FM,fontSize:13,color:C.teal,fontWeight:600}}>{row.value} <span style={{color:C.muted,fontWeight:400,fontSize:11}}>{row.unit}</span></span>
              </div>
            ))}
          </div>
          <div style={{marginTop:16,padding:"12px 14px",background:C.gold+"15",borderRadius:9,border:`1px solid ${C.gold}33`}}>
            <div style={{fontSize:12,color:C.gold,fontWeight:600,marginBottom:4}}>Note on FDP Start Times</div>
            <div style={{fontSize:11,color:C.silver,lineHeight:1.6}}>
              The maximum Flight Duty Period varies by acclimation time (local start time):
              <br/>• 0000–0459: 9 hours · 0500–0659: 10 hours · 0700–1259: 11–13 hours · 1300–2159: 12 hours · 2200–2359: 10 hours
              <br/>Add 2 hours for 3-pilot crew, 3 hours for 4-pilot crew (augmented).
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  function navigate(newPage) {
    if(newPage === page) return; // no-op if already on this page
  ...
  useEffect(() => {
    ...
    function onPopState(e) {
      ...
      if(prev) {
        ...
        // No more history entries — we're at the root, stay on dashboard
        // rather than exiting the app
        window.history.pushState({ page: "dashboard" }, "", "");
        setPage("dashboard");
      }
    }
    // Seed the initial history entry so the very first back press
