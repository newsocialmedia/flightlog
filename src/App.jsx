...
async function db_signUp(email, password, name, plan, airlineIata="", airlineName="") {
  if(isConfigured()) {
    ...
    const {data,error} = await sb.auth.signUp({email,password,options:{data:{name,airline_iata:airlineIata,airline_name:airlineName}}});
  ...
  const user = {id:"u"+Date.now(),email,name,plan:"starter",role:"pilot",joined:new Date().toISOString().slice(0,10),active:true,airlineIata,airlineName};
  ...
  async function submit() {
    ...
    try {
      if(mode==="login") {
        ...
        const {data,error} = await sb.auth.signUp({email,password,options:{data:{name,airline_iata:airlineIata}}});
          ...
          <div style={{display:"flex",flexDirection:"column",flex:1,justifyContent:"center"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:16}}>
              ...
              {mode==="signup"&&(
                <input
                  style={{padding:"14px 16px",borderRadius:14,border:"1px solid #E2E8F0",fontSize:15,background:"#fff",color:"#0F172A",outline:"none",WebkitAppearance:"none"}}
                  placeholder="Airline IATA (e.g. AA, DL)" value={airlineIata}
      ...
      <div className="auth-card">
        ...
        {mode==="signup" && (
          <div className="form-group">
            ...
            <input className="form-input" placeholder="e.g. UA, AA, DL" value={airlineIata} onChange={e=>setAirlineIata(e.target.value.toUpperCase().slice(0,3))} maxLength={3} style={{width:120}}/>
          </div>
        )}
        <div className="form-group">
        ...
        {(mode==="login"||mode==="signup")&&(
          <>
            ...
            <button
              ...
              style={{width:"100%",padding:"12px",borderRadius:14,border:"1px solid #E2E8F0",background:"#fff",color:"#0F172A",fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:8}}
            >
            ...
            <button
              type="button"
              onClick={()=>{
                window.location.href = `${SUPA_URL}/auth/v1/authorize?provider=apple&redirect_to=${encodeURIComponent(window.location.origin)}`;
              }}
              style={{width:"100%",padding:"12px",borderRadius:14,border:"none",background:"#000",color:"#fff",fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}
            >
...
async function resolveAirportCoords(code) {
  ...
  if(_airportGeocodeCache[code] !== undefined) return _airportGeocodeCache[code]; // may be null (a prior failed lookup)
  try {
    const res = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(code+" airport")}`,
      { headers: { "Accept-Language": "en" } }
    );
    const data = await res.json();
    const hit = data?.[0];
    const coords = hit ? [parseFloat(hit.lat), parseFloat(hit.lon)] : null;
...
function FlightRouteMap({dep, arr, dist, flightDateStr, flightDepTime, S, compact}) {
  ...
  useEffect(() => {
    ...
    if(AIRPORT_COORDS[dep] && AIRPORT_COORDS[arr]) { setResolving(false); }
    else { resolve(); }
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{padding:"14px 16px",background:S.surface,borderBottom:`1px solid ${S.border}`,display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10,backdropFilter:"blur(12px)"}}>
        ...
        {isSynced&&(
          <span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"4px 10px",borderRadius:100,background:C.greenBg,color:GREEN,fontSize:11,fontWeight:700,flexShrink:0}} title={tail?.updatedAt?`Synced ${fmtSyncTime(tail.updatedAt)}`:undefined}>
            ...
            {tail?.updatedAt?`Synced ${fmtSyncTime(tail.updatedAt)}`:"Auto-Synced"}
          </span>
        )}
        {onDeleteFlight&&(
          <button onClick={()=>setConfirmingDelete(true)} title="Delete flight" style={{width:36,height:36,borderRadius:"50%",background:S.panel,border:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
      ...
      <div style={{padding:"16px 16px 32px",maxWidth:600,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* Digital signature card -- collapsed by default; header alone
            shows enough status (fully signed / awaiting / not yet signed)
            that a pilot doesn't need to expand it just to check state. */}
