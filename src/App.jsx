...
async function _loadOurAirportsTable() {
  if(_ourAirportsTable) return _ourAirportsTable;
  if(_ourAirportsLoadPromise) return _ourAirportsLoadPromise;
  _ourAirportsLoadPromise = (async () => {
    try {
      const persisted = localStorage.getItem("fl_ourairports_table");
      if(persisted) { _ourAirportsTable = JSON.parse(persisted); return _ourAirportsTable; }
    } catch {}
    try {
      const res = await fetch("https://davidmegginson.github.io/ourairports-data/airports.csv");
      const text = await res.text();
      const table = {};
      // Minimal CSV parse: only the columns actually needed (iata_code,
      // latitude_deg, longitude_deg). OurAirports' iata_code column is
      // reliably unquoted/simple, so a plain split is safe here without
      // pulling in a full CSV parser for one field.
      ...
      try { localStorage.setItem("fl_ourairports_table", JSON.stringify(table)); } catch {}
      return table;
    } catch {
...
const TZ_REGION_BOUNDS = {
  "America/":  [5, 72, -168, -52],   // North America incl. Mexico, Canada, Alaska
...
async function resolveAirportCoords(code) {
  ...
  if(_airportGeocodeCache[code] !== undefined) {
    ...
    // A cached value may predate this plausibility check (e.g. PIA was
    // geocoded to Australia before this fix existed, then cached) --
    // re-validate before trusting it. If it now fails, fall through to a
    // fresh lookup rather than keep serving a known-bad cached result.
        ...
        <div style={{background:`linear-gradient(135deg,${BLUE},#1E3A8A)`,borderRadius:20,padding:"20px 22px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",zIndex:1}}>
            <div style={{textAlign:"center"}}>
              ...
              <div style={{fontSize:16,fontWeight:800,color:"#fff",marginTop:6}}>{formatTime(hasActualLive?tail?.actualDep:f?.depTime)}</div>
            ...
            <div style={{textAlign:"center"}}>
              ...
              <div style={{fontSize:16,fontWeight:800,color:"#fff",marginTop:6}}>{formatTime(hasActualLive?tail?.actualArr:f?.arrTime)}</div>
