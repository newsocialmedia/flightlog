...
function getNotifPrefs() {
  try { return { sync:true, edits:true, signReminders:true, ...JSON.parse(localStorage.getItem("fl_notif_prefs")||"{}") }; }
  catch { return { sync:true, edits:true, signReminders:true }; }
}
      ...
      <section id="features" style={{maxWidth:1200,margin:"0 auto",padding:"72px 20px",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14}}>
          {[
            ...
            {icon:"📋",bg:"#EFF6FF",bc:"#BFDBFE",title:"Jeppesen & ASA Export",desc:"Download in formats that match physical logbook columns exactly. Ready to import anywhere."},
            ...
            {icon:"🤖",bg:"#F0F9FF",bc:"#BAE6FD",title:"AI Flight Briefing",desc:"One-tap briefing for your next flight -- weather, NOTAMs, and route summary. Updated hourly."},
...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  const searchResults=useMemo(()=>{
    ...
    const PAGES=[["dashboard","Dashboard","home overview stats"],["upload","Upload Roster","import pdf csv roster"],["logbook","Logbook & Active Logs","flights daily roster view verify sign"],["analytics","Analytics","stats charts hours breakdown"],["map","Route Map","radar routes"],["settings","Settings","preferences appearance account theme"],["subscriptions","Subscription","billing plan invoices payment"],["membership","Membership","plan upgrade"],["referral","Referral","refer a pilot earn credit"],["support","Support","help contact"],["export","Export","jeppesen asa download backup"],["profile","Profile","pilot identity avatar"],["add-flight","Add Flight","manual entry new leg"]];
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {[["Last 30 days",analytics.last30.mins],["Last 6 mo",analytics.last6mo.mins],["Last 12 mo",analytics.last12mo.mins]].map(([l,m])=>(
              <div key={l} className="card" style={{textAlign:"center",padding:"14px 10px"}}>
                <div style={{fontFamily:"monospace",fontSize:18,fontWeight:700,color:C.green}}>{m?fmtMins(m):"--"}</div>
          ...
          {!isActive&&!isPastDue&&(<>
            ...
            <div style={{marginBottom:10,padding:20,borderRadius:14,background:`linear-gradient(135deg,${C.teal},${C.tealDim})`,position:"relative",overflow:"visible"}}>
              ...
              <div style={{marginBottom:14}}>
                {["Over 2 months free vs monthly","Locked-in rate -- never increases","Priority support response","Early access to new features"].map(f=>(
