...
.form-input{width:100%;background:${C.panel};border:1px solid ${C.border};color:${C.ink};padding:11px 14px;border-radius:10px;font-size:16px;outline:none;transition:border-color .15s}
...
.day-card{background:${C.surface};border:1px solid ${C.border};border-radius:14px;margin-bottom:8px;overflow:hidden;transition:border-color .15s;contain:layout}
...
.fr-input{background:${C.surface};border:1px solid ${C.border};color:${C.ink};padding:6px 10px;border-radius:8px;font-family:${FM};font-size:16px;width:120px;text-transform:uppercase;outline:none;transition:border-color .15s}
...
@media(max-width:768px){
  ...
  /* Drawer overlay */
  .drawer-overlay{display:block;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:1000;animation:fadeIn .15s ease}
  /* Drawer panel */
  .drawer-panel{position:fixed;top:0;left:0;bottom:0;width:76vw;max-width:300px;background:${C.surface};border-right:1px solid ${C.border};z-index:1001;display:flex;flex-direction:column;padding:0;animation:slideInLeft .18s ease;overflow-y:auto}
...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    // Check for existing roster for same month/year and replace it
    // rather than creating a duplicate. This handles re-uploading a
    // corrected roster without cluttering the list.
    const {data:existing} = await sb.from("rosters")
      ...
      .single();
    if(existing?.id) {
      // Update existing roster instead of inserting a new one
      const {error} = await sb.from("rosters")
        .update({period_label:roster.periodLabel, calendar:roster.calendar, updated_at:new Date().toISOString()})
        .eq("id", existing.id);
      if(error) throw new Error("Failed to update roster");
      return {...roster, id:existing.id};
    }
    const {data,error} = await sb.from("rosters").insert({user_id:userId,period_label:roster.periodLabel,year:roster.year,month_num:roster.monthNum,calendar:roster.calendar}).select().single();
  ...
  return (
    <div style={{maxWidth:560}}>
      ...
      <div style={{display:"flex",gap:0,marginBottom:20,borderBottom:`1px solid ${C.border}`}}>
        {[["info","Profile"],["subscription","Subscription"],["billing","Billing"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{
            padding:"8px 16px",border:"none",background:"none",cursor:"pointer",fontSize:13,fontWeight:600,
            color:tab===id?C.teal:C.muted,
            borderBottom:`2px solid ${tab===id?C.teal:"transparent"}`,
...
export default function App() {
  ...
  const pageTitle = {
    ...
    settings:"Settings", map:"Route Map", analytics:"Analytics", profile:"Profile",
