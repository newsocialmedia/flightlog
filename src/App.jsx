...
async function lookupFlight(flightNum, date, depTime, dep="", arr="") {
  const r = await fetch(`${SUPA_URL}/functions/v1/lookup-flight`, {
    ...
    body: JSON.stringify({ flightNum, date, depTime, dep, arr }),
  });
  const data = await r.json();
...
function FlightDetailPage({flight:f, tail, solar, dist, blockMins, day, roster, hasActual, dep, arr, isXC, onBack, onAutoLookup, lkStatus, lkError, onResetLimit, tmp, onTmpChange, onSaveTail, saving, onTailSaved, editingTimes, setEditingTimes, timeEdits, setTimeEdits, di, fi, userId}) {
  ...
  return(
    <div style={{maxWidth:600,paddingBottom:32}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {lkStatus==="ratelimit"&&(
          <div style={{marginTop:8,padding:"8px 10px",background:C.panel,borderRadius:6}}>
            <div style={{fontSize:11,color:C.muted,marginBottom:6}}>{`Daily limit reached (${AUTO_SYNC_DAILY_LIMIT}/day). Resets at midnight.`}</div>
            <button onClick={()=>{
              try{Object.keys(localStorage).filter(k=>k.startsWith("fl_autosync_")).forEach(k=>localStorage.removeItem(k));}catch{}
              onResetLimit?.();
            }} style={{fontSize:11,color:C.teal,background:"none",border:`1px solid ${C.teal}44`,borderRadius:5,padding:"3px 10px",cursor:"pointer"}}>
              Reset limit
            </button>
          </div>
        )}
