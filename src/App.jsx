  ...
  if(isConfigured()) {
    ...
    if(carryForwardDays.length > 0) {
      ...
      if(nextRoster?.id) {
        ...
        // Next month not uploaded yet — create a stub roster with just the carry-forward days
        // renumber carry-forward days so day 32 of Feb = day 2 of March, etc.
        const nextMonthDays = carryForwardDays.map(d => ({
          ...d,
          day: d.day - daysInMonth, // e.g. day 30 of Feb (daysInMonth=28) → day 2 of March
        }));
...
function FlightDetailPage({flight:f, tail, solar, dist, blockMins, day, roster, hasActual, dep, arr, isXC, onBack, onAutoLookup, lkStatus, tmp, onTmpChange, onSaveTail, saving, onTailSaved, editingTimes, setEditingTimes, timeEdits, setTimeEdits, di, fi, userId}) {
  ...
  useEffect(()=>{
    Promise.all([fetchWeather(f.dep), fetchWeather(f.arr)]).then(([dm,am])=>{
      setDepMetar(dm); setArrMetar(am);
    });
  },[]);
  ...
  async function loadBriefing() {
    ...
    try {
      const [dm,am,dt,at] = await Promise.all([fetchWeather(f.dep),fetchWeather(f.arr),fetchTaf(f.dep),fetchTaf(f.arr)]);
      const text = await getFlightBriefing({flightNum:f.flightNum,dep:f.dep,arr:f.arr,depTime:f.depTime,arrTime:f.arrTime,acType:f.acType,dateStr,dist},dm,am,dt,at);
      setBriefing(text);
  ...
  return (
    <div style={{maxWidth:600,paddingBottom:32}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {/* Block time — clearly distinguish scheduled vs actual */}
        <div style={{display:"flex",gap:8,marginBottom:14}}>
          <div style={{flex:1,background:C.panel,borderRadius:9,padding:"10px 12px",textAlign:"center"}}>
            <div style={{fontSize:10,color:C.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:".5px",marginBottom:2}}>
              {hasActual?"Actual Block":"Sched Block"}
            </div>
            <div style={{fontSize:16,fontWeight:700,color:hasActual?C.teal:C.silver,fontFamily:"monospace"}}>
              {blockMins?fmtMins(blockMins):"—"}
            </div>
            {!hasActual&&schedBlock&&<div style={{fontSize:9,color:C.muted,marginTop:1}}>timezone-adjusted est.</div>}
          </div>
          {hasActual&&schedBlock&&(
            <div style={{flex:1,background:C.panel,borderRadius:9,padding:"10px 12px",textAlign:"center"}}>
              <div style={{fontSize:10,color:C.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:".5px",marginBottom:2}}>Sched Block</div>
              <div style={{fontSize:16,fontWeight:700,color:C.muted,fontFamily:"monospace"}}>{fmtMins(schedBlock)}</div>
              <div style={{fontSize:9,color:C.muted,marginTop:1}}>from roster</div>
            </div>
          )}
        </div>
      ...
      {(depMetar||arrMetar)&&(
        <div className="card" style={{marginBottom:14}}>
          <div style={{fontSize:13,fontWeight:600,color:C.ink,marginBottom:10}}>Current Weather</div>
          {depMetar&&<div style={{marginBottom:8}}>
            <div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:".5px",marginBottom:3}}>{f.dep} METAR</div>
            <div style={{fontSize:11,fontFamily:"monospace",color:C.silver,background:C.panel,padding:"7px 10px",borderRadius:6}}>{depMetar.rawOb||"—"}</div>
          </div>}
          {arrMetar&&<div>
            <div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:".5px",marginBottom:3}}>{f.arr} METAR</div>
            <div style={{fontSize:11,fontFamily:"monospace",color:C.silver,background:C.panel,padding:"7px 10px",borderRadius:6}}>{arrMetar.rawOb||"—"}</div>
          </div>}
        </div>
      )}
      ...
      <div className="card">
        ...
        {!briefing&&!briefingLoading&&<div style={{fontSize:12,color:C.muted}}>Powered by Google Gemini + aviationweather.gov</div>}
      </div>
    </div>
  );
}
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed}) {
  ...
  const [expandedDays, setExpandedDays] = useState({}); // day index -> bool

  // Consume pendingFlight from Dashboard — auto-open the flight detail
  useEffect(()=>{
    if(pendingFlight && rosters.length>0) {
      if(pendingFlight.selectedFlight) {
        // Find the correct roster index
        const rIdx = rosters.findIndex(r=>
          r.id===pendingFlight.rosterId ||
          (r.year===pendingFlight.year && r.monthNum===pendingFlight.monthNum)
        );
  ...
  if(!roster) return (
    <div style={{textAlign:"center",padding:60,color:C.muted}}>
      No rosters uploaded. <br/><span style={{fontSize:13}}>Upload a PDF roster to see your logbook.</span>
    </div>
  );

  function tkey(di,fi) { return `${roster.id}-${di}-${fi}`; }

  // Compute totals across all flights in this roster
  ...
  function openFlight(di, fi, f, day) {
    ...
    setSelectedFlight({di,fi,f,tail:tails[tk]||{},tk,solar,dist,blockMins,day,hasActual,dep:hasActual?tail.actualDep:f.depTime,arr:hasActual?tail.actualArr:f.arrTime,isXC:(dist||0)>50});
  }

  // Full-page flight detail
  if(selectedFlight) {
    const {di,fi,f,tail,tk,solar,dist,blockMins,day,hasActual,dep,arr,isXC} = selectedFlight;
    // Always get fresh tail data
    const freshTail = tails[tk]||tail;
    const freshHasActual = !!(freshTail.actualDep&&freshTail.actualArr);
    return (
      <FlightDetailPage
        flight={f} tail={freshTail} solar={solar} dist={dist}
        blockMins={freshTail.actualBlockMins ?? schedMins(f) ?? 0}
        ...
        saving={saving[tk]}
        onTailSaved={(tailData)=>onTailSaved(tk,tailData)}
  ...
  // Days that have flights or duty codes
  const activeDays = (roster.calendar||[]).map((day,di)=>({day,di}))
    .filter(({day})=>day.flights?.length>0||day.dutyCode);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)"}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"10px 16px",overflowX:"auto"}}>
          {rosters.map((r,i)=>(
            <button key={r.id} onClick={()=>setSelRoster(i)} className={`month-tab${selRoster===i?" active":""}`}>
          ...
          {confirmDel?(
            <>
              <span style={{fontSize:11,color:C.red}}>Delete roster?</span>
            ...
            <button onClick={()=>setConfirmDel("y")} style={{fontSize:11,color:C.red,background:"none",border:`1px solid ${C.border}55`,borderRadius:6,padding:"3px 10px",cursor:"pointer",flexShrink:0}}>Delete</button>
          )}
        </div>
        <div style={{display:"flex",gap:20,padding:"6px 16px 10px",fontSize:12,color:C.muted}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:10}}>
        ...
        {activeDays.map(({day,di})=>{
          const isOpen = expandedDays[di]!==false; // default open
          const flights = day.flights||[];
          const flightCount = flights.filter((_,fi)=>!tails[tkey(di,fi)]?.cancelled).length;
          const flownCount  = flights.filter((_,fi)=>tails[tkey(di,fi)]?.actualBlockMins!=null).length;
          const dayBlockMins = flights.reduce((acc,f,fi)=>{
            const t=tails[tkey(di,fi)]||{};
            return acc + (t.actualBlockMins ?? schedMins(f) ?? 0);
          },0);

          return (
            <div key={di} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
              {/* Day header — sticky tab */}
              <div
                onClick={()=>setExpandedDays(p=>({...p,[di]:!isOpen}))}
                style={{
              ...
              >
                ...
                <div style={{
                  ...
                  background:flownCount>0&&flownCount===flightCount?C.teal:flightCount>0?C.teal+"22":C.panel,
                  display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                }}>
                  <div style={{fontSize:16,fontWeight:800,color:flownCount>0&&flownCount===flightCount?"#fff":C.ink,lineHeight:1}}>{String(day.day).padStart(2,"0")}</div>
                  <div style={{fontSize:9,fontWeight:600,color:flownCount>0&&flownCount===flightCount?"rgba(255,255,255,.8)":C.muted,textTransform:"uppercase"}}>{day.dow}</div>
                </div>
                {/* Route summary */}
                <div style={{flex:1,minWidth:0}}>
                  {flightCount>0?(
                    <>
                      <div style={{fontSize:14,fontWeight:700,color:C.ink}}>
                        <b>{flights[0]?.dep}</b>
                        {flights.map((f,fi)=>(
                          <span key={fi}> → <b>{f.arr}</b></span>
                        ))}
                      </div>
                      <div style={{fontSize:11,color:C.muted,marginTop:1}}>
                        {flights.map(f=>f.flightNum).join(" · ")} · {fmtMins(dayBlockMins)}
                      </div>
                    </>
                  ):(
                ...
                {/* Expand chevron */}
                <div style={{color:C.muted,fontSize:12,transform:isOpen?"rotate(180deg)":"rotate(0deg)",transition:"transform .2s"}}>▼</div>
              </div>
                ...
                <div style={{padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
                  {flights.map((f,fi)=>{
                    ...
                    return (
                      ...
                      >
                        {/* Flight number */}
                        <div style={{fontSize:12,fontWeight:700,color:C.teal,minWidth:56}}>{f.flightNum}</div>
                        {/* Route — bold dep/arr */}
                        <div style={{flex:1}}>
                          <div style={{fontSize:14,fontWeight:700,color:C.ink}}>
                            <b style={{fontSize:15}}>{f.dep}</b>
                            <span style={{color:C.muted,margin:"0 6px",fontWeight:400}}>→</span>
                            <b style={{fontSize:15}}>{f.arr}</b>
                            {isXC&&<span style={{marginLeft:6,fontSize:9,color:C.teal,fontWeight:700,background:C.teal+"18",padding:"1px 6px",borderRadius:4}}>XC</span>}
                          </div>
                          <div style={{fontSize:11,color:C.muted,marginTop:2}}>
                            {hasActual?(
                              <><span style={{color:C.teal}}>{tail.actualDep} → {tail.actualArr}</span> · <span style={{color:C.teal,fontWeight:600}}>{fmtMins(blockMins)}</span></>
                            ):(
                              <>{f.depTime} → {f.arrTime} · {blockMins?<span>{fmtMins(blockMins)} <span style={{opacity:.5,fontSize:10}}>sched</span></span>:"—"}</>
                            )}
                          </div>
                        </div>
                        {/* Right side info */}
                        <div style={{textAlign:"right",flexShrink:0}}>
                          {tail.tail?(
                            <div style={{fontSize:11,fontFamily:"monospace",color:C.silver}}>
                              {tail.tail}
                              {tail.finalSynced&&<span style={{color:C.green,marginLeft:3,fontSize:9}}>✓</span>}
                            </div>
                          ):(
                            <div style={{fontSize:10,color:C.muted}}>No tail</div>
                          )}
                          {dist&&<div style={{fontSize:10,color:C.muted,marginTop:2}}>{dist} NM</div>}
                          {solar.nightDep||solar.nightArr?<div style={{fontSize:9,color:C.teal,fontWeight:600}}>NIGHT</div>:null}
                        </div>
