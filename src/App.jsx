...
function schedMinsFromAirports(depTime, arrTime, depCode, arrCode) {
  ...
  // Convert both to UTC using DST-aware offsets
  const depUtcMins = depLocalMins - getAirportUtcOffsetMins(depCode);
  const arrUtcMins = arrLocalMins - getAirportUtcOffsetMins(arrCode);
  // Handle overnight wraparound
  const diff = ((arrUtcMins - depUtcMins) + 1440) % 1440;
  if(diff >= 15 && diff <= 600) return diff;
  // Fallback to naive same-timezone subtraction
...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  // Fetch gate info for next flight from FlightAware via lookup-flight
  useEffect(()=>{
    ...
    // Only fetch if flight is within 6 hours
    ...
    fetch(`${SUPA_URL}/functions/v1/lookup-flight`, {
      ...
      body: JSON.stringify({flightNum:f.flightNum, date:dateStr, depTime:f.depTime, includeGate:true}),
