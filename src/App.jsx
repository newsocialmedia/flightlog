    ...
    return(
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        ...
        <div style={{background:`linear-gradient(135deg,#10B981,#059669)`,padding:"20px 18px 16px",color:"#fff",flexShrink:0}}>
          ...
          {/* Carryover notice + tab toggle — ALWAYS visible */}
          <div style={{background:"rgba(255,255,255,0.15)",borderRadius:12,padding:"10px 14px",border:"1px solid rgba(255,255,255,0.25)"}}>
            <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:8}}>
              {carry.length>0
                ...
                : `No carryover into ${MONTHS_FULL[nextMNum]} ${nextYear} detected`}
            </div>
        ...
        <div style={{padding:"10px 16px 4px",flexShrink:0,background:S.surface,borderBottom:`1px solid ${S.border}`}}>
          <div style={{fontSize:11,color:S.muted,lineHeight:1.5}}>
            Parsed via {parsedRoster._layer==="netline_inline"?"deterministic parser":parsedRoster._layer==="csv_import"?"CSV import":"AI"}. Review and edit below. <strong style={{color:S.ink}}>Nothing is saved until you tap Confirm & Save</strong> — discarding or leaving this screen throws the upload away.
          </div>
        </div>
        ...
        {/* Bottom buttons — deferred-save model */}
        <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>
          <button
            onClick={confirmSave}
            disabled={saving}
            style={{width:"100%",padding:"14px",borderRadius:14,background:saving?"#6EE7B7":"linear-gradient(135deg,#10B981,#059669)",border:"none",color:"#fff",fontSize:14,fontWeight:800,cursor:saving?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}
          >
            {saving ? "Saving..." : (<>
              Confirm & Save Roster
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </>)}
          </button>
          <button
            onClick={()=>{
