...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  return(
    <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif",position:"relative"}}>
      ...
      <div style={{padding:"16px 20px 32px",maxWidth:1200,margin:"0 auto",width:"100%",boxSizing:"border-box",position:"relative",zIndex:1}}>
        ...
        {nextFlight&&(()=>{
          ...
          return(
            <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14,marginBottom:20}}>
              {/* Flight card */}
              <div
                onClick={()=>onOpenFlight&&onOpenFlight({f,day:nextFlight.day,roster:nextFlight.roster,di:nextFlight.di,fi:nextFlight.fi,tk:nextFlight.tk,tail:nextFlight.tail,dateStr:nextFlight.dateStr,dist:calcDist(f.dep,f.arr),solar:computeSolarTimes(f.dep,f.arr,f.depTime,f.arrTime,nextFlight.dateStr),blockMins:tail.actualBlockMins??schedMins(f)??0,hasActual:!!(tail.actualDep||tail.actualArr||tail.tail),dep:f.dep,arr:f.arr,isXC:(calcDist(f.dep,f.arr)||0)>50,userId})}
                ...
                <div style={{position:"relative",zIndex:1}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{textAlign:"center",minWidth:64}}>
                      <div style={{fontSize:"clamp(28px,5vw,38px)",fontWeight:900,color:S.ink,lineHeight:1,letterSpacing:"-1px"}}>{f.dep}</div>
                    ...
                    <div style={{textAlign:"center",minWidth:64}}>
                      <div style={{fontSize:"clamp(28px,5vw,38px)",fontWeight:900,color:S.ink,lineHeight:1,letterSpacing:"-1px"}}>{f.arr}</div>
                  ...
                  <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${S.border}`,display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:8,alignItems:"center"}}>
                    {/* Dep gate */}
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      return (a.f.depTime||"00:00").localeCompare(b.f.depTime||"00:00");
                  ...
                  return rows.slice(0,5).map(({f,t,dateStr,tk},idx)=>(
                    <tr key={tk} style={{borderBottom:`1px solid ${S.panel}`,cursor:"pointer"}} onClick={()=>setPage("logbook")}>
        ...
        {todayFlights.length>0&&(
          <div style={{background:S.surface,borderRadius:20,border:`1px solid ${S.border}`,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",marginBottom:20}}>
            ...
            {todayFlights.map(({f,tk,tail},i)=>(
              <div key={tk} style={{padding:"14px 20px",borderBottom:i<todayFlights.length-1?`1px solid ${S.panel}`:"none",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>setPage("logbook")}>
        ...
        {/* QUICK ACTIONS */}
        <h3 style={{fontSize:15,fontWeight:700,color:S.ink,marginBottom:12}}>Quick Actions</h3>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            ...
            {label:"Logbook",page:"logbook",primary:false,svg:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="4" y="2" width="13" height="18" rx="2" stroke={S.blue} strokeWidth="2"/><path d="M8 7h6M8 11h4" stroke={S.blue} strokeWidth="2" strokeLinecap="round"/></svg>},
            ...
            <button key={label} onClick={()=>setPage(page)} style={{padding:"14px 12px",borderRadius:14,textAlign:"left",background:primary?`linear-gradient(135deg,${S.blue},${S.purple})`:S.surface,border:primary?"none":`1px solid ${S.border}`,color:primary?"#fff":S.ink,cursor:"pointer",display:"flex",alignItems:"center",gap:10,boxShadow:primary?`0 4px 14px ${S.blue}30`:"0 2px 4px rgba(0,0,0,0.04)",transition:"opacity .1s"}}>
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed}) {
  ...
  // Restore / clear selected flight from sessionStorage
  useEffect(()=>{
    try {
      const saved = sessionStorage.getItem("fl_open_flight");
      if(saved) { const {flightData} = JSON.parse(saved); setSelectedFlight(flightData); }
    } catch {}
    ...
    return(
      <div style={{flex:1,overflowY:"auto",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        ...
        <div style={{padding:"16px 20px",background:"rgba(248,250,252,0.9)",backdropFilter:"blur(12px)",borderBottom:`1px solid ${S.border}`,position:"sticky",top:0,zIndex:10,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {/* Export buttons */}
            <button onClick={()=>csvExport(rosters,tails)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:10,background:S.surface,border:`1px solid ${S.border}`,color:S.silver,fontSize:12,fontWeight:600,cursor:"pointer"}}>
          ...
          <div style={{background:S.surface,borderRadius:24,border:`1px solid ${S.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.06)",overflow:"hidden"}}>
            ...
            <div style={{padding:"14px 20px",borderBottom:`1px solid ${S.border}`,background:"rgba(248,250,252,0.5)",display:"flex",flexWrap:"wrap",gap:10,alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                >
                  <option value={-1}>All rosters</option>
                  {rosters.map((r,i)=><option key={r.id} value={i}>{r.periodLabel}</option>)}
                </select>
              </div>
            </div>
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.base}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,background:C.surface,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,flexShrink:0}}>
        <div style={{display:"flex",gap:8,overflowX:"auto",flexWrap:"nowrap",flex:1}}>
          {rosters.map((r,i)=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
              <button onClick={()=>setSelRoster(i)} style={{padding:"5px 14px",borderRadius:100,fontSize:12,fontWeight:600,border:"none",cursor:"pointer",background:selRoster===i?C.teal:C.panel,color:selRoster===i?"#fff":C.muted}}>
                {r.periodLabel}
              </button>
              {selRoster===i&&(
                <button
                  onClick={async()=>{
                    if(!window.confirm(`Delete roster "${r.periodLabel}"? This cannot be undone.`)) return;
                    try{
                      await sb.from("rosters").delete().eq("id",r.id);
                      onDeleteRoster(r.id);
                      setSelRoster(Math.max(0,i-1));
                    }catch(e){alert(e.message);}
                  }}
                  title="Delete this roster"
                  style={{width:22,height:22,borderRadius:"50%",background:"none",border:`1px solid ${C.border}`,color:C.muted,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,lineHeight:1}}
                >×</button>
              )}
            </div>
          ))}
...
function FlightDetailPage({
  ...
  async function fetchBriefing() {
    ...
    try {
      ...
      const wxRes = await fetch(`https://aviationweather.gov/api/data/metar?ids=${f?.dep?.length===3?"K"+f.dep:f.dep}&format=json&hours=2`).then(r=>r.json()).catch(()=>[]);
      const wxInfo = wxRes?.[0] ? `${f.dep} weather: ${wxRes[0].rawOb}` : "";
      const prompt = `Brief this flight for an airline pilot: ${f?.flightNum} ${f?.dep}→${f?.arr} on ${dateStr} departing ${f?.depTime}. Aircraft: ${f?.acType||"regional jet"}. ${wxInfo} Give a concise 3-paragraph professional briefing: departure, enroute, arrival. Include NOTAMs or cautions if notable.`;
      ...
      <div style={{padding:"16px 16px 32px",maxWidth:600,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:18,padding:"16px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:700,color:S.muted,textTransform:"uppercase",letterSpacing:".5px"}}>Actual Times</div>
            {!editingTimes&&(
          ...
          {editingTimes?(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={async()=>{
                  if(onTailSaved&&timeEdits){
                    const updates={...tail,...timeEdits};
                    await onSaveTail&&onSaveTail(updates);
                  }
                  setEditingTimes&&setEditingTimes(false);
                }} style={{flex:1,padding:"10px",borderRadius:10,background:BLUE,border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>
                  Save
                </button>
                <button onClick={()=>{setEditingTimes&&setEditingTimes(false);setTimeEdits&&setTimeEdits({});}} style={{flex:1,padding:"10px",borderRadius:10,background:"none",border:`1px solid ${S.border}`,color:S.muted,fontSize:13,cursor:"pointer"}}>
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
              {[["Out",tail?.actualDep||f?.depTime,"—"],["In",tail?.actualArr||f?.arrTime,"—"],["Block",blockMins2?fmtMins(blockMins2):null,"—"]].map(([label,val,fallback])=>(
                <div key={label} style={{textAlign:"center"}}>
                  <div style={{fontSize:18,fontWeight:800,color:val?S.ink:S.muted,fontFamily:label==="Block"?"inherit":"monospace"}}>{val||fallback}</div>
                  <div style={{fontSize:10,color:S.muted,marginTop:3,textTransform:"uppercase",letterSpacing:".5px"}}>{label}</div>
                </div>
              ))}
            </div>
          )}
...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  const now = new Date();
  const [rosterIdx, setRosterIdx] = useState(()=>{
    // Default to current month's roster
    const cur = rosters.findIndex(r=>{
      const mNum = r.monthNum??r.month_num??0;
      return r.year===now.getFullYear()&&mNum===now.getMonth();
    });
    return cur>=0?cur:rosters.length-1;
  });
  const [sel, setSel] = useState(null); // selected day number
  const [adding, setAdding] = useState(false);
  const [newFlight, setNewFlight] = useState({flightNum:"",dep:"",arr:"",depTime:"",arrTime:"",acType:""});
  const [saving, setSaving] = useState(false);

  const roster = rosters[rosterIdx];
  const mNum = roster?(roster.monthNum??roster.month_num??0):now.getMonth();
  const year = roster?roster.year:now.getFullYear();
  const firstDow = new Date(year,mNum,1).getDay();
  const daysInMonth = new Date(year,mNum+1,0).getDate();
  const DAYS=["S","M","T","W","T","F","S"];
  const MONTH_NAMES=["January","February","March","April","May","June","July","August","September","October","November","December"];

  function getDay(d){ return roster?.calendar?.find(c=>c.day===d)||{day:d,dow:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(year,mNum,d).getDay()],flights:[],isOff:false}; }
  function isFlight(d){ return (getDay(d).flights||[]).length>0; }
  function isToday(d){ return d===now.getDate()&&mNum===now.getMonth()&&year===now.getFullYear(); }
  function isSynced(d){
    const day=getDay(d);
    if(!(day.flights||[]).length) return false;
    const di=(roster?.calendar||[]).findIndex(c=>c.day===d);
    if(di<0) return false;
    return (day.flights||[]).some((_,fi)=>tails[`${roster.id}-${di}-${fi}`]?.tail);
  }

  const selDay = sel ? getDay(sel) : null;
  const selDi = sel && roster ? (roster.calendar||[]).findIndex(c=>c.day===sel) : -1;

  async function saveNewFlight(){
    const fn=newFlight.flightNum.trim().toUpperCase().replace(/\s+/g,"");
    const dep=newFlight.dep.trim().toUpperCase();
    const arr=newFlight.arr.trim().toUpperCase();
    if(!fn||!dep||!arr||!newFlight.depTime||!newFlight.arrTime) return alert("Fill in all fields");
    const hasPrefix=/^([A-Z]{2,3}|[A-Z][0-9]|[0-9][A-Z])\d+$/.test(fn);
    if(!hasPrefix) return alert("Include carrier prefix e.g. G7 4475");
    setSaving(true);
    try{
      const nc=[...(roster?.calendar||[])];
      const di=nc.findIndex(c=>c.day===sel);
      const nf={flightNum:fn,dep,arr,depTime:newFlight.depTime,arrTime:newFlight.arrTime,acType:newFlight.acType.trim().toUpperCase()||""};
      if(di>=0){nc[di]={...nc[di],flights:[...(nc[di].flights||[]),nf]};}
      else{nc.push({day:sel,dow:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(year,mNum,sel).getDay()],isOff:false,flights:[nf]});nc.sort((a,b)=>a.day-b.day);}
      await db_saveRoster(user.id,{...roster,calendar:nc});
      onRosterUpdated(roster.id,nc);
      setAdding(false);
      setNewFlight({flightNum:"",dep:"",arr:"",depTime:"",arrTime:"",acType:""});
    }catch(e){alert(e.message);}
    setSaving(false);
  }
  ...
  if(!roster) return(
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12,background:S.bg}}>
      <div style={{fontSize:40}}>📅</div>
      <div style={{fontSize:15,color:S.muted}}>No roster loaded yet</div>
      <div style={{fontSize:13,color:S.muted}}>Upload a roster first</div>
    </div>
  );
  ...
  const dayDetailPanel = (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {/* Day header card */}
      {selDay?(
        <div style={{background:S.surface,borderRadius:24,border:`1px solid ${S.border}`,boxShadow:"0 4px 16px rgba(0,0,0,0.06)",padding:"20px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative",overflow:"hidden",flexShrink:0}}>
          <div style={{position:"absolute",top:0,right:0,width:80,height:80,background:"#F5F3FF",borderRadius:"0 24px 0 100%"}}/>
          <div>
            <h3 style={{fontSize:18,fontWeight:800,color:S.ink,margin:0}}>{selDay.dow}, {MONTH_NAMES[mNum].slice(0,3)} {sel}</h3>
            <p style={{fontSize:13,color:S.muted,margin:"4px 0 0",fontWeight:500}}>
              {(selDay.flights||[]).length} leg{(selDay.flights||[]).length!==1?"s":""}
              {(()=>{
                const totalMins=(selDay.flights||[]).reduce((acc,f,fi)=>{
                  const di2=(roster.calendar||[]).findIndex(c=>c.day===sel);
                  const t=di2>=0?tails[`${roster.id}-${di2}-${fi}`]||{}:{};
                  return acc+(t.actualBlockMins!=null?t.actualBlockMins:schedMins(f)||0);
                },0);
                return totalMins>0?" · "+fmtMins(totalMins)+" block":"";
              })()}
            </p>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center",position:"relative",zIndex:1}}>
            <button onClick={()=>setAdding(p=>!p)} style={{padding:"7px 14px",borderRadius:10,background:adding?S.panel:S.blue,border:"none",color:adding?S.muted:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>
              {adding?"Cancel":"+ Add"}
            </button>
            <button onClick={()=>{setSel(null);setAdding(false);}} style={{width:32,height:32,borderRadius:"50%",background:S.panel,border:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:16,color:S.muted,fontWeight:700}}>
              ×
            </button>
          </div>
        </div>
      ):(
        <div style={{background:S.surface,borderRadius:24,border:`2px dashed ${S.border}`,padding:"28px 22px",textAlign:"center",flexShrink:0}}>
          <div style={{fontSize:28,marginBottom:8}}>📅</div>
          <div style={{fontSize:14,fontWeight:600,color:S.muted}}>Tap a flight day to see details</div>
        </div>
      )}

      {/* Add flight form */}
      {adding&&sel&&(
        <div style={{background:S.surface,borderRadius:20,border:`1px solid ${S.border}`,padding:"16px 18px",flexShrink:0}}>
          <div style={{fontSize:13,fontWeight:700,color:S.ink,marginBottom:12}}>Add flight to {MONTH_NAMES[mNum].slice(0,3)} {sel}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
            {[["flightNum","Flight # (e.g. G7 4475)"],["acType","Aircraft"],["dep","Dep"],["arr","Arr"]].map(([k,ph])=>(
              <div key={k}>
                <div style={{fontSize:10,color:S.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:".5px",marginBottom:3}}>{ph}</div>
                <input style={{width:"100%",padding:"8px 10px",borderRadius:8,border:`1px solid ${S.border}`,fontSize:13,background:S.surface,color:S.ink,outline:"none",boxSizing:"border-box"}} placeholder={ph} value={newFlight[k]} onChange={e=>setNewFlight(p=>({...p,[k]:e.target.value}))}/>
              </div>
            ))}
            <div>
              <div style={{fontSize:10,color:S.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:".5px",marginBottom:3}}>Dep time</div>
              <input type="time" style={{width:"100%",padding:"8px 10px",borderRadius:8,border:`1px solid ${S.border}`,fontSize:13,background:S.surface,color:S.ink,outline:"none",boxSizing:"border-box"}} value={newFlight.depTime} onChange={e=>setNewFlight(p=>({...p,depTime:e.target.value}))}/>
            </div>
            <div>
              <div style={{fontSize:10,color:S.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:".5px",marginBottom:3}}>Arr time</div>
              <input type="time" style={{width:"100%",padding:"8px 10px",borderRadius:8,border:`1px solid ${S.border}`,fontSize:13,background:S.surface,color:S.ink,outline:"none",boxSizing:"border-box"}} value={newFlight.arrTime} onChange={e=>setNewFlight(p=>({...p,arrTime:e.target.value}))}/>
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={saveNewFlight} disabled={saving} style={{flex:1,padding:"10px",borderRadius:10,background:S.blue,border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>
              {saving?"⟳ Saving...":"Save flight"}
            </button>
            <button onClick={()=>{setAdding(false);setNewFlight({flightNum:"",dep:"",arr:"",depTime:"",arrTime:"",acType:""}); }} style={{flex:1,padding:"10px",borderRadius:10,background:"none",border:`1px solid ${S.border}`,color:S.muted,fontSize:13,cursor:"pointer"}}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {/* Flight legs */}
      {selDay&&(selDay.flights||[]).map((f,fi)=>{
        const di2=(roster.calendar||[]).findIndex(c=>c.day===sel);
        const tk=`${roster.id}-${di2}-${fi}`;
        const t=tails[tk]||{};
        const synced=!!t.tail;
        const cancelled=!!t.cancelled;
        const dist=calcDist(f.dep,f.arr);
        const blockMins=t.actualBlockMins!=null?t.actualBlockMins:schedMins(f)||0;
        const mNum2=roster.monthNum??roster.month_num??0;
        const dateStr=`${roster.year}-${String(mNum2+1).padStart(2,"0")}-${String(sel).padStart(2,"0")}`;
        const isXC=(dist||0)>50;
        return(
          <div key={fi}
            onClick={()=>onOpenFlight&&onOpenFlight({f,day:selDay,roster,di:di2,fi,tk,tail:t,dateStr,dist,solar:computeSolarTimes(f.dep,f.arr,f.depTime,f.arrTime,dateStr),blockMins,hasActual:!!(t.actualDep||t.actualArr||t.tail),dep:f.dep,arr:f.arr,isXC,userId:user.id})}
            style={{background:S.surface,borderRadius:20,border:`1px solid ${cancelled?"#FCA5A5":synced?"#A7F3D0":S.border}`,boxShadow:"0 2px 8px rgba(0,0,0,0.04)",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",transition:"border-color .15s",}}
            onMouseEnter={e=>e.currentTarget.style.borderColor=cancelled?"#FCA5A5":"#C4B5FD"}
            onMouseLeave={e=>e.currentTarget.style.borderColor=cancelled?"#FCA5A5":synced?"#A7F3D0":S.border}
          >
            {/* Status bar */}
            <div style={{width:5,height:52,borderRadius:3,background:cancelled?"#EF4444":synced?S.green:S.border,flexShrink:0}}/>
            
            {/* Times */}
            <div style={{width:52,flexShrink:0}}>
              <div style={{fontSize:17,fontWeight:900,color:S.ink,lineHeight:1}}>{t.actualDep||f.depTime}</div>
              <div style={{fontSize:11,color:S.muted,marginTop:4,fontWeight:500}}>{t.actualArr||f.arrTime}</div>
            </div>

            {/* Flight info */}
            <div style={{flex:1,borderLeft:`1px solid ${S.border}`,paddingLeft:14}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                <span style={{fontSize:13,fontWeight:800,color:S.blue}}>{f.flightNum}</span>
                {f.acType&&<span style={{fontSize:10,fontWeight:700,color:S.silver,background:S.panel,padding:"2px 6px",borderRadius:4}}>{f.acType}</span>}
                {isXC&&!cancelled&&<span style={{fontSize:10,fontWeight:700,color:S.purple,background:"#F5F3FF",padding:"2px 6px",borderRadius:4}}>XC</span>}
                {cancelled&&<span style={{fontSize:10,fontWeight:700,color:"#EF4444",background:"#FEF2F2",padding:"2px 6px",borderRadius:4}}>CNCL</span>}
              </div>
              <div style={{fontSize:14,fontWeight:700,color:S.ink,display:"flex",alignItems:"center",gap:8}}>
                {f.dep}
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M12 5l7 7-7 7" stroke={S.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                {f.arr}
              </div>
              {t.tail&&(
                <div style={{display:"flex",alignItems:"center",gap:5,marginTop:4}}>
                  <span style={{fontSize:11,color:S.muted,fontFamily:"monospace",fontWeight:600}}>{t.tail}</span>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke={S.green} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
              )}
            </div>
            ...
            <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
              <div style={{fontSize:15,fontWeight:800,color:S.ink}}>{blockMins?fmtMins(blockMins):"—"}</div>
              {dist&&<div style={{fontSize:10,color:S.muted,fontWeight:600}}>{dist} NM</div>}
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{marginTop:4,opacity:.3}}><path d="M9 18l6-6-6-6" stroke={S.purple} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
          </div>
        );
      })}
      {selDay&&!(selDay.flights||[]).length&&!adding&&(
        <div style={{background:S.surface,borderRadius:20,border:`1px solid ${S.border}`,padding:"28px",textAlign:"center"}}>
          <div style={{fontSize:13,color:S.muted}}>Rest day — no flights scheduled</div>
        </div>
      )}
    </div>
  );
  ...
  return(
    <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      {/* Sticky header */}
      <div style={{padding:"14px 16px",background:"rgba(248,250,252,0.95)",backdropFilter:"blur(12px)",borderBottom:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,position:"sticky",top:0,zIndex:10}}>
        <h1 style={{fontSize:20,fontWeight:800,color:S.ink,margin:0,letterSpacing:"-.5px"}}>Roster View</h1>
        <div style={{width:36,height:36,borderRadius:"50%",background:S.surface,border:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M1 4v6h6M23 20v-6h-6" stroke={S.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M20.49 9A9 9 0 005.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 013.51 15" stroke={S.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </div>
      </div>

      {/* ── DESKTOP LAYOUT: split grid ── */}
      <div style={{flex:1,overflow:"hidden",display:"grid",gridTemplateColumns:"minmax(0,7fr) minmax(0,5fr)",gap:0}} className="cal-desktop-grid">
        
        {/* LEFT: Calendar */}
        <div style={{overflowY:"auto",padding:"20px 16px 20px 20px",display:"flex",flexDirection:"column",gap:16}}>
          {/* Month title + roster tabs */}
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                <h2 style={{fontSize:28,fontWeight:900,color:S.ink,lineHeight:1.1,margin:0,letterSpacing:"-1px"}}>{MONTH_NAMES[mNum]}</h2>
                <div style={{fontSize:16,fontWeight:700,color:S.muted,marginTop:2}}>{year}</div>
              </div>
              {/* Legend */}
              <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
                {[[S.blue,"Flight"],[S.green,"Synced"],[S.amber,"Standby"]].map(([color,label])=>(
                  <div key={label} style={{display:"flex",alignItems:"center",gap:6,fontSize:11,fontWeight:600,color:S.muted}}>
            ...
            <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
              {rosters.map((r,i)=>{
                const rm=r.monthNum??r.month_num??0;
                const label=`${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][rm]} ${r.year}`;
                const isActive=i===rosterIdx;
                return(
                  <button key={r.id} onClick={()=>{setRosterIdx(i);setSel(null);setAdding(false);}} style={{
                    whiteSpace:"nowrap",padding:"7px 18px",borderRadius:100,border:"none",
                    fontSize:13,fontWeight:700,cursor:"pointer",flexShrink:0,
                    background:isActive?S.blue:"#F1F5F9",
                    color:isActive?"#fff":S.muted,
                    boxShadow:isActive?`0 4px 12px ${S.blue}30`:"none",
                    transition:"all .15s",
                  }}>
                    {label}
                  </button>
          ...
          {/* Calendar */}
          <div style={{background:S.surface,borderRadius:24,border:`1px solid ${S.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.06)",overflow:"hidden",flex:1}}>
            {/* Day headers */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:S.panel,borderBottom:`1px solid ${S.border}`}}>
              {DAYS.map((d,i)=>(
                <div key={i} style={{textAlign:"center",fontSize:11,fontWeight:800,color:S.muted,textTransform:"uppercase",letterSpacing:".8px",padding:"10px 0"}}>
                  {d}
                </div>
              ))}
            </div>
            {/* Cells */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",borderTop:`1px solid ${S.border}`,borderLeft:`1px solid ${S.border}`}}>
              {cells.map((d,i)=>{
                if(!d) return <div key={i} style={{aspectRatio:"1",borderRight:`1px solid ${S.border}`,borderBottom:`1px solid ${S.border}`,background:S.bg}}/>;
                const hasFlight=isFlight(d);
                const today=isToday(d);
                const synced=isSynced(d);
                const selected=sel===d;
                const bgColor=selected?"#1D4ED8":synced?"#059669":hasFlight?"#2563EB":S.surface;
                const textColor=hasFlight||selected?"#fff":today?S.blue:S.muted;
                return(
                  <div key={i} onClick={()=>{setSel(selected?null:d);setAdding(false);}} style={{
                    aspectRatio:"1",borderRight:`1px solid ${S.border}`,borderBottom:`1px solid ${S.border}`,
                    background:bgColor,cursor:hasFlight?"pointer":"default",
                    display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",
                    padding:"6px 4px",transition:"all .1s",position:"relative",
                    boxShadow:selected?"inset 0 0 0 2px #fff, inset 0 0 0 3px #1D4ED8":"none",
                  }}
                  onMouseEnter={e=>{if(hasFlight&&!selected)e.currentTarget.style.filter="brightness(0.92)";}}
                  onMouseLeave={e=>{e.currentTarget.style.filter="none";}}
                  >
                    <span style={{fontSize:13,fontWeight:hasFlight?800:today?700:500,color:textColor,lineHeight:1,marginTop:2}}>{d}</span>
                    {hasFlight&&(
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" style={{marginTop:3,opacity:.9}}>
                        <path d="M5 17l6-10 3 5 3-4 4 9" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    )}
                    {today&&!hasFlight&&(
                      <div style={{width:4,height:4,borderRadius:"50%",background:S.blue,position:"absolute",bottom:4}}/>
                    )}
                    {synced&&selected&&(
                      <div style={{position:"absolute",bottom:3,right:3,width:6,height:6,borderRadius:"50%",background:"#34D399"}}/>
                    )}
                  </div>
                );
              })}
        ...
        {/* RIGHT: Day detail panel (desktop only, scrollable) */}
        <div style={{overflowY:"auto",padding:"20px 20px 20px 8px",display:"flex",flexDirection:"column"}} className="cal-right-panel">
          {dayDetailPanel}
        </div>
      </div>

      {/* ── MOBILE LAYOUT: stacked below calendar ── */}
      <div style={{display:"none"}} className="cal-mobile-stack">
      ...
      <style>{`
        @media(max-width:768px){
          .cal-desktop-grid{grid-template-columns:1fr!important;overflow-y:auto;}
          .cal-right-panel{display:none!important;}
          .cal-mobile-stack{display:block!important;}
        }
      `}</style>
    </div>
  );
...
function ActiveLogsPage({user, rosters, tails, onRosterUpdated, onTailSaved}) {
  ...
  // ── State ─────────────────────────────────────────────────────────────────
  const [verifyModal, setVerifyModal] = useState(null); // roster object
  const [editEdits, setEditEdits] = useState({}); // {tk: {tail, actualBlockMins, acType}}
  ...
  const classified = useMemo(()=>{
    ...
    for(const r of rosters){
      ...
      if(isSigned){ verified.push({r,mNum}); continue; }
        ...
        <div style={{background:S.surface,borderRadius:24,width:"100%",maxWidth:840,boxShadow:"0 32px 80px rgba(0,0,0,0.2)",overflow:"hidden",marginBottom:24}}>
                      ...
                      <td style={{padding:"6px 10px"}}>
                        <input
                          ...
                          onChange={e=>{
                            ...
                            logEdit(tk,"acType",f.acType,val);
          ...
          <div style={{padding:"20px 28px",borderTop:`1px solid ${S.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap",background:S.surface}}>
            <div style={{fontSize:12,color:S.muted,lineHeight:1.6,maxWidth:400}}>
              ...
              <strong style={{color:S.ink}}>14 CFR §61.51</strong>. This action is{" "}
              <strong style={{color:"#DC2626"}}>irreversible</strong>.
            </div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>
              {classified.active.map(({r,mNum,totalFlights,syncedCount,syncPct})=>(
                <div key={r.id} style={{background:S.surface,borderRadius:20,border:`1.5px solid ${GREEN_BORDER}`,padding:"20px 22px",boxShadow:"0 2px 12px rgba(5,150,105,0.08)"}}>
                  ...
                  <div style={{fontSize:11,color:S.muted,marginTop:6}}>{syncPct}% · Updates within 15 min of landing</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>
              {classified.needsVerify.map(({r,mNum,totalFlights,syncedCount,syncPct})=>(
                <div key={r.id} style={{background:S.surface,borderRadius:20,border:`1.5px solid ${AMBER_BORDER}`,padding:"20px 22px",boxShadow:"0 2px 12px rgba(245,158,11,0.08)"}}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:14,marginBottom:16}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
              {classified.verified.map(({r,mNum})=>{
                ...
                const totalFlights=(r.calendar||[]).reduce((a,d)=>a+(d.flights||[]).length,0);
                return(
                  <div key={r.id} style={{background:S.surface,borderRadius:20,border:`1.5px solid ${BLUE_BORDER}`,padding:"20px 22px",boxShadow:"0 2px 12px rgba(37,99,235,0.06)"}}>
...
const MORE_PAGES = ["analytics","active-logs","map","profile","settings","membership","support","export","referral"];
    ...
    },
    {
      ...
      icon:(active)=>(
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
          <path d="M12 4v12M12 4L8 8M12 4l4 4" stroke={active?BLUE:MUTED} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M4 17v1a3 3 0 003 3h10a3 3 0 003-3v-1" stroke={active?BLUE:MUTED} strokeWidth="2.2" strokeLinecap="round"/>
        </svg>
      ),
    },
    {
      ...
      icon:(active)=>(
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M8 4H6a2 2 0 00-2 2v14a2 2 0 002 2h12a2 2 0 002-2V8l-4-4H8z" stroke={active?BLUE:MUTED} strokeWidth="2" fill="none"/>
          <path d="M8 11h5M8 15h3" stroke={active?BLUE:MUTED} strokeWidth="2" strokeLinecap="round"/>
        </svg>
      ),
    },
    {
...
function MorePage({user, setPage, onLogout, rosters, tails}) {
  ...
  const pilotItems = [
    ...
    {id:"active-logs", label:"Active Logs",             bg:S.greenBg, color:S.green,
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M9 12h6M9 16h6M9 8h6M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" stroke="#059669" strokeWidth="2" strokeLinecap="round"/></svg>},
    ...
    {id:"membership",  label:"Manage Subscription",     bg:`${S.blue}18`, color:S.blue,
     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="2" y="5" width="20" height="14" rx="2" stroke={S.blue} strokeWidth="2"/><path d="M2 10h20" stroke="#1D4ED8" strokeWidth="2"/><path d="M6 15h4" stroke="#1D4ED8" strokeWidth="2" strokeLinecap="round"/></svg>},
