...
function ActiveLogsPage({user, rosters, tails, onRosterUpdated, onTailSaved, onDeleteRoster}) {
  ...
  async function applyAndSignRoster(roster, edits){
    ...
    if(auditEntries.length){
      ...
      try {
        await sb.from("audit_log").insert(auditEntries.map(e=>({
          user_id: user.id, roster_id: roster.id, flight_key: e.tk,
          field: e.field, old_value: e.oldVal, new_value: e.newVal,
          signed_at: e.ts,
        })));
      } catch(e) { console.warn("audit_log server write failed (non-fatal):", e?.message); }
    }
    return auditEntries;
  }
                        ...
                        <input
                          ...
                          onBlur={e=>{
                            ...
                            if(mins!=null && mins!==dispActBlockMins){
                              ...
                              setActiveEdits(p=>({...p,[tk]:{...p[tk],actualBlockMins:mins}}));
                            }
                          }}
...
function AdminAuditLogs() {
  ...
  useEffect(()=>{
    if(!selectedPilotId) { setEntries([]); return; }
    ...
    (async()=>{
      ...
      }catch(e){ console.warn("audit_log fetch failed:", e.message); setEntries([]); }
