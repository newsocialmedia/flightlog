...
body{background:${C.base};color:${C.ink};font-family:'Inter',system-ui,sans-serif;line-height:1.5;font-size:14px;overflow:hidden;height:100dvh;width:100%;}
  ...
  return(
    <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"grid",gridTemplateColumns:"minmax(0,7fr) minmax(0,5fr)",gap:0}} className="cal-desktop-grid">
        ...
        {/* LEFT: Calendar */}
        <div style={{overflowY:"auto",padding:"20px 16px 20px 20px",display:"flex",flexDirection:"column",gap:16}} className="cal-left-panel">
      ...
      <style>{`
        @media(max-width:768px){
          ...
          .cal-left-panel{overflow-y:visible!important;min-height:0;}
        }
      `}</style>
    </div>
  );
...
function ActiveLogsPage({user, rosters, tails, onRosterUpdated, onTailSaved}) {
  ...
  if(drillFlight){
    ...
    return <FlightDetailPage
      ...
      onAutoLookup={async()=>{try{const res=await fetch(`${SUPA_URL}/functions/v1/lookup-flight`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${sb.auth._token||SUPA_ANON}`,"apikey":SUPA_ANON},body:JSON.stringify({flightNum:f.flightNum,date:dateStr,dep:f.dep,arr:f.arr})});const d=await res.json();if(d.tail||d.actualDep){const u={...tail,...d};onTailSaved(tk,u);setDrillFlight(p=>({...p,tail:u,hasActual:true}));}}catch(e){alert(e.message);}}}
...
function TotalTimesTab({analytics, rosters, tails}) {
  ...
  // Build cumulative totals matching Jeppesen logbook columns
  const totals = useMemo(()=>{
    let totalMins=0, picMins=0, sicMins=0, nightMins=0, xcMins=0,
        ifrMins=0, turbineMins=0, multiMins=0, singleMins=0,
        simMins=0, dayLdg=0, nightLdg=0, approaches=0,
        totalLegs=0, airports=new Set();
    ...
    for(const r of rosters){
      ...
      (r.calendar||[]).forEach((d,di)=>{
        (d.flights||[]).forEach((f,fi)=>{
          ...
          // Multi-engine: most commercial jets/turboprops
          ...
          totalMins+=mins; totalLegs+=1;
          // Assume SIC for airline ops (adjust if you track PIC separately)
          sicMins+=mins;
          ...
          // Landings: 1 per leg (day or night based on arrival)
    ...
    return{totalMins,picMins,sicMins,nightMins,xcMins,ifrMins,turbineMins,
           multiMins,singleMins,simMins,dayLdg,nightLdg,approaches,
           totalLegs,airports:airports.size};
  },[rosters,tails]);

  const BLUE="#1D4ED8"; const PURPLE="#7C3AED";

  const boxes = [
    {label:"Total Time",    val:fmtMins(totals.totalMins),     color:BLUE,   desc:"All logged block time"},
    {label:"PIC",           val:fmtMins(totals.picMins),       color:PURPLE, desc:"Pilot-in-command"},
    {label:"SIC / Co-Pilot",val:fmtMins(totals.sicMins),       color:PURPLE, desc:"Second-in-command"},
    {label:"Night",         val:fmtMins(totals.nightMins),     color:"#0F172A", desc:"Civil twilight or later"},
    {label:"Cross Country", val:fmtMins(totals.xcMins),        color:"#059669", desc:"Legs >50 NM straight-line"},
    {label:"Actual IFR",    val:fmtMins(totals.ifrMins),       color:"#7C3AED", desc:"Tracked when synced"},
    {label:"Turbine",       val:fmtMins(totals.turbineMins),   color:BLUE,   desc:"Jet or turboprop aircraft"},
    {label:"Multi-Engine",  val:fmtMins(totals.multiMins),     color:BLUE,   desc:"Multi-engine rated flights"},
    {label:"Single-Engine", val:fmtMins(totals.singleMins),    color:"#F59E0B", desc:"Single-engine flights"},
    {label:"Simulator",     val:fmtMins(totals.simMins),       color:S.muted, desc:"SIM duty days"},
    {label:"Day Landings",  val:totals.dayLdg,                 color:"#059669", desc:"Daytime arrivals"},
    {label:"Night Landings",val:totals.nightLdg,               color:"#0F172A", desc:"Nighttime arrivals"},
    {label:"Approaches",    val:totals.approaches,             color:PURPLE, desc:"IFR approaches logged"},
    {label:"Total Legs",    val:totals.totalLegs,              color:BLUE,   desc:"All flight legs"},
    {label:"Airports",      val:totals.airports,               color:"#059669", desc:"Unique airports visited"},
  ];

  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {/* Header */}
      <div style={{background:`linear-gradient(135deg,${BLUE},${PURPLE})`,borderRadius:18,padding:"16px 18px",color:"#fff",marginBottom:4}}>
        <div style={{fontSize:14,fontWeight:800,letterSpacing:"-.3px"}}>Jeppesen Logbook — Total Times</div>
        <div style={{fontSize:12,color:"rgba(255,255,255,0.7)",marginTop:3}}>All recorded flight time from verified rosters · Updated automatically</div>
      </div>

      {/* Time boxes grid — matches Jeppesen column layout */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:10}}>
        {boxes.map(({label,val,color,desc})=>(
          <div key={label} style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:16,padding:"14px 16px",display:"flex",flexDirection:"column",gap:6}}>
            ...
            <div style={{fontSize:10,color:S.muted,lineHeight:1.4}}>{desc}</div>
          </div>
        ))}
      </div>

      {/* Note */}
      <div style={{background:S.panel,borderRadius:12,padding:"12px 14px",fontSize:11,color:S.muted,lineHeight:1.6}}>
        <strong style={{color:S.ink}}>Note:</strong> PIC time is tracked when your roster or airline ops spec designates you as PIC. SIC time reflects co-pilot ops. IFR actual time and approaches require manual entry or future AeroAPI integration. Times accumulate from all <strong>verified & signed</strong> rosters.
      </div>
    </div>
  );
}
...
function AnalyticsPage({user,rosters,tails}){
  ...
  return(
    <div>
      ...
      {tab==="totaltimes"&&<TotalTimesTab analytics={analytics} rosters={rosters} tails={tails}/>}
      {tab==="overview"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
...
// ─── BOTTOM TAB BAR ───────────────────────────────────────────────────────────
const TAB_PAGES = ["dashboard","calendar","active-logs","upload","more"];
...
export default function App() {
  ...
  return(
    ...
    <div key={themeKey}>
      ...
      {screen==="app"&&user&&(
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base}} className="app-shell">
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row",minHeight:0}}>
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
              ...
              <div style={{flex:1,minHeight:0,overflow:"hidden",display:"flex",flexDirection:"column"}} className="mobile-page-content">
