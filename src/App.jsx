...
function ExportPage({rosters, tails}) {
  ...
  function doCSVExport() {
    ...
    const headers=["Date","Flight","From","To","Dep Time","Arr Time","Tail","Aircraft","Block Hrs","Night Hrs","XC Hrs","Day Hrs","Distance NM","Cancelled"];
    const csv=[
      headers.join(","),
      ...rows.map(r=>[r.date,r.flightNum,r.dep,r.arr,r.depTime,r.arrTime,r.tail,r.acType,r.blockHrs,r.nightHrs,r.xcHrs,r.dayHrs,r.distNM,r.cancelled?"Yes":"No"].map(v=>`"${v}"`).join(","))
    ].join("\n");
    ...
    const csv=[
      ...
      ...rows.map(r=>{
        ...
        return [
          ...
          r.cancelled?"CANCELLED":r.flightNum,
        ].map(v=>`"${v}"`).join(",");
      })
    ].join("\n");
