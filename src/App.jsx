...
function AddFlightPage({user, rosters, onRosterSaved, setPage}) {
  ...
  async function save(){
    ...
    const wasFirstEverRoster = rosters.length===0; // captured BEFORE the save
    ...
    try{
      ...
      setSuccess(true);
      setTimeout(()=>{setPage&&setPage(wasFirstEverRoster?"onboarding-tutorial":"active-logs");},1200);
...
// -- Onboarding tutorial: shown once, right after a pilot's genuinely
// first-ever save (roster upload or manual Add Flight), confirmed via
// wasFirstEverRoster in confirmSave/AddFlightPage's save(). Five slides,
// each pairing a short explanation with a genuine, live-rendered mini-
// preview of that real feature -- built from this app's own actual color
// tokens, badge patterns, and layout conventions (confirmed directly from
// the real components: the Upcoming Flight card, Active Logs' verify flow,
...
function OnboardingTutorial({setPage}) {
  ...
  function finish(){ setPage?.("dashboard"); }
...
export default function App() {
  ...
  async function handleAuth(u){
    ...
    setPage(u.role==="admin"?"admin-overview":(isBrandNewPilot?"onboarding-start":"dashboard"));
    setScreen("app");
      ...
      {screen==="app"&&user&&(()=>{
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base,position:"fixed",top:0,left:0,right:0,bottom:0}} className="app-shell">
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row",minHeight:0}}>
            {!["onboarding-start","onboarding-tutorial"].includes(page)&&(
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
              ...
              {/* Fixed bottom nav -- outside scrollable area so it never
                  scrolls away. Hidden entirely during onboarding (start
                  screen or tutorial) so a brand-new pilot can't navigate
                  away from the flow before finishing it or tapping Skip. */}
              {!["onboarding-start","onboarding-tutorial"].includes(page)&&(
