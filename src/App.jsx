...
function PostUploadVerifyScreen({parsedRoster, rosters, user, onRosterSaved, setParsedRoster, setStatus, setMsg, setPage}) {
  const S = getS();
    ...
    // Don't check for r._carryover (that flag is never set); just find any
    // roster for the following month that already exists in state, since the
    // only way it exists right after an upload is via the carryover logic.
    const nextMNum  = mNum===11?0:mNum+1;
    const nextYear  = mNum===11?parsedRoster.year+1:parsedRoster.year;
    const nextMonthCarryover = rosters.find(r=>
    ...
    async function saveCalendar(newCal, roster=activeRoster) {
      ...
      try {
        ...
        onRosterSaved(updated); // updates rosters[] in parent App state for both tabs
        if(vTab==="current") setParsedRoster(updated);
        // For carryover tab, onRosterSaved updates rosters[] which is the source
        // nextMonthCarryover reads from -- no extra state needed
      } catch(e) { alert(e.message); }
    }
...
function UploadPage({user, rosters, onRosterSaved, setPage}) {
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base,position:"fixed",top:0,left:0,right:0,bottom:0}} className="app-shell">
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row",minHeight:0}}>
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
              ...
              {page==="upload"&&<UploadPage user={user} rosters={rosters} onRosterSaved={handleRosterSaved} setPage={navigate}/>}
