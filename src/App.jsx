...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    // ── Extract carry-forward days ──
    const daysInMonth = new Date(roster.year, roster.monthNum+1, 0).getDate();
    const cal = roster.calendar || [];

    // Debug: log what we're working with
    console.log(`[carry-forward] Feb has ${daysInMonth} days, calendar has ${cal.length} days`);
    console.log(`[carry-forward] Last 5 days:`, cal.slice(-5).map(d=>d.day));

    let carryForwardDays = [];
    let thisDays = cal;

    if(Array.isArray(roster.carryForwardDays) && roster.carryForwardDays.length > 0) {
      carryForwardDays = roster.carryForwardDays;
      thisDays = cal.filter(d => d.day >= 1 && d.day <= daysInMonth);
      console.log(`[carry-forward] Using explicit carryForwardDays:`, carryForwardDays.map(d=>d.day));
    } else {
      // Pattern A: day counter resets (e.g. 28→1 means March 1 after Feb 28)
      let carryStartIdx = cal.length;
      for(let i = 1; i < cal.length; i++) {
        if(cal[i].day < cal[i-1].day && cal[i].day <= 5 && cal[i-1].day >= 20) {
          carryStartIdx = i;
          console.log(`[carry-forward] Pattern A detected at index ${i}: day ${cal[i-1].day}→${cal[i].day}`);
          break;
        }
      }
      // Pattern B: days explicitly numbered beyond month end
      const resetCarry = cal.slice(carryStartIdx);
      const highCarry  = cal.slice(0, carryStartIdx).filter(d => d.day > daysInMonth);
      const rawCarry = [
        ...highCarry.map(d => ({...d, day: d.day - daysInMonth})),
        ...resetCarry,
      ];
      carryForwardDays = rawCarry.filter(d => d.day >= 1 && d.day <= 10); // allow up to day 10 not just 5
      thisDays = cal.slice(0, carryStartIdx).filter(d => d.day >= 1 && d.day <= daysInMonth);
      console.log(`[carry-forward] Detected ${carryForwardDays.length} carry-forward days:`, carryForwardDays.map(d=>d.day));
      console.log(`[carry-forward] thisDays count:`, thisDays.length);
    }

    // Route carry-forward days to next month's roster
    if(carryForwardDays.length > 0) {
      ...
      const normalizedCarry = carryForwardDays.filter(d => d.day >= 1 && d.day <= nextDaysInMonth);

      if(normalizedCarry.length > 0) {
        ...
        if(nextRoster?.id) {
          // Next month exists — merge without duplicating
          ...
          // Next month not uploaded yet — create stub roster
    ...
    // ── Now save this month's roster (update or insert) ──
    ...
    if(existing?.id) {
      // Update with only this month's days (no carry-forward contamination)
      const {error} = await sb.from("rosters")
        .update({period_label:roster.periodLabel, calendar:thisDays})
        .eq("id", existing.id);
...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  // Fetch gate info for next flight — only within 6 hours of departure
  useEffect(()=>{
    if(!nextFlight) { setGateInfo(null); return; }
    ...
    if(hoursUntil > 6 || hoursUntil < -1) return;
    fetch(`${SUPA_URL}/functions/v1/lookup-flight`, {
      ...
      body: JSON.stringify({flightNum:f.flightNum, date:dateStr, depTime:f.depTime}),
    }).then(res=>res.json()).then(data=>{
      if(data.depGate||data.arrGate||data.depTerminal) {
        setGateInfo({depGate:data.depGate, arrGate:data.arrGate, depTerminal:data.depTerminal, arrTerminal:data.arrTerminal});
      }
    }).catch(()=>{});
  },[nextFlight?.f?.flightNum]);
...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  // RosterBuster exact colors from screenshot 1
  const TYPE_STYLE = {
    flight:  {bg:"#2C7BE5", text:"#FFFFFF", sub:"rgba(255,255,255,0.8)"},
    flown:   {bg:"#00875A", text:"#FFFFFF", sub:"rgba(255,255,255,0.8)"},
    standby: {bg:"#F59E0B", text:"#FFFFFF", sub:"rgba(255,255,255,0.8)"},
    off:     {bg:"#FFFFFF", text:"#374151", sub:"#9CA3AF"},
  };

  const isToday = d => today.getFullYear()===year&&today.getMonth()===mNum&&today.getDate()===d;

  function openDay(day) { setSelDay(day); setSheetOpen(true); setAdding(false); }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden",background:"#F2F2F7",position:"relative"}}>

      {/* Month label + roster tabs */}
      <div style={{background:"#FFFFFF",borderBottom:"1px solid #E5E7EB",flexShrink:0}}>
        {/* Month + roster selector row */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px 4px"}}>
          <div style={{fontSize:20,fontWeight:700,color:"#111827"}}>{monthLabel}</div>
          <div style={{display:"flex",gap:0,overflowX:"auto"}}>
            {rosters.map((r,i)=>(
              <button key={r.id} onClick={()=>{setSelRoster(i);closeSheet();}} style={{
                padding:"4px 10px",borderRadius:6,border:"none",
                background:selRoster===i?"#2C7BE5":"none",
                color:selRoster===i?"#fff":"#6B7280",
        ...
        {/* Legend */}
        <div style={{display:"flex",gap:14,padding:"6px 16px 10px"}}>
      ...
      <div style={{
        ...
        {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=>(
          <div key={d} style={{textAlign:"center",fontSize:10,fontWeight:600,color:"#9CA3AF",padding:"4px 0",letterSpacing:".3px"}}>
            {d}
          </div>
        ))}
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:sheetOpen?360:0}}>
        <div style={{
          display:"grid",
          ...
          {cells.map((day,idx)=>{
            if(!day) return <div key={`b${idx}`} style={{background:"#F9FAFB",minHeight:72}}/>;
            ...
            // Day totals
            const dayBlock = flights.reduce((a,f,fi)=>{
              const t=tails[`${roster.id}-${di}-${fi}`]||{};
              return a+(t.actualBlockMins??schedMins(f)??0);
            },0);
            ...
            return(
              <div key={day} onClick={()=>openDay(day)} style={{
                ...
                outline: selected?`2px solid #2C7BE5`:"none",
                outlineOffset:"-2px",
                ...
                <div style={{
                  ...
                  background:_today?"#2C7BE5":type==="off"?"transparent":"rgba(0,0,0,0.15)",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  marginBottom:3,
                }}>
                  <span style={{
                    fontSize:11,fontWeight:_today?700:600,lineHeight:1,
                    color:_today?"#fff":style.text,
                  }}>{day}</span>
                </div>

                {/* Flight info */}
                {flights.slice(0,3).map((f,fi)=>{
                  const t=tails[`${roster.id}-${di}-${fi}`]||{};
                  return(
                    <div key={fi} style={{marginBottom:1,lineHeight:1.15}}>
                      <div style={{fontSize:8,fontWeight:700,color:style.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                        {f.dep}-{f.arr}
                      </div>
                      <div style={{fontSize:7,color:style.sub,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                        {t.actualDep||f.depTime}
                      </div>
                    </div>
                  );
                })}
                {flights.length>3&&<div style={{fontSize:7,color:style.sub}}>+{flights.length-3}</div>}

                {/* Standby code */}
                {type==="standby"&&d?.dutyCode&&!flights.length&&(
                  <div style={{fontSize:9,fontWeight:700,color:style.text}}>{d.dutyCode}</div>
                )}

                {/* Block total bottom right */}
                {dayBlock>0&&(
                  <div style={{position:"absolute",bottom:3,right:3,fontSize:7,fontWeight:700,color:style.sub}}>
                    {fmtMins(dayBlock)}
                  </div>
                )}
              </div>
      ...
      {/* Backdrop */}
      {sheetOpen&&<div onClick={closeSheet} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.35)",zIndex:99}}/>}
      ...
      <div style={{
        ...
        <div style={{padding:"8px 16px 10px",borderBottom:"1px solid #F3F4F6",flexShrink:0}}>
            ...
            <div>
              ...
              {sel?.d?.flights?.length>0&&(
                <div style={{fontSize:11,color:"#6B7280",marginTop:1}}>
                  {sel.d.flights.length} leg{sel.d.flights.length>1?"s":""} · {fmtMins(
                    sel.d.flights.reduce((a,f,fi)=>{const t=tails[`${roster.id}-${sel.di}-${fi}`]||{};return a+(t.actualBlockMins??schedMins(f)??0);},0)
                  )}
                </div>
              )}
            </div>
            <button onClick={closeSheet} style={{width:28,height:28,borderRadius:"50%",background:"#F3F4F6",border:"none",cursor:"pointer",fontSize:16,color:"#6B7280",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
          </div>
        </div>

        <div style={{overflowY:"auto",flex:1,paddingBottom:20}}>
          {!sel?.d?.flights?.length&&!sel?.d?.dutyCode&&(
            <div style={{textAlign:"center",padding:"28px 0",color:"#9CA3AF",fontSize:13}}>Rest day</div>
          )}
          {sel?.d?.dutyCode&&!sel?.d?.flights?.length&&(
            <div style={{margin:"10px 16px",padding:"12px",borderRadius:10,background:"#FFFBEB",border:"1px solid #FCD34D"}}>
              <div style={{fontSize:14,fontWeight:700,color:"#B45309"}}>{sel.d.dutyCode}</div>
            </div>
          )}
          ...
          {(sel?.d?.flights||[]).map((f,fi)=>{
            ...
            return(
              <div key={fi} onClick={()=>{
                ...
                <div style={{width:4,height:48,borderRadius:2,background:synced?"#00875A":"#2C7BE5",marginRight:12,flexShrink:0}}/>
                <div style={{minWidth:50,flexShrink:0,marginRight:12}}>
                  <div style={{fontSize:15,fontWeight:700,color:"#111827"}}>{hasActual?t.actualDep:f.depTime}</div>
                  <div style={{fontSize:11,color:"#9CA3AF",marginTop:2}}>{hasActual?t.actualArr:f.arrTime}</div>
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}>
                    <span style={{fontSize:12,fontWeight:700,color:"#2C7BE5"}}>{f.flightNum}</span>
                    {f.acType&&<span style={{fontSize:10,color:"#9CA3AF",background:"#F3F4F6",padding:"1px 5px",borderRadius:3}}>{f.acType}</span>}
                    {(dist||0)>50&&<span style={{fontSize:9,color:"#2C7BE5",background:"#EFF6FF",padding:"1px 5px",borderRadius:3,fontWeight:700}}>XC</span>}
                    {solar.nightMins>0&&<span style={{fontSize:9,color:"#7C3AED",background:"#EDE9FE",padding:"1px 5px",borderRadius:3}}>🌙</span>}
                  </div>
                  <div style={{fontSize:15,fontWeight:700,color:"#111827"}}>{f.dep} → {f.arr}</div>
                  {t.tail&&<div style={{fontSize:10,fontFamily:"monospace",color:"#6B7280",marginTop:2}}>{t.tail}{synced&&<span style={{color:"#00875A",marginLeft:4}}>✓</span>}</div>}
                </div>
                <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                  {bMins>0&&<div style={{fontSize:13,fontWeight:700,color:"#111827"}}>{fmtMins(bMins)}</div>}
                  {dist&&<div style={{fontSize:10,color:"#9CA3AF",marginTop:2}}>{dist} NM</div>}
                </div>
                <div style={{color:"#D1D5DB",marginLeft:8,fontSize:18}}>›</div>
              </div>
            );
          })}
          ...
          {!adding?(
            <div style={{padding:"10px 16px"}}>
              <button onClick={()=>setAdding(true)} style={{width:"100%",padding:"10px",borderRadius:8,background:"none",border:"1px dashed #D1D5DB",color:"#9CA3AF",fontSize:13,cursor:"pointer"}}>
                + Add flight
              </button>
            </div>
          ):(
            <div style={{margin:"8px 16px",padding:"12px",borderRadius:10,background:"#F9FAFB",border:"1px solid #E5E7EB"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={saveNewFlight} disabled={saving} style={{flex:1,padding:"9px",borderRadius:7,background:"#2C7BE5",border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  {saving?<span className="spinner">⟳</span>:"Save"}
                </button>
                <button onClick={()=>setAdding(false)} style={{flex:1,padding:"9px",borderRadius:7,background:"none",border:"1px solid #E5E7EB",color:"#6B7280",fontSize:13,cursor:"pointer"}}>Cancel</button>
              </div>
            </div>
          )}
        </div>
...
function FlightDetailPage({flight:f, tail, solar, dist, blockMins, day, roster, hasActual, dep, arr, isXC, onBack, onAutoLookup, lkStatus, lkError, tmp, onTmpChange, onSaveTail, saving, onTailSaved, editingTimes, setEditingTimes, timeEdits, setTimeEdits, di, fi, userId}) {
      ...
      <div className="card" style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:editingTimes?12:0}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={onAutoLookup} disabled={lkStatus==="loading"||lkStatus==="ratelimit"} style={{padding:"5px 12px",borderRadius:7,background:C.teal+"11",border:`1px solid ${C.teal}44`,color:C.teal,fontSize:12,fontWeight:600,cursor:"pointer"}}>
              {lkStatus==="loading"?<span className="spinner">⟳</span>:lkStatus==="notfound"?"Not found":lkStatus==="ratelimit"?"⏳ Limit":"🔍 Auto"}
            </button>
          </div>
        </div>
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed}) {
  ...
  async function autoLookup(di,fi,f,dayNum){
    ...
    const limit = checkAutoSyncLimit(user.id);
    if(!limit.allowed) {
      setLkStatus(p=>({...p,[tk]:"ratelimit"}));
      setLkError(p=>({...p,[tk]:`Daily limit reached (${AUTO_SYNC_DAILY_LIMIT}/day). Resets at midnight.`}));
      return;
    }
    ...
    console.log("[autoLookup] flightNum:", f.flightNum, "date:", dateStr, "depTime:", f.depTime, "tk:", tk);
    setLkStatus(p=>({...p,[tk]:"loading"}));
    try{
      incrementAutoSyncCount(user.id); // count before calling to prevent rapid tapping
...
const MORE_PAGES = ["analytics","map","profile","settings"];
  ...
  const tabs = [
    {id:"dashboard", icon:(
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M3 12L12 3L21 12V21H15V15H9V21H3V12Z" fill={active==="dashboard"?"#2C7BE5":"#9CA3AF"}/>
      </svg>
    )},
    {id:"calendar", icon:(
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <rect x="3" y="4" width="18" height="17" rx="2" stroke={active==="calendar"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" fill="none"/>
        <path d="M8 2V6M16 2V6M3 9H21" stroke={active==="calendar"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" strokeLinecap="round"/>
        <rect x="7" y="13" width="3" height="3" rx="0.5" fill={active==="calendar"?"#2C7BE5":"#9CA3AF"}/>
        <rect x="14" y="13" width="3" height="3" rx="0.5" fill={active==="calendar"?"#2C7BE5":"#9CA3AF"}/>
      </svg>
    )},
    {id:"logbook", icon:(
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <rect x="4" y="2" width="13" height="18" rx="2" stroke={active==="logbook"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" fill="none"/>
        <path d="M4 6H17M4 10H14M4 14H11" stroke={active==="logbook"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" strokeLinecap="round"/>
        <circle cx="18" cy="18" r="4" fill={active==="logbook"?"#2C7BE5":"#9CA3AF"}/>
        <path d="M18 16V18L19.5 19.5" stroke="white" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    )},
    {id:"upload", icon:(
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M12 3L12 15M12 3L8 7M12 3L16 7" stroke={active==="upload"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M4 17V19C4 20.1 4.9 21 6 21H18C19.1 21 20 20.1 20 19V17" stroke={active==="upload"?"#2C7BE5":"#9CA3AF"} strokeWidth="2" strokeLinecap="round"/>
      </svg>
    )},
    {id:"more", icon:(
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <circle cx="5" cy="12" r="2" fill={active==="more"?"#2C7BE5":"#9CA3AF"}/>
        <circle cx="12" cy="12" r="2" fill={active==="more"?"#2C7BE5":"#9CA3AF"}/>
        <circle cx="19" cy="12" r="2" fill={active==="more"?"#2C7BE5":"#9CA3AF"}/>
      </svg>
    )},
  ];
    ...
    <div style={{
      ...
      {tabs.map(tab=>(
        <button key={tab.id} onClick={()=>setPage(tab.id)} style={{
          ...
          {active===tab.id&&(
            <div style={{position:"absolute",bottom:6,width:4,height:4,borderRadius:"50%",background:"#2C7BE5"}}/>
          )}
        </button>
      ))}
    </div>
...
function MorePage({user, setPage, onLogout, isDark, onToggleTheme, rosters, tails}) {
  const items = [
    {id:"analytics",   icon:"📊", label:"Statistics"},
    {id:"map",         icon:"🗺️",  label:"Route Map"},
    {id:"profile",     icon:"👤", label:"Profile"},
    {id:"settings",    icon:"⚙️",  label:"Settings"},
  ];
  return (
    <div style={{flex:1,overflowY:"auto",background:C.base,padding:"24px 0"}}>
      <div style={{padding:"0 16px 16px",fontSize:13,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".8px"}}>More</div>
      {items.map(item=>(
      ...
      {screen==="app"&&user&&(
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base}}>
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
            ...
            {page==="settings"&&<MorePage user={user} page={page} setPage={navigate} onLogout={handleLogout} isDark={isDark} onToggleTheme={handleToggleTheme} rosters={rosters} tails={tails}/>}
          ...
          <BottomTabBar page={page} setPage={navigate}/>
        </div>
      )}
      <InstallPrompt/>
    </div>
