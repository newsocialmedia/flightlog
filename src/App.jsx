    ...
    try {
      ...
      try {
        ...
        for(let p=1; p<=pdfDoc.numPages; p++){
          ...
          // Sort lines top-to-bottom (highest Y first in PDF coords), items left-to-right
          const sortedYs = Object.keys(byY).map(Number).sort((a,b)=>b-a);
          const pageLines = [];
          for(const y of sortedYs){
            const items = byY[y].sort((a,b)=>a.x-b.x);
            // Build a line with approximate spacing based on X positions
            // Use a character-per-unit ratio to place text at correct column positions
      ...
      // -- Try deterministic client-side parse first (instant, no API call) ---
      if(layoutText.length > 200 && /FlD/.test(layoutText) && /G7\s+\d{3,4}/.test(layoutText)) {
        setMsg("Parsing roster (deterministic)...");
        try {
          // Period detection from text
      ...
      if(!roster) {
        ...
        // Send layoutText to the edge function so it doesn't need pdftotext server-side.
        // The edge function uses it directly for the AI parse, falling back to
        // raw PDF vision only if layoutText is empty (scanned/image PDFs).
        const body = layoutText.length > 200
          ? { layoutText, filename: file.name }
