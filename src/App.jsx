  ...
  async function checkSubscription() {
    try {
      ...
      const { data, error } = await sb.from("profiles")
        .select("subscription_status,plan,subscription_end")
        .eq("id", user.id);
      if(error) { console.error("[Wall]", error); return false; }
      const profile = Array.isArray(data) ? data[0] : data;
      console.log("[Wall] status from DB:", profile?.subscription_status);
