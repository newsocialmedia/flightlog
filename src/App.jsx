...
function getAirportUtcOffsetMins(airportCode: string): number {
  const tz = AIRPORT_TZ[airportCode];
  if(!tz) return 0;
  try {
    // Use Intl to get actual current UTC offset including DST
    const now = new Date();
    ...
    // Parse "GMT-5" or "GMT+5:30" format
  ...
  useEffect(()=>{
    try {
      ...
      if(saved) {
        ...
        setSelectedFlight(flightData);
        sessionStorage.removeItem("fl_open_flight"); // clear after restore
      }
    } catch {}
  },[]);
