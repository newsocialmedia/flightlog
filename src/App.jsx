...
async function fetchProfile(userId, accessToken) {
  try {
    let token = accessToken || SUPA_ANON;
    if(!accessToken) {
      try {
...
function MorePage({user, setPage, onLogout, isDark, onToggleTheme, rosters, tails}) {
  ...
  const adminItems = [
    {id:"admin-overview", icon:"◈",  label:"Admin Overview"},
...
export default function App() {
  ...
  async function handleAuth(u) {
    setUser(u);
    // TEMP DEBUG — shows what role was received
    console.log("handleAuth user:", JSON.stringify({id:u.id, email:u.email, role:u.role, name:u.name}));
    try {
      const [rs,ts]=await Promise.all([db_loadRosters(u.id),db_loadTails(u.id)]);
      setRosters(rs); setTails(ts);
    } catch {}
    ...
    <div key={themeKey}>
      ...
      {screen==="app"&&user&&(
        <div style={{display:"flex",flexDirection:"column",height:"100dvh",overflow:"hidden",background:C.base}}>
          ...
          {/* TEMP DEBUG - remove after fixing admin */}
          {user&&<div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,background:user.role==="admin"?"#00875A":"#FF0000",color:"#fff",fontSize:11,textAlign:"center",padding:"2px 0"}}>
            Role: {user.role||"undefined"} | Email: {user.email} | ID: {user.id?.slice(0,8)}
          </div>}
          ...
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"row"}}>
            ...
            <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"}}>
              ...
              {page==="admin-users"&&user?.role==="admin"&&<div style={{flex:1,overflowY:"auto"}}><AdminUsers/></div>}
              {page==="admin-rosters"&&user?.role==="admin"&&<div style={{flex:1,overflowY:"auto"}}><AdminRosters/></div>}
