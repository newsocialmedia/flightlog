...
function UploadPage({user, rosters, onRosterSaved, onReloadRosters, setPage}) {
  ...
  async function parseRoster() {
    ...
    try {
      ...
      let layoutText = "";
      let pdfItems = []; // [{x, y, str, page}] exact visual coordinates from pdf.js
      let roster;
      try {
        ...
        for(let p=1; p<=pdfDoc.numPages; p++){
          ...
          // Group text items by Y position (rounded to nearest 3 units = same line)
          const byY = {};
          for(const item of content.items){
            ...
            // Collect exact-coordinate item for the deterministic parser.
            // viewport.transform flips Y so y increases DOWNWARD (visual order).
            pdfItems.push({x, y, str: item.str, page: p});
            const yKey = Math.round(y/3)*3;
            if(!byY[yKey]) byY[yKey]=[];
            byY[yKey].push({x, text:item.str});
          }
          // Sort lines top-to-bottom (y increases downward in viewport space)
          ...
          for(const y of sortedYs){
            ...
            const lineLen = Math.ceil(maxX/6) + 20;
      ...
      if(!roster) {
        ...
        // Send layoutText for fast AI parse + pdfBase64 as fallback for scanned PDFs
        // or if the layout text AI call fails. Always send both.
        const body = { layoutText, pdfBase64: base64, filename: file.name };
  ...
  function parseNetlineFromItems(items) {
    ...
    // ── Day-header strip: the page-1 Y band holding the most day labels ──────
    const p1 = items.filter(i=>i.page===1);
    const pureLabels = p1.filter(i=>{const s=i.str.trim();return DAY_PREFIX.test(s)&&s.length<=5;});
    const bands = {};
    pureLabels.forEach(i=>{const b=Math.round(i.y/5)*5;(bands[b]=bands[b]||[]).push(i);});
    let strip=[], stripY=0;
    Object.entries(bands).forEach(([b,arr])=>{ if(arr.length>strip.length){strip=arr;stripY=Number(b);} });
    if(strip.length<15) return null; // full strip has ~28-31 labels

    const stripSorted=[...strip].sort((a,b)=>a.x-b.x);

    // ── Marker row: FlD / LCR / SIM / TVL... directly below the strip ────────
    const fldAll = p1.filter(i=>i.str.trim()==='FlD' && i.y>stripY && i.y<stripY+50);
    if(!fldAll.length) return null;
    const fldY = fldAll.map(i=>i.y).sort((a,b)=>a-b)[Math.floor(fldAll.length/2)]; // median
    const markers = p1.filter(i=>{
      const s=i.str.trim();
      return Math.abs(i.y-fldY)<7 && /^[A-Z][a-zA-Z]{1,4}$/.test(s) && !HEADER_WORDS.has(s) && !DAY_PREFIX.test(s);
    });
    ...
    for(const lab of stripSorted){
      const d=parseInt(DAY_PREFIX.exec(lab.str.trim())[2]);
      ...
      const marker=near.sort((a,b)=>Math.abs(a.x-lab.x)-Math.abs(b.x-lab.x))[0].str.trim();
