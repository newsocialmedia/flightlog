...
function UploadPage({user, rosters, onRosterSaved, onReloadRosters, setPage}) {
  ...
  async function parseRoster() {
    ...
    try {
      ...
      // no character-cell drift. Self-validating: returns null unless every FlD
      // duty day has flights, so it's either provably complete or defers to AI.
      if(pdfItems.length > 50) {
        ...
        } catch(e) { console.warn('[Upload] Items parser failed:', e?.message); }
      }
      ...
      if(!roster && layoutText.length > 200 && /FlD/.test(layoutText) && /G7\s*\d{3,4}/.test(layoutText)) {
        ...
        } catch(e) { console.warn('[Upload] Deterministic parse failed:', e?.message); }
      }
  ...
  function parseNetlineFromItems(items) {
    ...
    if(strip.length<15) return null; // full strip has ~28-31 labels
    ...
    if(!markerHitsRaw.length) return null;
    const fldHits = markerHitsRaw.filter(h=>h.groups[1]==='FlD');
    if(!fldHits.length) return null;
    ...
    if(dutyDays.size===0 && Object.keys(codeDays).length===0) return null;
    ...
    for(const d of dutyDays){
      ...
      if(!day || day.flights.length===0) return null;
    }
    // 2. No off day may have C/I content lurking in its column (unrecognized duty)
    for(const day of calendar){
      if(day.isOff && dayTexts[day.day] && /C\/I/.test((dayTexts[day.day]||[]).join(' '))) return null;
    }
    ...
    for(const d of carryDuty){
      ...
      if(flights.length===0) return null; // carry duty day missing flights — defer
