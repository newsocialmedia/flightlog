...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  if(!roster) return (
    <div style={{textAlign:"center",padding:60,color:C.muted}}>
      No rosters yet. Upload a PDF roster to get started.
    </div>
  );
  ...
  const firstDow = new Date(year, monthNum, 1).getDay(); // 0=Sun
  ...
  function dayStatus(day) {
    ...
    const {d, di} = entry;
    if(!d.flights?.length) return d.dutyCode ? "duty" : "off";
    const allFlown = d.flights.every((_,fi) => tails[`${roster.id}-${di}-${fi}`]?.actualBlockMins != null);
    const anyFlown = d.flights.some((_,fi) => tails[`${roster.id}-${di}-${fi}`]?.actualBlockMins != null);
    if(allFlown) return "flown";
    if(anyFlown) return "partial";
    return "scheduled";
  }

  function getDutyColor(status) {
    switch(status) {
      case "flown":     return {bg:"#1A3D5C", border:"#2D8CF0", text:"#7EC8F8"};
      case "partial":   return {bg:"#1A3D40", border:"#34C97A", text:"#7EF8B8"};
      case "scheduled": return {bg:"#1A2D1A", border:"#34C97A", text:"#A0F0A0"};
      case "duty":      return {bg:"#2D2200", border:"#F0A030", text:"#F0C860"};
      case "off":       return {bg:C.base,    border:C.border,  text:C.muted};
      default:          return {bg:C.base,    border:C.border,  text:C.muted};
    }
  }

  const isToday = (day) => today.getFullYear()===year && today.getMonth()===monthNum && today.getDate()===day;
  ...
  async function saveNewFlight() {
    ...
    if(!fn||!dep||!arr||!depTime||!arrTime) return;
    setSaving(true);
    ...
    try {
      await db_saveRoster(user.id, {...roster, calendar:newCalendar});
  ...
  async function deleteFlight(fi) {
    ...
    const di = selected.di;
    const newCalendar = [...(roster.calendar||[])];
    const newFlights = (newCalendar[di].flights||[]).filter((_,i)=>i!==fi);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"10px 16px",overflowX:"auto"}}>
          {rosters.map((r,i)=>(
            <button key={r.id} onClick={()=>{setSelRoster(i);closeSheet();}}
              className={"month-tab"+(selRoster===i?" active":"")}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px 10px"}}>
          <div style={{fontSize:16,fontWeight:700,color:C.ink}}>{monthLabel}</div>
          <div style={{display:"flex",gap:12,fontSize:11,color:C.muted}}>
            <span><span style={{color:C.teal}}>●</span> {(roster.calendar||[]).filter(d=>d.flights?.length>0).length} duty</span>
            <span><span style={{color:C.silver}}>●</span> {daysInMonth - (roster.calendar||[]).filter(d=>d.flights?.length>0||d.dutyCode).length} off</span>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d=>(
          <div key={d} style={{textAlign:"center",fontSize:10,fontWeight:700,color:C.muted,padding:"6px 0",letterSpacing:".5px"}}>
            {d}
          </div>
        ))}
      </div>
      ...
      {/* Calendar grid */}
      <div style={{flex:1,overflowY:"auto",paddingBottom:sheetVisible?280:0}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:1,background:C.border}}>
          {cells.map((cell,idx)=>{
            if(cell.type==="blank") return (
              <div key={`b${idx}`} style={{background:C.base,minHeight:88}}/>
            );
            const day = cell.day;
            const entry = dayMap[day];
            const status = dayStatus(day);
            const colors = getDutyColor(status);
            const d = entry?.d;
            const di = entry?.di;
            const isSelected = selDay===day && sheetVisible;
            const _isToday = isToday(day);

            return (
              <div key={day} onClick={()=>openDay(day)} style={{
                background: isSelected ? colors.bg+"ee" : colors.bg,
                border: isSelected ? `2px solid ${colors.border}` : `2px solid transparent`,
                minHeight:88, padding:"6px 5px", cursor:"pointer",
                ...
                <div style={{
                  ...
                  background:_isToday?C.teal:"none",
                  marginBottom:3,
                }}>
                  <span style={{fontSize:11,fontWeight:_isToday?700:600,color:_isToday?"#fff":colors.text}}>
                    {day}
                  </span>
                </div>

                {/* Duty type indicator */}
                {status==="duty"&&d?.dutyCode&&(
                  <div style={{fontSize:8,fontWeight:700,color:colors.text,letterSpacing:".3px",marginBottom:2}}>
                    {d.dutyCode}
                  </div>
                )}

                {/* Flight legs */}
                {d?.flights?.slice(0,2).map((f,fi)=>{
                  const tk = `${roster.id}-${di}-${fi}`;
                  const tail = tails[tk]||{};
                  const isFlown = tail.actualBlockMins!=null;
                  return (
                    <div key={fi} style={{marginBottom:2}}>
                      <div style={{fontSize:8,fontWeight:700,color:isFlown?C.teal:colors.text,lineHeight:1.2}}>
                        {f.flightNum}
                      </div>
                      <div style={{fontSize:8,color:colors.text,opacity:.85,lineHeight:1.2}}>
                        {f.dep}→{f.arr}
                      </div>
                    </div>
                  );
                ...
                {d?.flights?.length>2&&(
                  <div style={{fontSize:8,color:colors.text,opacity:.6}}>+{d.flights.length-2}</div>
                )}

                {/* Status dot */}
                {status!=="off"&&status!=="empty"&&(
                  <div style={{
      ...
      {/* Bottom sheet — slides up when day selected */}
      <div style={{
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              <div style={{fontSize:15,fontWeight:700,color:C.ink}}>
                {selDay&&new Date(year,monthNum,selDay).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}
              </div>
              {selected?.d?.flights?.length>0&&(
                <div style={{fontSize:11,color:C.muted,marginTop:1}}>
                  {selected.d.flights.length} flight{selected.d.flights.length>1?"s":""}
                  {" · "}{fmtMins(selected.d.flights.reduce((a,f,fi)=>{
        ...
        <div style={{overflowY:"auto",padding:"0 16px 24px",flex:1}}>
          {!selected?.d?.flights?.length && !selected?.d?.dutyCode && (
            <div style={{textAlign:"center",padding:"20px 0",color:C.muted,fontSize:13}}>Day off</div>
          )}
          {selected?.d?.dutyCode && !selected?.d?.flights?.length && (
            <div style={{padding:"12px 16px",borderRadius:10,background:C.gold+"18",border:`1px solid ${C.gold}44`,marginBottom:10}}>
              <div style={{fontSize:13,fontWeight:600,color:C.gold}}>{selected.d.dutyCode}</div>
            </div>
          )}
          ...
          {(selected?.d?.flights||[]).map((f,fi)=>{
            ...
            const dateStr = `${roster.year}-${String(monthNum2+1).padStart(2,"0")}-${String(selDay).padStart(2,"0")}`;
            const solar = computeNightTime(dateStr,f.dep,f.arr,tail.actualDep||f.depTime,tail.actualArr||f.arrTime);
            ...
            return (
              <div key={fi} onClick={()=>{
                if(onOpenFlight) {
                  ...
                  onOpenFlight({rosterId:roster.id,year:roster.year,monthNum:roster.monthNum,
                    selectedFlight:{
                      di:selected.di,fi,f,tail,tk,solar,dist,blockMins,day:day2,
                      hasActual,dep:hasActual?tail.actualDep:f.depTime,
                      arr:hasActual?tail.actualArr:f.arrTime,isXC:(dist||0)>50,
                    }
                  });
                }
              }} style={{
                ...
                background:hasActual?C.teal+"0d":C.panel,
                border:`1px solid ${hasActual?C.teal+"33":C.border}`,
                transition:"all .1s",
              }}>
                {/* Flight number */}
                <div style={{minWidth:64,flexShrink:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:C.teal}}>{f.flightNum}</div>
                  {f.acType&&<div style={{fontSize:10,color:C.muted,marginTop:1}}>{f.acType}</div>}
                </div>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:15,fontWeight:700,color:C.ink}}>
                    <b>{f.dep}</b>
                    <span style={{color:C.muted,fontWeight:400,margin:"0 6px"}}>→</span>
                    <b>{f.arr}</b>
                    {(dist||0)>50&&<span style={{marginLeft:6,fontSize:9,color:C.teal,background:C.teal+"18",padding:"1px 5px",borderRadius:3,fontWeight:700}}>XC</span>}
                  </div>
                  <div style={{fontSize:11,color:hasActual?C.teal:C.muted,marginTop:2}}>
                    {hasActual
                      ? `${tail.actualDep} → ${tail.actualArr} · ${fmtMins(blockMins)}`
                      : `${f.depTime} → ${f.arrTime}${blockMins?" · "+fmtMins(blockMins):" (sched)"}`}
                  </div>
                  {(solar.nightDep||solar.nightArr)&&(
                    <div style={{fontSize:9,color:C.silver,marginTop:1}}>🌙 Night</div>
                  )}
                </div>

                {/* Right side */}
                <div style={{textAlign:"right",flexShrink:0}}>
                  {tail.tail&&<div style={{fontSize:11,fontFamily:"monospace",color:C.silver}}>{tail.tail}</div>}
                  {dist&&<div style={{fontSize:10,color:C.muted,marginTop:1}}>{dist} NM</div>}
                  {tail.finalSynced&&<div style={{fontSize:9,color:C.teal,marginTop:1}}>✓ synced</div>}
                </div>
          ...
          {!adding&&(
            <button onClick={()=>setAdding(true)} style={{
              ...
              color:C.muted,fontSize:13,cursor:"pointer",marginTop:4,
            }}>+ Add flight</button>
          )}

          {/* Add flight form */}
          {adding&&(
            <div style={{marginTop:8,padding:"14px",borderRadius:12,background:C.panel,border:`1px solid ${C.border}`}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div><div className="form-label">Flight #</div><input className="form-input" placeholder="G7 1234" value={form.flightNum} onChange={e=>setForm(p=>({...p,flightNum:e.target.value}))}/></div>
                <div><div className="form-label">Aircraft</div><input className="form-input" placeholder="CRJ7" value={form.acType} onChange={e=>setForm(p=>({...p,acType:e.target.value}))}/></div>
                <div><div className="form-label">Departure</div><input className="form-input" placeholder="ORD" value={form.dep} onChange={e=>setForm(p=>({...p,dep:e.target.value}))}/></div>
                <div><div className="form-label">Arrival</div><input className="form-input" placeholder="STL" value={form.arr} onChange={e=>setForm(p=>({...p,arr:e.target.value}))}/></div>
                <div><div className="form-label">Dep time</div><input className="form-input" type="time" value={form.depTime} onChange={e=>setForm(p=>({...p,depTime:e.target.value}))}/></div>
                <div><div className="form-label">Arr time</div><input className="form-input" type="time" value={form.arrTime} onChange={e=>setForm(p=>({...p,arrTime:e.target.value}))}/></div>
              </div>
              <div style={{display:"flex",gap:8}}>
                <button onClick={saveNewFlight} disabled={saving} className="btn-teal" style={{flex:1,padding:"9px"}}>
                  {saving?<span className="spinner">⟳</span>:"Save"}
                </button>
                <button onClick={()=>setAdding(false)} style={{flex:1,padding:"9px",borderRadius:8,background:"none",border:`1px solid ${C.border}`,color:C.muted,cursor:"pointer",fontSize:13}}>
                  Cancel
                </button>
              </div>
            </div>
      ...
      {sheetVisible&&(
        <div onClick={closeSheet} style={{
          ...
          zIndex:99,background:"rgba(0,0,0,.2)",
        }}/>
      )}
    </div>
  );
