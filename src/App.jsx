          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[["Pilot Flying",true],["Pilot Non-Flying",false]].map(([lbl,val])=>(
              <button key={lbl} onClick={()=>{
                set("pilotFlying", val);
                if(val && form.dep && form.arr && form.depTime && form.arrTime && form.date){
                  const solar = computeSolarTimes(form.dep, form.arr, form.depTime, form.arrTime, form.date);
                  if(solar?.nightArr){ set("nightLdg",(form.nightLdg||0)+1); }
                  else { set("dayLdg",(form.dayLdg||0)+1); }
                }
    ...
    <div style={{flex:1,overflowY:"auto",overscrollBehaviorY:"contain",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{position:"relative",zIndex:2,padding:"0 16px 32px",maxWidth:600,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[["Pilot Flying",true],["Pilot Non-Flying",false]].map(([lbl,val])=>(
                    <button key={lbl} onClick={()=>{
                      setDetailsForm(p=>{
                        const next={...p,pilotFlying:val};
                        if(val && f?.dep && f?.arr && effDepTime && effArrTime && dateStr){
                          const solar=computeSolarTimes(f.dep,f.arr,effDepTime,effArrTime,dateStr);
                          if(solar?.nightArr) next.nightLdg=(p.nightLdg||0)+1;
                          else next.dayLdg=(p.dayLdg||0)+1;
                        }
                        return next;
                      });
        ...
        <div style={{background:`linear-gradient(135deg,#3B82F6,${BLUE})`,borderRadius:18,padding:"16px 18px",color:"#fff"}}>
          ...
          {briefing?(
            <div style={{background:"rgba(255,255,255,0.12)",borderRadius:12,padding:"12px 14px",fontSize:12,lineHeight:1.7,color:"rgba(255,255,255,0.9)",marginBottom:10,maxHeight:160,overflowY:"auto",overscrollBehaviorY:"contain"}}>
              {briefing}
            </div>
          ):(
...
function SettingsPage({user, rosters, tails, isDark, onToggleTheme}) {
  ...
  function download(){
    csvExport(rosters,tails); // triggers its own download internally -- no return value to use
  }
  const [pdfExporting, setPdfExporting] = useState(null); // null | "jeppesen" | "asa"
  async function downloadJeppesenPdf(){
    setPdfExporting("jeppesen");
    try{ await jeppesenExportPdf(rosters,tails); }
    catch(e){ alert(e.message||"Could not generate Jeppesen PDF."); }
    setPdfExporting(null);
  }
  async function downloadAsaPdf(){
    setPdfExporting("asa");
    try{ await asaExportPdf(rosters,tails); }
    catch(e){ alert(e.message||"Could not generate ASA PDF."); }
    setPdfExporting(null);
  }
  ...
  return (
    ...
    <div style={{maxWidth:600,margin:"0 auto",padding:"0 0 40px"}}>
      ...
      <div className="card">
        ...
        <p style={{fontSize:13,color:C.muted,marginBottom:12}}>Download your complete logbook as CSV, or as a PDF replica of the Jeppesen or ASA paper logbook page layout.</p>
        <button className="btn-orange" onClick={download} disabled={rosters.length===0} style={{marginBottom:10}}>↓ Download CSV ({flights.length} flights)</button>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={downloadJeppesenPdf} disabled={rosters.length===0||pdfExporting} style={{flex:1,minWidth:160,padding:"10px 14px",borderRadius:10,border:`1px solid ${C.border}`,background:C.surface,color:C.ink,fontSize:13,fontWeight:600,cursor:(rosters.length===0||pdfExporting)?"not-allowed":"pointer",opacity:pdfExporting&&pdfExporting!=="jeppesen"?0.6:1}}>
            {pdfExporting==="jeppesen"?"Generating...":"↓ Jeppesen PDF"}
          </button>
          <button onClick={downloadAsaPdf} disabled={rosters.length===0||pdfExporting} style={{flex:1,minWidth:160,padding:"10px 14px",borderRadius:10,border:`1px solid ${C.border}`,background:C.surface,color:C.ink,fontSize:13,fontWeight:600,cursor:(rosters.length===0||pdfExporting)?"not-allowed":"pointer",opacity:pdfExporting&&pdfExporting!=="asa"?0.6:1}}>
            {pdfExporting==="asa"?"Generating...":"↓ ASA PDF"}
          </button>
        </div>
      </div>
    </div>
