...
const sb = {
      ...
      const builder = {
        ...
        then(resolve, reject) {
          ...
          return fetch(url, fetchOpts).then(async r => {
            // 204 No Content (typical for DELETE) has no body — don't try to parse JSON
  ...
  auth: {
    ...
    async signUp({ email, password, options:{data:meta}={} }={}) {
      ...
      try { localStorage.setItem("fl_token", d.access_token); localStorage.setItem("fl_user", JSON.stringify(d.user)); } catch{}
    ...
    async signOut() {
      ...
      try { localStorage.removeItem("fl_token"); localStorage.removeItem("fl_user"); } catch{}
    },
    async getUser() {
...
body{background:${C.base};color:${C.ink};font-family:${FB};line-height:1.6;font-size:15px;-webkit-text-size-adjust:100%}
button{cursor:pointer;font-family:${FB};font-size:14px}
input,textarea,select{font-family:${FB};color-scheme:dark;font-size:15px}
...
.day-date{font-family:${FM};font-size:14px;color:${C.teal};min-width:72px}
...
.day-ft{font-family:${FM};font-size:14px;color:${C.muted}}
...
.fr-num{font-family:${FM};font-size:13px;color:${C.teal}}
.fr-apt{font-size:15px;font-weight:600;color:${C.ink}}
.fr-time{font-family:${FM};font-size:13px;color:${C.muted}}
.fr-ac{font-size:13px;color:${C.muted}}
...
// ─────────────────────────────────────────────────────────────────────────────
function AppLandingPage({onLogin, onSignup}) {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        ...
        if(cred?.type==="password") setSavedCred({email:cred.id, password:cred.password});
      } catch {}
    })();
  },[]);
  ...
  async function bioSignIn() {
    ...
    try {
      // db_signIn is defined later — we call onLogin which opens AuthPage
      // pre-filled; for now trigger the credential-based sign-in directly
      const {data,error} = await sb.auth.signInWithPassword({email:savedCred.email, password:savedCred.password});
      if(error||!data?.user) throw new Error("Sign in failed.");
      const {data:profile} = await sb.from("profiles").select("*").eq("id", data.user.id).single();
      window._bioAuthUser = {...data.user,...(profile||{})};
      window.dispatchEvent(new CustomEvent("fl-bio-auth", {detail: window._bioAuthUser}));
    } catch(e) { setBioErr(e.message||"Sign in failed."); }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background:"radial-gradient(ellipse 80% 50% at 50% 0%, #2D8CF015 0%, transparent 70%)",
      }}/>
      <div style={{
        position:"absolute", bottom:-80, left:-80, width:320, height:320,
        borderRadius:"50%", background:"#2D8CF008", pointerEvents:"none",
        border:"1px solid #2D8CF018",
      }}/>
      <div style={{
        position:"absolute", top:120, right:-60, width:200, height:200,
        borderRadius:"50%", background:"#2D8CF005", pointerEvents:"none",
        border:"1px solid #2D8CF010",
      }}/>

      {/* Top — logo */}
      <div style={{paddingTop:64, textAlign:"center", position:"relative"}}>
        <div style={{
          fontSize:11, letterSpacing:"3px", color:"#2D8CF0",
          textTransform:"uppercase", fontWeight:600, marginBottom:20,
          fontFamily:"Inter,sans-serif",
        }}>Pilot Logbook</div>
        <div style={{
          fontSize:48, fontWeight:700, color:"#FFFFFF",
          fontFamily:"Fraunces,serif", letterSpacing:"-1px", lineHeight:1.1,
        }}>
          Flight<span style={{color:"#2D8CF0"}}>Log</span>
        </div>
        <div style={{
          fontSize:14, color:"#5A6680", marginTop:12,
          fontFamily:"Inter,sans-serif", letterSpacing:".3px",
        }}>Your hours, automated.</div>
      </div>

      {/* Middle — feature pills */}
      <div style={{display:"flex", flexDirection:"column", alignItems:"center", gap:12, position:"relative"}}>
        {[
          ["✈","AI reads your PDF roster"],
          ["🔄","Tail numbers sync automatically"],
          ["📊","Analytics & FAR 117 tracking"],
        ].map(([icon,text])=>(
          <div key={text} style={{
      ...
      <div style={{width:"100%", maxWidth:340, position:"relative"}}>
        ...
        {savedCred && (
          <div style={{marginBottom:16}}>
            <div style={{fontSize:11, color:"#5A6680", textAlign:"center", marginBottom:10, letterSpacing:".3px"}}>
              SAVED ACCOUNT
            </div>
            <button onClick={bioSignIn} disabled={bioLoading} style={{
              ...
              {bioLoading ? <span className="spinner">⟳</span> : <>🔒 Sign in as {savedCred.email}</>}
            </button>
            {bioErr && <div style={{fontSize:12, color:"#E05A5A", textAlign:"center", marginTop:8}}>{bioErr}</div>}
            <div style={{textAlign:"center", margin:"12px 0"}}>
              <span style={{fontSize:12, color:"#2A3450"}}>─────  or  ─────</span>
            </div>
          </div>
        )}

        <button onClick={onLogin} style={{
          width:"100%", padding:"17px", borderRadius:14, marginBottom:12,
          background:"#2D8CF0", border:"none",
          color:"#FFFFFF", fontSize:16, fontWeight:600,
          fontFamily:"Inter,sans-serif", cursor:"pointer",
          boxShadow:"0 4px 24px #2D8CF040",
        }}>
          Sign in
        </button>
        ...
        <button onClick={onSignup} style={{
          ...
          color:"#A8B4CC", fontSize:16, fontWeight:500,
          fontFamily:"Inter,sans-serif", cursor:"pointer",
        }}>
          Create account
        </button>

        <div style={{textAlign:"center", marginTop:20, fontSize:11, color:"#2A3450", letterSpacing:".3px"}}>
...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated}) {
  const [sel,setSel]=useState(()=>defaultRosterIndex(rosters));
                  ...
                  return (
                    <div key={fi} className="flight-row-2line" style={isCancelled?{opacity:.65}:undefined}>
                      ...
                      <div className="flight-row-bottom">
                        ...
                        <button className="fr-lookup" onClick={()=>autoLookup(di,fi,f,d.day)} disabled={ls==="loading"||ls==="ratelimit"} title={ls==="ratelimit"?"Daily lookup limit reached — data syncs automatically":"Manual lookup"}>
                          {ls==="loading"?<span className="spinner">⟳</span>:ls==="notfound"?"—":ls==="error"?"✗":ls==="ratelimit"?"⏳":"🔍 Auto"}
                        </button>
...
async function db_loadAircraftTypes() {
  if(!isConfigured()) return {};
  const {data} = await sb.from("aircraft_types").select("type_code,is_multi,is_turbine,is_jet").eq("active",true);
  const map={};
  (data||[]).forEach(r=>{ map[r.type_code]={isMulti:r.is_multi,isTurbine:r.is_turbine,isJet:r.is_jet}; });
  return map;
}
...
function computeAnalytics(rosters, tails, timeRules, flightTimes, aircraftTypes={}) {
  const now = new Date();
  const results = {
  ...
  for(const roster of rosters) {
    (roster.calendar||[]).forEach((day, di) => {
      (day.flights||[]).forEach((f, fi) => {
        ...
        // PIC/SIC from rules + overrides only (no auto-detection possible)
        ...
        // Multi/Single/Turbine: check overrides first, then rule, then
        // auto-detect from the aircraft type DB. This makes the app work
        // correctly for any aircraft without requiring a manual rule to be
        // set for every flight — e.g. a B738 is automatically multi+turbine,
        // a C172 is automatically single, etc.
        const acInfo = aircraftTypes[f.acType?.toUpperCase()] || null;
        // For unknown aircraft types in Part 121 ops, assume multi+turbine
        // since virtually all airline aircraft are multi-engine turbine.
...
function AnalyticsPage({user, rosters, tails}) {
  ...
  const analytics = useMemo(()=>computeAnalytics(rosters, tails, timeRules, flightTimes, aircraftTypes), [rosters, tails, timeRules, flightTimes, aircraftTypes]);
