...
async function getGeminiBriefing(flightInfo, depMetar, arrMetar, depTaf, arrTaf) {
  if(!GEMINI_API_KEY) {
    return "Add VITE_GEMINI_API_KEY to your .env to enable AI briefings.";
  }
  const prompt = `You are a professional aviation dispatcher giving a concise pre-flight briefing to an airline pilot.

Flight: ${flightInfo.flightNum} | ${flightInfo.dep} → ${flightInfo.arr}
Date: ${flightInfo.dateStr} | Dep: ${flightInfo.depTime} | Arr: ${flightInfo.arrTime}
Aircraft: ${flightInfo.acType} | Distance: ${flightInfo.dist} NM

DEPARTURE (${flightInfo.dep}) METAR: ${depMetar?.rawOb || "Not available"}
DEPARTURE TAF: ${depTaf?.rawTAF?.slice(0,300) || "Not available"}

ARRIVAL (${flightInfo.arr}) METAR: ${arrMetar?.rawOb || "Not available"}
ARRIVAL TAF: ${arrTaf?.rawTAF?.slice(0,300) || "Not available"}

Give a short, clear briefing in 4 sections:
1. **Departure** — current conditions, ceiling, visibility, wind, any concerns
2. **Enroute** — general conditions, anything to be aware of
3. **Arrival** — current conditions, forecast, any concerns
4. **Overall** — one sentence summary and go/no-go suggestion

Keep it concise — 150 words max. Use plain language, no jargon overload.`;
  ...
  } catch(e) { return `Briefing error: ${e.message}`; }
}
...
function FlightDetailPage({flight:f, tail, solar, dist, blockMins, day, roster, hasActual, dep, arr, isXC, onBack, onAutoLookup, lkStatus, tmp, onTmpChange, onSaveTail, saving}) {
  ...
  async function loadBriefing() {
    ...
    const text = await getGeminiBriefing({...f, dateStr, dist}, dm, am, dt, at);
    setBriefing(text);
      ...
      <div className="card">
        ...
        {!briefing&&!briefingLoading&&(
          <div style={{fontSize:12,color:C.muted}}>
            ...
            {!GEMINI_API_KEY&&<div style={{marginTop:4,color:C.gold}}>⚠ Add VITE_GEMINI_API_KEY to .env to enable</div>}
          </div>
        )}
      </div>
    </div>
