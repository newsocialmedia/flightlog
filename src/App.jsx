...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  // RosterBuster color scheme
  const DUTY_COLORS = {
    flight: "#0066CC",   // blue — scheduled flight
    flown:  "#00875A",   // green — completed
    duty:   "#FF8B00",   // amber — standby/training
    off:    "transparent",
  };
  ...
  async function saveNewFlight() {
    ...
    try {
      ...
      if(sel){newCal[sel.di]={...newCal[sel.di],flights:[...(newCal[sel.di].flights||[]),nf]};}
      else{const dow=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(year,mNum,selDay).getDay()];newCal.push({day:selDay,dow,isOff:false,flights:[nf]});newCal.sort((a,b)=>a.day-b.day);}
      await db_saveRoster(user.id,{...roster,calendar:newCal});
      onRosterUpdated(roster.id,newCal);
  ...
  async function deleteFlight(fi) {
    if(!sel) return;
    const newCal=[...(roster.calendar||[])];
    newCal[sel.di]={...newCal[sel.di],flights:(newCal[sel.di].flights||[]).filter((_,i)=>i!==fi)};
    try{await db_saveRoster(user.id,{...roster,calendar:newCal});onRosterUpdated(roster.id,newCal);}catch(e){alert(e.message);}
  }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",overflow:"hidden",background:"#F5F6F7",position:"relative"}}>

      {/* Roster tabs */}
      <div style={{background:"#fff",borderBottom:"1px solid #E5E7EB",flexShrink:0}}>
        <div style={{display:"flex",gap:0,overflowX:"auto",padding:"0 12px"}}>
          {rosters.map((r,i)=>(
            <button key={r.id} onClick={()=>{setSelRoster(i);closeSheet();}} style={{
              padding:"10px 14px",border:"none",background:"none",cursor:"pointer",
              fontSize:13,fontWeight:600,whiteSpace:"nowrap",
              color:selRoster===i?"#0066CC":"#6B7280",
              borderBottom:`2px solid ${selRoster===i?"#0066CC":"transparent"}`,
              marginBottom:-1,
      ...
      {/* Month header */}
      <div style={{background:"#fff",padding:"12px 16px 8px",flexShrink:0,borderBottom:"1px solid #E5E7EB"}}>
        <div style={{fontSize:17,fontWeight:700,color:"#111827",marginBottom:2}}>{monthLabel}</div>
        <div style={{display:"flex",gap:16,fontSize:11,color:"#6B7280"}}>
          {[["flight","Flights","#0066CC"],["flown","Completed","#00875A"],["duty","Standby/Duty","#FF8B00"],["off","Days off","#9CA3AF"]].map(([type,label,col])=>{
            const count = type==="off"
              ? daysInMonth - Object.values(dayMap).filter(e=>e.d.flights?.length||e.d.dutyCode).length
              : Object.entries(dayMap).filter(([,e])=>dutyType(parseInt(e.d.day))===type).length;
            return count>0?(
              <span key={type} style={{display:"flex",alignItems:"center",gap:4}}>
                <span style={{width:8,height:8,borderRadius:2,background:col,display:"inline-block"}}/>
                <span><b style={{color:"#111827"}}>{count}</b> {label}</span>
              </span>
            ):null;
          })}
        </div>
      ...
      <div style={{
        ...
        {["SUN","MON","TUE","WED","THU","FRI","SAT"].map(d=>(
          <div key={d} style={{textAlign:"center",fontSize:9,fontWeight:700,color:"#9CA3AF",padding:"5px 0",letterSpacing:".8px"}}>
            {d}
          </div>
        ))}
      </div>
      ...
      {/* Calendar grid */}
      <div style={{flex:1,overflowY:"auto",paddingBottom:sheetOpen?320:0}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:0}}>
          {cells.map((day,idx)=>{
            if(!day) return <div key={`b${idx}`} style={{minHeight:90,background:"#F9FAFB",borderRight:"1px solid #E5E7EB",borderBottom:"1px solid #E5E7EB"}}/>;
            const e = dayMap[day];
            const type = dutyType(day);
            const color = DUTY_COLORS[type];
            const d = e?.d;
            const di = e?.di;
            const _today = isToday(day);
            const selected = selDay===day&&sheetOpen;
            const dayTotal = d?.flights?.reduce((a,f,fi)=>{
            ...
            return (
              <div key={day} onClick={()=>openDay(day)} style={{
                ...
                borderLeft: type!=="off"?`3px solid ${color}`:"3px solid transparent",
                cursor:"pointer",
                padding:"5px 4px 4px",
                ...
                <div style={{
                  ...
                  background:_today?"#0066CC":"transparent",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  marginBottom:3,
                }}>
                  <span style={{fontSize:11,fontWeight:_today?700:500,color:_today?"#fff":type==="off"?"#9CA3AF":"#111827",lineHeight:1}}>
                    {day}
                  </span>
                </div>

                {/* Duty code */}
                {type==="duty"&&d?.dutyCode&&(
                  <div style={{fontSize:8,fontWeight:700,color:"#FF8B00",marginBottom:1,lineHeight:1.2}}>
                    {d.dutyCode}
                  </div>
                )}

                {/* Flight legs — compact RosterBuster style */}
                {d?.flights?.slice(0,3).map((f,fi)=>{
                  const tk=`${roster.id}-${di}-${fi}`;
                  const t=tails[tk]||{};
                  const flown=t.actualBlockMins!=null;
                  return(
                    <div key={fi} style={{marginBottom:1}}>
                      <div style={{fontSize:8,color:flown?"#00875A":"#0066CC",fontWeight:600,lineHeight:1.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                        {f.depTime} {f.dep}-{f.arr}
                      </div>
                    </div>
                  );
                })}
                {d?.flights?.length>3&&<div style={{fontSize:8,color:"#6B7280"}}>+{d.flights.length-3}</div>}

                {/* Total block */}
                {dayTotal>0&&(
                  <div style={{position:"absolute",bottom:3,right:3,fontSize:8,color:type==="flown"?"#00875A":"#0066CC",fontWeight:600}}>
                    {fmtMins(dayTotal)}
                  </div>
                )}
              </div>
      ...
      {/* Backdrop */}
      {sheetOpen&&(
        <div onClick={closeSheet} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.3)",zIndex:99}}/>
      )}
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{padding:"10px 16px 12px",borderBottom:"1px solid #F3F4F6",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontSize:15,fontWeight:700,color:"#111827"}}>
                {selDay&&new Date(year,mNum,selDay).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}
              </div>
              {sel?.d?.flights?.length>0&&(
                <div style={{fontSize:12,color:"#6B7280",marginTop:2}}>
                  {sel.d.flights.length} leg{sel.d.flights.length>1?"s":""} ·{" "}
                  {fmtMins(sel.d.flights.reduce((a,f,fi)=>{
                    const t=tails[`${roster.id}-${sel.di}-${fi}`]||{};
                    return a+(t.actualBlockMins??schedMins(f)??0);
                  },0))} total block
                </div>
              )}
            </div>
            <button onClick={closeSheet} style={{background:"none",border:"none",fontSize:22,color:"#9CA3AF",cursor:"pointer",padding:"0 4px",lineHeight:1}}>×</button>
          </div>
        </div>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"8px 0 24px"}}>
          {!sel?.d?.flights?.length&&!sel?.d?.dutyCode&&(
            <div style={{textAlign:"center",padding:"28px 0",color:"#9CA3AF",fontSize:13}}>Day off</div>
          )}
          {sel?.d?.dutyCode&&!sel?.d?.flights?.length&&(
            <div style={{margin:"8px 16px",padding:"12px 16px",borderRadius:10,background:"#FFFBEB",border:"1px solid #FCD34D"}}>
              <div style={{fontSize:14,fontWeight:600,color:"#B45309"}}>{sel.d.dutyCode}</div>
            </div>
          )}
          ...
          {(sel?.d?.flights||[]).map((f,fi)=>{
            ...
            return(
              <div key={fi} onClick={()=>{
                ...
                {/* Color bar */}
                <div style={{width:3,height:44,borderRadius:2,background:hasActual?"#00875A":"#0066CC",marginRight:12,flexShrink:0}}/>

                {/* Times */}
                <div style={{minWidth:88,flexShrink:0}}>
                  <div style={{fontSize:14,fontWeight:700,color:"#111827"}}>{hasActual?t.actualDep:f.depTime}</div>
                  <div style={{fontSize:11,color:"#6B7280",marginTop:1}}>{hasActual?t.actualArr:f.arrTime}</div>
                </div>

                {/* Flight + route */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                    <span style={{fontSize:13,fontWeight:700,color:"#0066CC"}}>{f.flightNum}</span>
                    {f.acType&&<span style={{fontSize:10,color:"#9CA3AF",background:"#F3F4F6",padding:"1px 5px",borderRadius:3}}>{f.acType}</span>}
                    {(dist||0)>50&&<span style={{fontSize:9,color:"#0066CC",background:"#EFF6FF",padding:"1px 5px",borderRadius:3,fontWeight:700}}>XC</span>}
                  </div>
                  <div style={{fontSize:14,color:"#111827",fontWeight:600}}>
                    <b>{f.dep}</b> <span style={{color:"#9CA3AF",fontWeight:400}}>→</span> <b>{f.arr}</b>
                  </div>
                  {solar.nightMins>0&&<div style={{fontSize:10,color:"#6B7280",marginTop:1}}>🌙 {fmtMins(solar.nightMins)} night</div>}
                </div>

                {/* Right */}
                <div style={{textAlign:"right",flexShrink:0}}>
                  {bMins>0&&<div style={{fontSize:13,fontWeight:700,color:"#111827"}}>{fmtMins(bMins)}</div>}
                  {t.tail&&<div style={{fontSize:10,fontFamily:"monospace",color:"#6B7280",marginTop:1}}>{t.tail}</div>}
                  {t.finalSynced&&<div style={{fontSize:9,color:"#00875A",marginTop:1}}>✓</div>}
                </div>

                <div style={{color:"#D1D5DB",marginLeft:8,fontSize:18,flexShrink:0}}>›</div>
              </div>
            );
          })}
          ...
          {!adding&&(
            <div style={{padding:"12px 16px"}}>
              ...
              }}>+ Add flight to this day</button>
            </div>
          )}

          {adding&&(
            <div style={{margin:"8px 16px",padding:"14px",borderRadius:10,background:"#F9FAFB",border:"1px solid #E5E7EB"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={saveNewFlight} disabled={saving} style={{flex:1,padding:"9px",borderRadius:7,background:"#0066CC",border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  {saving?<span className="spinner">⟳</span>:"Save"}
                </button>
                <button onClick={()=>setAdding(false)} style={{flex:1,padding:"9px",borderRadius:7,background:"none",border:"1px solid #E5E7EB",color:"#6B7280",fontSize:13,cursor:"pointer"}}>
                  Cancel
                </button>
              </div>
            </div>
