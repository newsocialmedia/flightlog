...
async function asaExportPdf(rosters, tails, preBuiltRows) {
  ...
  doc.autoTable({
    ...
    body: dataRows,
    theme: "grid",
    styles: {fontSize:7, cellPadding:2, lineColor:[0,0,0], lineWidth:0.75, valign:"middle", halign:"center"},
    headStyles: {fillColor:[255,255,255], textColor:[0,0,0], fontStyle:"bold", fontSize:6.5},
    columnStyles: colWidths.reduce((acc,w,i)=>{ acc[i]={cellWidth:w}; return acc; }, {}),
    didParseCell: (data) => {
  ...
  async function save(){
    ...
    try{
      ...
      const flight = {
        ...
        ...(form.takeoffs>0?{loggedTakeoffs:form.takeoffs}:{}),
        ...(form.approaches>0?{
    ...
    <div style={{flex:1,overflowY:"auto",overscrollBehaviorY:"contain",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{padding:"16px 16px 80px",maxWidth:560,margin:"0 auto"}}>
        ...
        <div style={SECTION}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[["Takeoffs","takeoffs"],["Day Ldg","dayLdg"],["Night Ldg","nightLdg"],["Approaches","approaches"]].map(([lbl,k])=>{
              function step(delta){
...
function FlightDetailPage({
  ...
  const [detailsForm, setDetailsForm] = useState(()=>({
    ...
    // Defaults to the existing landing count for a flight saved before this
    // field existed (standard assumption: one takeoff per set of landings
    // on a normal single-leg flight), or 0 for a genuinely fresh flight.
    // Once explicitly saved, loggedTakeoffs becomes the real, independent
    // value from here on.
    takeoffs:    f?.loggedTakeoffs ?? ((f?.loggedDayLandings||0)+(f?.loggedNightLandings||0)),
                ...
                <button onClick={async()=>{
                  ...
                  try{
                    ...
                    const fields = {
                      ...
                      loggedTakeoffs:      detailsForm.takeoffs>0 ? detailsForm.takeoffs : undefined,
