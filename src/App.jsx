...
function LogbookPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, pendingFlight, onPendingFlightConsumed}) {
  ...
  async function forceResync(di,fi,f,dayNum){
    ...
    try{
      // Step 1: Delete cache entries directly via PostgREST — custom client
      // delete() can fail silently, so we use fetch() directly here.
      const num = f.flightNum.replace(/[^0-9]/g,"");
      const idents = ["GJS","UAL","G7"].map(p=>p+num);
      for(const ident of idents) {
        try {
          await fetch(
            `${SUPA_URL}/rest/v1/flight_cache?ident=eq.${ident}&flight_date=eq.${dateStr}`,
      ...
      // Step 2: Unlock tail_log — remove actual times so cron re-syncs
      await fetch(
        `${SUPA_URL}/rest/v1/tail_logs?user_id=eq.${user.id}&roster_id=eq.${roster.id}&flight_key=eq.${di}-${fi}`,
        {method:"PATCH", headers:{
          ...
          actual_block_mins: null,
          final_synced: false,
          settle_checked: false,
        })}
      );
      // Step 3: Call FlightAware fresh
