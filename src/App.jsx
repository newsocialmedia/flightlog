...
async function fetchProfile(userId, accessToken) {
  try {
    ...
    const { data, error } = await sb.from("profiles").select("*").eq("id", userId);
...
function SubscriptionWall({user, onSubscribed, onLogout}) {
  ...
  async function checkSubscription() {
    try {
      // Restore token if lost after page reload
      if(!sb.auth._token) {
        try { const t = localStorage.getItem("fl_token"); if(t) sb.auth._token = t; } catch {}
      }
      // sb.from() returns {data, error} — unwrap correctly
      const { data, error } = await sb.from("profiles")
        .select("subscription_status,plan")
        .eq("id", user.id);
      if(error) { console.error("[Wall] DB error:", error); return false; }
      const profile = Array.isArray(data) ? data[0] : data;
      console.log("[Wall] status:", profile?.subscription_status);
