...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    // Handle carry-forward days (e.g. March 1-2 in a February roster).
    // Priority 1: parser explicitly extracted these in roster.carryForwardDays
    // Priority 2: heuristic detection from calendar array structure
    ...
    if(Array.isArray(roster.carryForwardDays) && roster.carryForwardDays.length > 0) {
      // Parser explicitly identified carry-forward days — most reliable
      carryForwardDays = roster.carryForwardDays;
      thisDays = cal.filter(d => d.day >= 1 && d.day <= daysInMonth);
    } else {
      // Heuristic: detect reset-counter (day drops from >20 back to ≤5) or high-numbered days
      ...
      const resetCarry = cal.slice(carryStartIdx); // reset-counter days
      const highCarry  = cal.slice(0, carryStartIdx).filter(d => d.day > daysInMonth); // high-numbered
      carryForwardDays = [
