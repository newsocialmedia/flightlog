...
function CalendarPage({user, rosters, tails, onRosterUpdated, onOpenFlight}) {
  ...
  const RB = {
    ...
    card:     "#2C2C2E",
    sheet:    "#1C1C1E",
    flight:   "#1D4ED8",   // blue — scheduled flight
    flown:    "#15803D",   // green — completed
    standby:  "#B45309",   // amber — standby/duty
    off:      "#3A3A3C",   // dark gray — day off
    today:    "#0A84FF",   // iOS blue — today circle
    text:     "#FFFFFF",
    sub:      "#EBEBF5",
    muted:    "#8E8E93",
    sep:      "#38383A",
  ...
  if(!roster) return (
    <div style={{minHeight:"100vh",background:RB.bg,display:"flex",alignItems:"center",justifyContent:"center",color:RB.muted,fontSize:14}}>
  ...
  function cardColor(type) {
    return {flight:RB.flight,flown:RB.flown,standby:RB.standby,off:RB.off}[type]||RB.off;
  }
  ...
  function closeSheet() { setSheetOpen(false); setTimeout(()=>setSelDay(null),300); }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",overflow:"hidden",background:RB.bg,position:"relative"}}>

      {/* Roster tabs */}
      <div style={{background:RB.card,borderBottom:`1px solid ${RB.sep}`,flexShrink:0,display:"flex",overflowX:"auto"}}>
        {rosters.map((r,i)=>(
          <button key={r.id} onClick={()=>{setSelRoster(i);closeSheet();}} style={{
            padding:"10px 16px",border:"none",background:"none",cursor:"pointer",
            fontSize:13,fontWeight:600,whiteSpace:"nowrap",
            color:selRoster===i?RB.today:RB.muted,
            borderBottom:`2px solid ${selRoster===i?RB.today:"transparent"}`,
      ...
      {/* Month header */}
      <div style={{background:RB.card,padding:"14px 16px 10px",flexShrink:0,borderBottom:`1px solid ${RB.sep}`}}>
        <div style={{fontSize:20,fontWeight:700,color:RB.text,marginBottom:10,letterSpacing:".2px"}}>{monthLabel}</div>
        {/* Legend */}
        <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          {[["flight",RB.flight,"Flight"],["flown",RB.flown,"Completed"],["standby",RB.standby,"Standby"],["off",RB.off,"Day Off"]].map(([type,col,label])=>{
            const count = type==="off"
              ? cells.filter(d=>d&&dutyType(d)==="off").length
              : cells.filter(d=>d&&dutyType(d)===type).length;
            if(!count) return null;
            return (
              <div key={type} style={{display:"flex",alignItems:"center",gap:5}}>
                <div style={{width:10,height:10,borderRadius:3,background:col}}/>
                <span style={{fontSize:11,color:RB.muted}}>{label} <span style={{color:RB.text,fontWeight:600}}>{count}</span></span>
              </div>
            );
          })}
        </div>
      ...
      {/* Day of week labels */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:RB.bg,flexShrink:0,padding:"8px 4px 4px"}}>
        {["S","M","T","W","T","F","S"].map((d,i)=>(
          <div key={i} style={{textAlign:"center",fontSize:12,fontWeight:600,color:RB.muted,letterSpacing:".3px"}}>{d}</div>
        ))}
      </div>

      {/* Calendar grid */}
      <div style={{flex:1,overflowY:"auto",padding:"2px 4px",paddingBottom:sheetOpen?340:8}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
          {cells.map((day,idx)=>{
            if(!day) return <div key={`b${idx}`} style={{borderRadius:10,minHeight:76}}/>;
            const e = dayMap[day];
            const type = dutyType(day);
            const bgColor = cardColor(type);
            const d = e?.d;
            const di = e?.di;
            const _today = isToday(day);
            const selected = selDay===day&&sheetOpen;
            const firstFlight = d?.flights?.[0];
            const tk0 = firstFlight&&di!=null?`${roster.id}-${di}-0`:null;
            const tail0 = tk0?tails[tk0]||{}:{};
            const depTime = tail0?.actualDep||firstFlight?.depTime||"";
            const dayBlock = d?.flights?.reduce((a,f,fi)=>{
            ...
            return (
              <div key={day} onClick={()=>openDay(day)} style={{
                ...
                border:selected?`2px solid ${RB.today}`:"2px solid transparent",
                opacity: type==="off"?0.55:1,
                transition:"opacity .1s,border .1s",
              }}>
                {/* Day number */}
                <div style={{
                  width:20,height:20,borderRadius:"50%",
                  background:_today?RB.today:"transparent",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  marginBottom:4,
                }}>
                  <span style={{fontSize:11,fontWeight:600,color:_today?"#fff":type==="off"?RB.muted:RB.text,lineHeight:1}}>
                    {day}
                  </span>
                </div>

                {/* Flight info */}
                {type!=="off"&&firstFlight&&(
                  <>
                    {depTime&&(
                      <div style={{fontSize:13,fontWeight:700,color:"#fff",lineHeight:1,marginBottom:2}}>
                        {depTime}
                      </div>
                    )}
                    <div style={{fontSize:10,fontWeight:600,color:"rgba(255,255,255,.85)",lineHeight:1.2,marginBottom:1}}>
                      {firstFlight.dep}
                      {d.flights.length>1
                        ? "›"+d.flights[d.flights.length-1].arr
                        : "→"+firstFlight.arr}
                    </div>
                    <div style={{fontSize:9,color:"rgba(255,255,255,.6)",lineHeight:1}}>
                      {d.flights.length>1?`${d.flights.length} legs`:firstFlight.flightNum}
                    </div>
                  </>
                )}

                {/* Standby/duty code */}
                {type==="standby"&&d?.dutyCode&&!firstFlight&&(
                  <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,.9)",marginTop:2}}>
                    {d.dutyCode}
                  </div>
                )}

                {/* Block time bottom-right */}
                {dayBlock>0&&(
                  <div style={{position:"absolute",bottom:5,right:6,fontSize:9,fontWeight:700,color:"rgba(255,255,255,.75)"}}>
                    {fmtMins(dayBlock)}
                  </div>
                )}
                ...
                {/* Synced dot */}
                {type==="flown"&&(
                  <div style={{position:"absolute",top:5,right:5,width:5,height:5,borderRadius:"50%",background:"rgba(255,255,255,.5)"}}/>
                )}
              </div>
            );
          })}
      ...
      {/* Backdrop */}
      {sheetOpen&&(
        <div onClick={closeSheet} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.5)",zIndex:99,transition:"opacity .28s"}}/>
      )}
      ...
      <div style={{
        ...
        <div style={{padding:"12px 0 4px",textAlign:"center",flexShrink:0}}>
          <div style={{width:36,height:4,borderRadius:2,background:RB.sep,display:"inline-block"}}/>
        </div>

        {/* Date header */}
        <div style={{padding:"6px 16px 12px",borderBottom:`1px solid ${RB.sep}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:17,fontWeight:700,color:RB.text}}>
                {selDay&&new Date(year,mNum,selDay).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}
              </div>
              {sel?.d?.flights?.length>0&&(
                <div style={{fontSize:12,color:RB.muted,marginTop:2}}>
            ...
            <button onClick={closeSheet} style={{background:RB.off,border:"none",borderRadius:"50%",width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:RB.sub,fontSize:16,fontWeight:700}}>×</button>
          </div>
        </div>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"8px 0 32px"}}>
          {!sel?.d?.flights?.length&&!sel?.d?.dutyCode&&(
            <div style={{textAlign:"center",padding:"32px 0",color:RB.muted,fontSize:13}}>Rest day</div>
          )}
          {sel?.d?.dutyCode&&!sel?.d?.flights?.length&&(
            <div style={{margin:"12px 16px",padding:"14px 16px",borderRadius:12,background:RB.standby+"33",border:`1px solid ${RB.standby}55`}}>
              <div style={{fontSize:15,fontWeight:700,color:RB.standby}}>{sel.d.dutyCode}</div>
              <div style={{fontSize:12,color:RB.muted,marginTop:2}}>Standby / Duty</div>
            </div>
          )}
          ...
          {(sel?.d?.flights||[]).map((f,fi)=>{
            ...
            return(
              <div key={fi} onClick={()=>{
                ...
                padding:"14px 16px",
                borderBottom:`1px solid ${RB.sep}`,
                cursor:"pointer",
                transition:"background .1s",
              }}>
                {/* Left color bar */}
                <div style={{width:4,height:52,borderRadius:2,background:synced?RB.flown:RB.flight,marginRight:14,flexShrink:0}}/>

                {/* Times column */}
                <div style={{minWidth:52,flexShrink:0,marginRight:12}}>
                  <div style={{fontSize:15,fontWeight:700,color:RB.text,lineHeight:1}}>
                    {hasActual?t.actualDep:f.depTime}
                  </div>
                  <div style={{width:1,height:14,background:RB.sep,margin:"3px 0 3px 7px"}}/>
                  <div style={{fontSize:13,fontWeight:500,color:RB.muted,lineHeight:1}}>
                    {hasActual?t.actualArr:f.arrTime}
                  </div>
                </div>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                    <span style={{fontSize:13,fontWeight:700,color:RB.today}}>{f.flightNum}</span>
                    {f.acType&&<span style={{fontSize:10,color:RB.muted,background:RB.badge,padding:"2px 6px",borderRadius:4}}>{f.acType}</span>}
                    {(dist||0)>50&&<span style={{fontSize:10,color:RB.today,background:RB.today+"22",padding:"2px 6px",borderRadius:4,fontWeight:700}}>XC</span>}
                    {solar.nightMins>0&&<span style={{fontSize:10,color:"#A78BFA",background:"#A78BFA22",padding:"2px 6px",borderRadius:4}}>🌙</span>}
                  </div>
                  <div style={{fontSize:16,fontWeight:700,color:RB.text}}>
                    {f.dep} <span style={{color:RB.muted,fontWeight:300}}>→</span> {f.arr}
                  </div>
                  {t.tail&&<div style={{fontSize:11,fontFamily:"monospace",color:RB.muted,marginTop:3}}>{t.tail}{synced&&<span style={{color:RB.flown,marginLeft:4}}>✓</span>}</div>}
                </div>

                {/* Right */}
                <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                  {bMins>0&&<div style={{fontSize:14,fontWeight:700,color:RB.text}}>{fmtMins(bMins)}</div>}
                  {dist&&<div style={{fontSize:10,color:RB.muted,marginTop:2}}>{dist} NM</div>}
                </div>

                <div style={{color:RB.muted,marginLeft:10,fontSize:20,flexShrink:0,lineHeight:1}}>›</div>
              </div>
            );
          })}
          ...
          {!adding?(
            <div style={{padding:"12px 16px"}}>
              <button onClick={()=>setAdding(true)} style={{width:"100%",padding:"12px",borderRadius:10,background:"none",border:`1px dashed ${RB.sep}`,color:RB.muted,fontSize:13,cursor:"pointer"}}>
                + Add flight to this day
              </button>
            </div>
          ):(
            <div style={{margin:"8px 16px",padding:"14px",borderRadius:12,background:RB.bg,border:`1px solid ${RB.sep}`}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={saveNewFlight} disabled={saving} style={{flex:1,padding:"10px",borderRadius:8,background:RB.today,border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  {saving?<span className="spinner">⟳</span>:"Save"}
                </button>
                <button onClick={()=>setAdding(false)} style={{flex:1,padding:"10px",borderRadius:8,background:"none",border:`1px solid ${RB.sep}`,color:RB.muted,fontSize:13,cursor:"pointer"}}>Cancel</button>
              </div>
            </div>
          )}
        </div>
