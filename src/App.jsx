    ...
    <div className="lp-root">
      ...
      <nav className="lp-nav">
        ...
        <div className="lp-nav-actions">
          ...
          <button className="lp-nav-cta" onClick={onSignup}>Get started →</button>
        </div>
      </nav>
      ...
      <section className="lp-hero">
        ...
        <div className="lp-badge"><div className="lp-badge-dot"/>Built for airline pilots</div>
        <h1 className="lp-headline">Your logbook,<br/><em>on autopilot.</em></h1>
        <p className="lp-sub">Upload your PDF roster once. FlightLog parses every leg, syncs actual block times and tail numbers via FlightAware, and keeps your hours current — automatically.</p>
        <div className="lp-hero-btns">
          <button className="btn-primary" onClick={onSignup}>Start your logbook →</button>
        ...
        <div style={{display:"flex",gap:40,marginTop:24,flexWrap:"wrap",justifyContent:"center"}}>
          {[
            ["✈","Any airline roster format"],
            ["⚡","Live FlightAware sync"],
            ["📋","Jeppesen & ASA export"],
          ].map(([icon,label])=>(
      ...
      <div className="lp-stats">
        {[
          ["99.2%","Roster parse accuracy"],
          ["< 15 min","Auto-sync after landing"],
          ["40+","Airlines supported"],
          ["Jeppesen","& ASA export ready"],
        ].map(([n,l])=>(
      ...
      <section id="features"><div className="lp-section">
        ...
        <h2 className="lp-section-title">Everything pilots actually need</h2>
        <p className="lp-section-sub">No manual entry. No spreadsheets. One PDF upload and FlightLog handles the rest.</p>
        <div className="features-grid">
          {[
            {icon:"📄",bg:"#EFF6FF",title:"Smart Roster Parsing",desc:"Reads any airline PDF roster — GoJet, United, Delta, American, and more. Extracts every leg, layover, and duty code instantly."},
            {icon:"✈",bg:"#F0FDF4",title:"Live FlightAware Sync",desc:"Automatically syncs actual tail numbers and block times from FlightAware within 15 minutes of landing. Zero manual input."},
            {icon:"📊",bg:"#FFF7ED",title:"Currency Tracking",desc:"Tracks FAR 61.57 landing and IFR currency, FAR 117 duty limits, and total hours — always up to date."},
            {icon:"📋",bg:"#F5F3FF",title:"Jeppesen & ASA Export",desc:"Export your logbook in formats that match the physical Jeppesen and ASA logbook columns exactly. Ready to import anywhere."},
            {icon:"🗺️",bg:"#EFF6FF",title:"Route Maps & Radar",desc:"Interactive route maps with live weather radar overlay for upcoming flights within 24 hours of departure."},
            {icon:"🔒",bg:"#FFF1F2",title:"Secure & Private",desc:"Your data is yours. Each pilot sees only their own logbook. Bank-grade encryption via Supabase and Stripe."},
      ...
      {/* How it works */}
      <section id="how" style={{background:"#f8f9fc",borderTop:"1px solid #E2E8F0"}}><div className="lp-section">
        <div className="lp-section-eyebrow">How it works</div>
        <h2 className="lp-section-title">Up and running in 3 steps</h2>
        <div className="how-grid">
          {[
            {icon:"📤",num:"01",title:"Upload your roster",desc:"Drop your monthly PDF roster. FlightLog reads any airline format and extracts every flight leg, duty code, and layover automatically."},
            {icon:"⚡",num:"02",title:"Flights sync automatically",desc:"Within 15 minutes of each landing, FlightLog pulls the actual tail number and block time from FlightAware — no action needed."},
            {icon:"📋",num:"03",title:"Export & stay current",desc:"Download in Jeppesen or ASA format, review your totals, track currency. Your logbook is always current and ready for a checkride."},
      ...
      {/* Pricing */}
      <section id="pricing" style={{borderTop:"1px solid #E2E8F0"}}><div className="lp-section">
        <div className="lp-section-eyebrow">Pricing</div>
        <h2 className="lp-section-title">One plan, full access</h2>
        <p className="lp-section-sub">Everything included. No feature tiers, no hidden limits.</p>
        <div className="pricing-grid">
          <div className="price-card">
            <div className="price-plan">Monthly</div>
            <div><span className="price-amount">$11</span><span className="price-period">/mo</span></div>
            <div className="price-desc">Full access, cancel anytime.</div>
            <ul className="price-features">
              <li>Unlimited roster uploads</li>
              <li>Live FlightAware sync</li>
              <li>Jeppesen & ASA export</li>
              <li>Currency tracking</li>
              <li>Route maps & weather</li>
            </ul>
            <button className="price-cta price-cta-ghost" onClick={onSignup}>Get started</button>
          </div>
          <div className="price-card featured">
            <div className="price-badge">BEST VALUE</div>
            <div className="price-plan">Annual</div>
            <div><span className="price-amount">$99</span><span className="price-period">/yr</span></div>
            <div className="price-desc">Save 25% — just $8.25/month.</div>
            <ul className="price-features">
              <li>Everything in monthly</li>
              <li>2 months free</li>
              <li>Priority support</li>
              <li>Early access to new features</li>
              <li>Locked-in rate guaranteed</li>
            </ul>
            <button className="price-cta price-cta-primary" onClick={onSignup}>Start annual plan</button>
          </div>
        </div>
        <p style={{fontSize:12,color:"#94A3B8",marginTop:16,textAlign:"center"}}>Secured by Stripe · Cancel anytime · No contracts</p>
      </div></section>
      ...
      <footer className="lp-footer">
        ...
        <div style={{display:"flex",gap:24}}>
          {["Privacy","Terms","Support"].map(l=><span key={l} style={{fontSize:13,color:"#64748B",cursor:"pointer"}}>{l}</span>)}
        </div>
...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  useEffect(()=>{
    ...
    setWxLoading(true);
    const icao = depAirport.length===3 ? "K"+depAirport : depAirport;
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:C.base,overflowY:"auto"}}>
      ...
      <div style={{padding:"20px 20px 16px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{width:40,height:40,borderRadius:"50%",background:`linear-gradient(135deg,${C.teal},${C.tealDim})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:15,fontWeight:700,flexShrink:0}}>
              {firstName[0]}
            </div>
          </div>
        </div>
      ...
      <div style={{flex:1,padding:"0 16px 32px"}}>
        ...
        {nextFlight&&(
          <div style={{marginBottom:14,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 16px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:miniBriefing?12:0}}>
              <div>
                <div style={{fontSize:13,fontWeight:700,color:C.ink}}>Flight Briefing</div>
                <div style={{fontSize:11,color:C.muted}}>{nextFlight.f.flightNum} &nbsp;{nextFlight.f.dep}→{nextFlight.f.arr}</div>
              </div>
              <button onClick={fetchMiniBriefing} disabled={briefingLoading} style={{
                ...
                {briefingLoading?<span className="spinner">loading</span>:miniBriefing?"Refresh":"Get Briefing"}
              </button>
            </div>
            {miniBriefing&&(
            ...
            {!miniBriefing&&!briefingLoading&&(
              <div style={{fontSize:12,color:C.muted,marginTop:8}}>
                Tap "Get Briefing" for weather, NOTAMs, and route info for your flight
              </div>
            )}
          </div>
        )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          {[
            {label:"Upload Roster",icon:"📤",page:"upload",primary:true},
            {label:"Logbook",icon:"📋",page:"logbook",primary:false},
            {label:"Roster View",icon:"📅",page:"calendar",primary:false},
            {label:"Export",icon:"⬇",page:"export",primary:false},
          ].map(({label,icon,page,primary})=>(
  ...
  return(
    <div style={{flex:1,overflowY:"auto",background:C.base,padding:"0 0 32px"}}>
      ...
      <div style={{padding:16}}>
        {/* Tabs */}
        <div style={{display:"flex",gap:0,marginBottom:16,background:C.panel,borderRadius:10,padding:3}}>
          {[["plan","My Plan"],["referral","Refer & Earn"]].map(([id,label])=>(
            <button key={id} onClick={()=>{setTab(id);if(id==="referral")loadReferral();}} style={{
              flex:1,padding:"8px",borderRadius:8,border:"none",cursor:"pointer",
              background:tab===id?C.surface:"transparent",color:tab===id?C.ink:C.muted,
        ...
        {tab==="plan"&&(<>
          ...
          <div className="card" style={{marginBottom:14}}>
            ...
            {["PDF roster parsing - any airline","Auto tail number & block time sync","FAR 61.57 & 117 currency tracking","Jeppesen & ASA export","Route maps with live radar","Flight briefings","Unlimited roster history"].map(f=>(
              <div key={f} style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
                <span style={{color:C.teal,fontWeight:800,fontSize:12}}>checkmark</span>
          ...
          {!isActive&&!isPastDue&&(<>
            ...
            <div style={{marginBottom:10,padding:20,borderRadius:14,background:`linear-gradient(135deg,${C.teal},${C.tealDim})`,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:-15,right:-15,width:80,height:80,borderRadius:"50%",background:"rgba(255,255,255,0.08)"}}/>
              <div style={{position:"absolute",top:-8,right:12,background:"#fff",color:C.teal,fontSize:9,fontWeight:800,padding:"3px 10px",borderRadius:100,letterSpacing:"1px"}}>BEST VALUE</div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div>
                  <div style={{fontSize:16,fontWeight:700,color:"#fff"}}>Annual Plan</div>
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.75)"}}>Save 41% vs monthly</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:28,fontWeight:900,color:"#fff",letterSpacing:"-1px"}}>$99</div>
                  <div style={{fontSize:11,color:"rgba(255,255,255,0.7)"}}>$8.25/mo</div>
                </div>
              </div>
              <button onClick={()=>subscribe("year")} disabled={!!loading} style={{width:"100%",padding:"13px",borderRadius:10,background:"#fff",border:"none",color:C.teal,fontSize:14,fontWeight:700,cursor:"pointer"}}>
                {loading==="annual"?<span className="spinner">loading</span>:"Subscribe - $99/year"}
              </button>
            ...
            <div className="card" style={{marginBottom:14,opacity:0.85}}>
              ...
              <button onClick={()=>subscribe("month")} disabled={!!loading} style={{width:"100%",padding:"11px",borderRadius:10,background:"none",border:`1.5px solid ${C.border}`,color:C.silver,fontSize:13,fontWeight:600,cursor:"pointer"}}>
                {loading==="monthly"?<span className="spinner">loading</span>:"Subscribe Monthly - $14/mo"}
              </button>
            </div>
        ...
        {/* REFERRAL TAB */}
        {tab==="referral"&&(<>
          {refLoading&&<div style={{textAlign:"center",padding:32,color:C.muted}}><span className="spinner">loading</span></div>}
          {referralData&&(<>
            <div className="card" style={{marginBottom:14,background:`linear-gradient(135deg,${C.teal}12,${C.teal}06)`,border:`1px solid ${C.teal}33`}}>
              <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:12}}>How it works</div>
              {[["1","Share your code with a fellow pilot"],["2","They subscribe to FlightLog"],["3","You get $11 credited to your next bill"]].map(([n,text])=>(
                <div key={n} style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                  <div style={{width:26,height:26,borderRadius:"50%",background:C.teal,color:"#fff",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{n}</div>
                  <div style={{fontSize:13,color:C.ink}}>{text}</div>
                </div>
              ))}
            </div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>
              {[{label:"Referred",val:referralData.referrals,icon:"Pilots"},{label:"Subscribed",val:referralData.converted,icon:"Paid"},{label:"Credits",val:`$${(referralData.credits||0)*11}`,icon:"Earned"}].map(s=>(
                <div key={s.label} className="card" style={{textAlign:"center",padding:"14px 8px"}}>
                  <div style={{fontSize:11,color:C.muted,marginBottom:4}}>{s.icon}</div>
                  <div style={{fontSize:22,fontWeight:800,color:C.teal}}>{s.val}</div>
                  <div style={{fontSize:10,color:C.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:".5px"}}>{s.label}</div>
                </div>
              ))}
            </div>

            <div className="card" style={{marginBottom:14}}>
              <div style={{fontSize:12,color:C.muted,fontWeight:600,marginBottom:8}}>YOUR REFERRAL CODE</div>
              <div style={{padding:"16px",borderRadius:10,background:C.panel,border:`1px solid ${C.border}`,textAlign:"center",marginBottom:12}}>
                <div style={{fontSize:28,fontWeight:900,color:C.teal,letterSpacing:"4px",fontFamily:"monospace"}}>{referralData.code}</div>
              </div>
              <div style={{display:"flex",gap:8}}>
                <button onClick={copyLink} style={{flex:1,padding:"11px",borderRadius:10,background:C.panel,border:`1px solid ${C.border}`,color:C.ink,fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  {copied?"Copied!":"Copy link"}
                </button>
                <button onClick={shareMsg} style={{flex:1,padding:"11px",borderRadius:10,background:C.teal,border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  Share
                </button>
              </div>
            </div>
            ...
            {referralData.referralList?.length>0&&(
              <div className="card">
                <div style={{fontSize:13,fontWeight:700,color:C.ink,marginBottom:10}}>Your referrals</div>
                {referralData.referralList.map((r,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<referralData.referralList.length-1?`1px solid ${C.border}`:"none"}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:C.ink}}>{r.name}</div>
                      <div style={{fontSize:11,color:C.muted}}>{r.joined?.slice(0,10)}</div>
                    </div>
                    <span className={`pill ${r.converted?"pill-green":"pill-muted"}`}>{r.converted?"Subscribed":"Pending"}</span>
                  </div>
                ))}
              </div>
            )}
            {(!referralData.referralList||referralData.referralList.length===0)&&(
              <div className="card" style={{textAlign:"center",padding:"32px 16px"}}>
                <div style={{fontSize:32,marginBottom:8}}>✈</div>
                <div style={{fontSize:14,fontWeight:600,color:C.ink,marginBottom:4}}>No referrals yet</div>
                <div style={{fontSize:12,color:C.muted}}>Share your code with pilots in your crew room or aviation communities</div>
              </div>
            )}
          </>)}
        </>)}
...
function ExportPage({rosters, tails}) {
  return (
    <div style={{flex:1,overflowY:"auto",background:C.base,padding:"24px 16px"}}>
      ...
      <div style={{padding:16}}>
      {[
        {fmt:"CSV Universal",desc:"All fields — works with Excel, Google Sheets",icon:"📊",action:()=>csvExport(rosters,tails)},
        {fmt:"Jeppesen CSV",desc:"Matches Jeppesen Professional Pilot Logbook columns",icon:"📘",action:()=>jeppesenExport&&jeppesenExport(rosters,tails)||alert("Jeppesen export — see logbook export page")},
        {fmt:"ASA Standard",desc:"Matches ASA-SP-30 logbook columns",icon:"📗",action:()=>alert("ASA export — see logbook export page")},
      ].map(({fmt,desc,icon,action})=>(
        <div key={fmt} className="card" style={{marginBottom:10,display:"flex",alignItems:"center",gap:14}}>
          <div style={{fontSize:28}}>{icon}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:14,fontWeight:700,color:C.ink}}>{fmt}</div>
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>{desc}</div>
          </div>
          <button onClick={action} style={{padding:"8px 16px",borderRadius:8,background:C.teal,border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>Export</button>
        </div>
      ))}
      </div>
    </div>
...
function MorePage({user, setPage, onLogout, isDark, onToggleTheme, rosters, tails}) {
  ...
  const pilotItems = [
    ...
    {id:"membership", icon:"card",  label:"Membership"},
    {id:"support",    icon:"help",  label:"Support"},
    {id:"export",     icon:"dl",    label:"Export Logbook"},
  ];
  const adminItems = [
    {id:"admin-overview",  icon:"grid",  label:"Admin Overview"},
    {id:"admin-users",     icon:"users", label:"User Management"},
    {id:"admin-analysis",  icon:"chart", label:"Analysis"},
    {id:"admin-rosters",   icon:"docs",  label:"All Rosters"},
    {id:"admin-settings",  icon:"gear",  label:"Admin Settings"},
    {id:"profile",         icon:"user",  label:"Profile"},
  ];
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:C.base}}>
      ...
      <div style={{background:C.surface,borderRadius:12,margin:"0 16px",overflow:"hidden",border:`1px solid ${C.border}`}}>
        {items.map((item,i)=>(
          <button key={item.id} onClick={()=>setPage(item.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:14,padding:"14px 16px",border:"none",background:"none",cursor:"pointer",borderBottom:i<items.length-1?`1px solid ${C.border}`:"none",textAlign:"left"}}>
            <div style={{width:34,height:34,borderRadius:8,background:C.panel,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
              {item.icon==="chart"?"📊":item.icon==="map"?"🗺️":item.icon==="user"?"👤":item.icon==="gear"?"⚙️":item.icon==="card"?"💳":item.icon==="help"?"🎧":item.icon==="dl"?"📤":item.icon==="grid"?"◈":item.icon==="users"?"👥":item.icon==="docs"?"📋":"⚙️"}
            </div>
      ...
      <div style={{padding:"16px 20px",fontSize:11,color:C.muted,textAlign:"center"}}>FlightLog · {user?.email}</div>
    </div>
  );
}
...
export default function App() {
  ...
  async function handleAuth(u){
    ...
    if(params.get("checkout")==="success"){
      try{const profile=await fetchProfile(u.id,sb.auth._token);if(profile?.id)u={...u,...profile};setUser(u);}catch{}
      window.history.replaceState({},"",window.location.pathname);
    }
    if(u.role!=="admin"){
      const hasAccess=u.subscription_status==="active"||u.subscription_status==="past_due";
      if(!hasAccess){setPage("membership");setScreen("app");return;}
    }
