...
function LogbookHubPage({user, rosters, tails, onTailSaved, onDeleteRoster, onRosterUpdated, onOpenFlight, pendingFlight, onPendingFlightConsumed, setPage, initialTab}) {
  const S = getS();
  const [hubTab, setHubTab] = useState(initialTab || "daily"); // "daily" | "logbookview" | "roster" | "activelogs"
  ...
  const TABS = [
    {id:"daily",       label:"Daily View",  icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="17" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M3 9h18M8 2v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"logbookview", label:"Logbook View",icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="4" y="2" width="13" height="18" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M8 7h6M8 11h4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"roster",      label:"Roster View", icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="17" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M8 2V6M16 2V6M3 9H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
    {id:"activelogs",  label:"Active Logs", icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M9 12h6M9 16h6M9 8h6M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>, badge:unsignedCount>0?unsignedCount:null},
  ];
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      {/* 4-tab switcher -- matches Quick Actions card visual language */}
            ...
            return(
              <button
                ...
                style={{
                  ...
                  boxShadow:isActive?`0 4px 12px ${S.blue}30`:"0 1px 3px rgba(0,0,0,0.04)",
                  position:"relative",
                }}
              >
                ...
                {label}
                {badge!=null&&(
                  <span style={{position:"absolute",top:-4,right:-4,minWidth:16,height:16,borderRadius:100,background:"#F59E0B",color:"#fff",fontSize:9,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px",border:`2px solid ${S.bg}`}}>{badge}</span>
                )}
              </button>
            );
          })}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.base}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 16px"}}>
        {!roster?(
          ...
          <>
            {(roster.calendar||[]).filter(d=>(d.flights||[]).length>0).sort((a,b)=>b.day-a.day).map((day,di_rel)=>{
              const di = (roster.calendar||[]).findIndex(d=>d.day===day.day);
              const sortedFlights = [...(day.flights||[])].map((f,fi)=>({f,fi})).sort((a,b)=>(b.f.depTime||"00:00").localeCompare(a.f.depTime||"00:00"));
...
export default function App() {
  ...
  const [logbookInitialTab, setLogbookInitialTab] = useState("daily");
