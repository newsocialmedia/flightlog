...
function UploadPage({user, rosters, onRosterSaved, onReloadRosters, setPage}) {
  ...
  function parseNetlineFromItems(items) {
    ...
    const FLIGHT_SRC = '(DH\\s*\\/?\\s*)?([A-Z][A-Z0-9])\\s*(\\d{2,4})(?:\\s*\\/\\s*\\d+)?\\s*([A-Z]{3})\\s*!?\\s*(\\d{4})\\s*!?\\s*(\\d{4})\\s*([A-Z]{3})(?:\\s*(CR[0-9J]|E[0-9]{2}|[A-Z][0-9]{2}[A-Z]?))?';
    ...
    for(const pg of pages){
      ...
      for(const it of pi){
        ...
        while((m = DAY_ANY.exec(it.str)) !== null){
          hits.push({x: it.x + m.index*charPitch(it), y: it.y, day: parseInt(m[2]), rest: it.str.slice(m.index + m[0].length)});
        }
      }
      ...
      if(stripArr.length >= 10){
        ...
        for(const h of strip){
          ...
          if(near.length){
            ...
            const tgt = inCarry ? carryMarker : dayMarker;
            if(tgt[h.day]===undefined) tgt[h.day]=mk;
          }
        }
        for(const h of hits) if(Math.abs(h.y-stripY)>10) inline.push({pg, x:h.x, y:h.y, day:h.day, rest:h.rest});
      } else {
        for(const h of hits) inline.push({pg, x:h.x, y:h.y, day:h.day, rest:h.rest});
      }
    }
    ...
    for(const pg of pages){
      ...
      margins.forEach((mg, bi)=>{
        ...
        blk.forEach((l, k)=>{
          if(!seenReset && prevSeq>=20 && l.day<prevSeq && l.day<=14) seenReset = true;
          prevSeq = l.day;
          ...
          const bucket = seenReset ? carryText : dayText;
    ...
    Object.keys(flightsByDay).map(Number).sort((a,b)=>a-b).forEach(d=>{
      ...
      if(go.length){
        ...
        const target = d + 1;
        if(target <= dim){
          flightsByDay[target] = flightsByDay[target] || [];
          const ex = new Set(flightsByDay[target].map(f=>f.flightNum+f.dep+f.depTime));
          for(const f of go) if(!ex.has(f.flightNum+f.dep+f.depTime)) flightsByDay[target].push(f);
    ...
    // ── Carryover days (strip reset + relocation overflow) ───────────────────
    const carryForwardDays = [...overflowCarry];
    for(const d of Object.keys(carryMarker).map(Number)){
      const mk = carryMarker[d];
      const fl = extractFlights(carryText[d]);
      if(mk==='FlD' && fl.length===0)
        return {_fail:`carryover duty day ${d} produced zero flights`};
      const nd = new Date(year, monthNum+1, d);
      carryForwardDays.push({day:d, dow:DOW[nd.getDay()], isOff:false, dutyCode:mk==='FlD'?null:mk, flights:fl});
    }
