...
.auth-logo-img{display:block;max-width:220px;width:70%;height:auto;margin:0 auto 12px}
  ...
  for(const r of rosters){
    ...
    (r.calendar||[]).forEach((d,di)=>{
      (d.flights||[]).forEach((f,fi)=>{
        ...
        if(flightDate>=cutoff30)  { last30.mins+=mins;  last30.legs+=1;  last30.night+=nightMins;  last30.xc+=xcMins;  last30.landings+=ldgs; }
        if(flightDate>=cutoff6mo) { last6mo.mins+=mins; last6mo.legs+=1; last6mo.night+=nightMins; last6mo.xc+=xcMins; last6mo.landings+=ldgs; }
        if(flightDate>=cutoff12mo){ last12mo.mins+=mins;last12mo.legs+=1;last12mo.night+=nightMins;last12mo.xc+=xcMins;last12mo.landings+=ldgs; }
      });
    });
  }
  return{
...
function LandingPage({onLogin, onSignup}) {
  ...
  return(
    <div style={{background:S.bg,minHeight:"100vh",fontFamily:"Inter,system-ui,sans-serif",color:S.ink,overflowX:"hidden",position:"relative"}}>
      ...
      {/* NAV -- ported exactly from provided design source: dark #0b1625 page
          background, floating pill-shaped glass nav (bg-white/5 + blur),
          blue square logo mark, plain white CTA pill. */}
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,padding:"24px 16px 0"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",padding:12,borderRadius:100,background:"rgba(255,255,255,0.05)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,0.1)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"0 16px"}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#3B82F6",flexShrink:0}}/>
            <span style={{fontWeight:700,fontSize:18,color:"#fff"}}>AVIATESYNC</span>
          </div>
          <div style={{display:"flex",gap:24,fontSize:14,color:"#D1D5DB"}}>
            ...
            <button onClick={onLogin} style={{background:"none",border:"none",color:"#D1D5DB",cursor:"pointer",padding:0,fontSize:14}}>Log In</button>
          </div>
          <button onClick={onSignup} style={{background:"#fff",color:"#000",padding:"8px 24px",borderRadius:100,fontWeight:700,fontSize:14,border:"none",cursor:"pointer",whiteSpace:"nowrap"}}>Get Started</button>
        </div>
      </nav>

      {/* HERO -- exact port: #0b1625 background, 7xl bold two-line headline,
          gray-400 subtext, two store-badge placeholders, blurred blue glow
          behind a plain rounded phone silhouette on the right. */}
      <section style={{background:"#0b1625",paddingTop:140,paddingBottom:80}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 16px",display:"flex",alignItems:"center",flexWrap:"wrap",gap:40}}>
          <div style={{flex:"1 1 480px",minWidth:0}}>
            <h1 style={{fontSize:"clamp(40px,6vw,72px)",fontWeight:700,lineHeight:1.1,marginBottom:24,color:"#fff"}}>
              The Pilot's Logbook.<br/>Automated.
            </h1>
            <p style={{fontSize:20,color:"#9CA3AF",marginBottom:32,maxWidth:512,lineHeight:1.5}}>
              Securely sync your airline roster and log every flight in seconds with industry-leading accuracy.
            </p>
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
              <a href="#" onClick={e=>e.preventDefault()} style={{height:56,width:160,background:"#000",borderRadius:12,border:"1px solid #374151",display:"flex",alignItems:"center",justifyContent:"center",gap:8,textDecoration:"none"}}>
          ...
          <div style={{flex:"1 1 320px",display:"flex",justifyContent:"center",minWidth:0}}>
            <div style={{position:"relative"}}>
              <div style={{position:"absolute",inset:0,background:"#3B82F6",filter:"blur(64px)",opacity:0.3}}/>
              <div style={{position:"relative",width:288,height:600,background:"#000",borderRadius:48,border:"8px solid #1F2937",boxShadow:"0 25px 50px rgba(0,0,0,0.5)"}}/>
            </div>
          </div>
        </div>
      </section>
...
function AddFlightPage({user, rosters, onRosterSaved, setPage}) {
  ...
  const [form, setForm] = useState({
    ...
    actualIfr:"", simIfr:"", dayLdg:0, nightLdg:0, approaches:0, approachTypes:[],
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
        ...
        <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"10px 14px",display:"flex",gap:10}}>
          <button
            ...
            disabled={saving}
            style={{flex:1,padding:"13px 8px",borderRadius:14,background:"none",border:"1.5px solid #FECACA",color:"#DC2626",fontSize:13,fontWeight:700,cursor:saving?"not-allowed":"pointer",opacity:saving?0.6:1}}
          >
            Discard
          </button>
          <button
...
function ImportLogbookPreview({preview, rosters, user, onReloadRosters, setImportPreview, setStatus, setMsg}) {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"#F8FAFC"}}>
      <div style={{flexShrink:0,background:"linear-gradient(135deg,#7C3AED,#4F46E5)",padding:"18px 16px 14px"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 16px"}}>
        {(preview.warnings.length>0 || preview.unmapped.length>0 || preview.skipped.length>0) && (
          <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"10px 12px",marginBottom:12}}>
        ...
        {months.map(m=>(
          <div key={m.key} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",borderRadius:10,background:m.exists?"#FEF2F2":S.surface,border:`1px solid ${m.exists?"#FECACA":S.border}`,marginBottom:6}}>
            <div>
            ...
            {m.exists
              ? <span style={{fontSize:10,fontWeight:800,color:"#DC2626",background:"#FEE2E2",padding:"3px 8px",borderRadius:99}}>SKIPPED — roster exists</span>
              : <span style={{fontSize:10,fontWeight:800,color:"#059669",background:"#D1FAE5",padding:"3px 8px",borderRadius:99}}>NEW</span>}
          </div>
        ))}
      ...
      <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>
        <button onClick={confirmImport} disabled={saving||!importable.length}
          style={{width:"100%",padding:"14px",borderRadius:14,background:saving?"#C4B5FD":!importable.length?"#E2E8F0":"linear-gradient(135deg,#7C3AED,#4F46E5)",border:"none",color:!importable.length&&!saving?"#94A3B8":"#fff",fontSize:14,fontWeight:800,cursor:saving||!importable.length?"not-allowed":"pointer"}}>
        ...
        <button onClick={()=>{ if(saving) return; setImportPreview(null); }} disabled={saving}
          style={{width:"100%",padding:"11px",borderRadius:14,background:"none",border:"1.5px solid #FECACA",color:"#DC2626",fontSize:13,fontWeight:700,cursor:saving?"not-allowed":"pointer",opacity:saving?0.6:1}}>
          Discard
        </button>
      </div>
    </div>
...
function PageHero({title, subtitle, icon, from="#1D4ED8", to="#7C3AED"}) {
  return (
    ...
    <div style={{flex:1,overflowY:"auto",background:"#F8FAFC"}}>
      <div style={{padding:16,maxWidth:560}}>
        ...
        <div onClick={()=>importRef.current?.click()}
          style={{border:"1.5px solid #DDD6FE",borderRadius:16,padding:"14px 16px",background:"#FAF5FF",cursor:"pointer",marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11.5,color:"#7C3AED",lineHeight:1.45}}>ForeFlight · LogTen Pro · MyFlightbook · ASA / Jeppesen exports</div>
          </div>
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:C.base}}>
      <div style={{padding:16,maxWidth:540}}>
        <PageHero title="Profile" subtitle="Your pilot identity" icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4" stroke="#fff" strokeWidth="2.2"/><path d="M4 20c1.5-3.5 4.5-5 8-5s6.5 1.5 8 5" stroke="#fff" strokeWidth="2.2" strokeLinecap="round"/></svg>} from="#0EA5E9" to="#1D4ED8"/>
      ...
      <div style={{padding:16}}>
        ...
        {data&&(<>
          ...
          <div style={{marginBottom:14,padding:20,borderRadius:14,background:`linear-gradient(135deg,${C.teal},${C.tealDim})`,position:"relative",overflow:"hidden"}}>
            ...
            {[["1","Share your unique referral code or link"],["2","Your friend subscribes to AviateSync"],["3","You automatically get $11 off your next bill"]].map(([n,t])=>(
...
function BottomTabBar({page, setPage, user}) {
  ...
  const pilotTabs = [
    ...
    },
    {
      ...
      isPopupTrigger: true,
      icon:(active)=>(
        <div style={{width:34,height:34,borderRadius:11,background:showAddPopup?BLUE:"linear-gradient(135deg,#1D4ED8,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:showAddPopup?"0 3px 10px rgba(29,78,216,0.4)":"0 3px 10px rgba(29,78,216,0.3)",transition:"transform .15s",transform:showAddPopup?"rotate(45deg)":"none"}}>
...
function SubscriptionsPage({user}) {
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"16px 18px 90px",background:S.bg}}>
      <PageHero title="Subscription" subtitle="Plan & billing" from="#059669" to="#0D9488"
      ...
      {error&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",color:"#DC2626",borderRadius:12,padding:"10px 12px",fontSize:12,fontWeight:600,marginBottom:12}}>{error}</div>}

      {tab==="plan"&&(
        <>
          {/* Current plan card */}
          <div style={{borderRadius:20,padding:"20px",marginBottom:14,color:"#fff",
            background:isActive?"linear-gradient(135deg,#1D4ED8,#7C3AED)":"linear-gradient(135deg,#334155,#1E293B)",
          ...
          {!isActive&&(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              ...
              <button onClick={()=>subscribe("year")} disabled={!!loading} style={{padding:"14px",borderRadius:14,border:"none",background:"linear-gradient(135deg,#1D4ED8,#7C3AED)",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>
        ...
        <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:18,overflow:"hidden"}}>
          ...
          {!invLoading&&invErr&&(
            <div style={{padding:"22px 16px",textAlign:"center"}}>
              <div style={{fontSize:12.5,color:"#DC2626",fontWeight:600,marginBottom:10}}>{invErr}</div>
          ...
          {!invLoading&&!invErr&&(invoices||[]).map((inv,i)=>(
            <a key={i} href={inv.url||undefined} target="_blank" rel="noreferrer"
              ...
              <span style={{textAlign:"right"}}>{chip(inv.status||"paid", inv.status==="paid"||!inv.status?"#ECFDF5":"#FEF3C7", inv.status==="paid"||!inv.status?"#059669":"#B45309")}</span>
            </a>
          ))}
        </div>
      )}
...
function MorePage({user, setPage, onLogout, rosters, tails}) {
  ...
  const pilotItems = [
    ...
    {id:"subscriptions", label:"Subscriptions",        bg:"#05966918", color:"#059669",
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",alignItems:"center",padding:"28px 16px"}}>
        <div style={{textAlign:"center",marginBottom:24,maxWidth:360}}>
          ...
          <div style={{fontSize:14,color:C.muted,lineHeight:1.65}}>
            Subscribe to unlock AviateSync. The app opens automatically when payment confirms.
          </div>
        </div>
