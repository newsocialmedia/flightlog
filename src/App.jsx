...
function PostUploadVerifyScreen({parsedRoster, rosters, user, onRosterSaved, setParsedRoster, setStatus, setMsg, setPage, fmtMins, schedMins, db_saveRoster, db_loadRosters}) {
  const S = getS();
    const mNum = parsedRoster.monthNum ?? parsedRoster.month_num ?? 0;
    const MONTHS_FULL = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    const MONTH_ABBR  = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

    // Detect carryover -- the next-month stub roster created automatically
    // when the AI parser found flights past the end of the uploaded month.
    const nextMNum  = mNum===11?0:mNum+1;
    const nextYear  = mNum===11?parsedRoster.year+1:parsedRoster.year;
    const nextMonthCarryover = rosters.find(r=>(r.monthNum??r.month_num??0)===nextMNum&&r.year===nextYear&&r._carryover);
    const hasCarryover = !!nextMonthCarryover;
    ...
    async function saveCalendar(newCal, roster=activeRoster) {
      ...
      try {
        ...
        if(vTab==="carryover") {
          // Update the carryover roster reference -- it was already in rosters[]
        } else {
    ...
    return(
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:S.bg,fontFamily:"Inter,system-ui,sans-serif"}}>
                ...
                <div key={day} style={{borderRadius:14,border:`1px solid ${isOff?S.border:S.blue+"44"}`,background:isOff?S.panel:S.surface,overflow:"hidden"}}>
                  ...
                  {(data.flights||[]).map((f,fi)=>(
                    <div key={fi} style={{borderTop:`1px solid ${S.border}`,padding:"8px 12px",display:"flex",alignItems:"center",gap:10}}>
                      <div style={{width:4,height:28,borderRadius:2,background:S.blue,flexShrink:0}}/>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:12,fontWeight:800,color:S.ink}}>{f.flightNum}</div>
                        <div style={{fontSize:11,color:S.muted}}>{f.dep}→{f.arr} · {f.depTime||"--"}
                      ...
                      <div style={{fontSize:11,color:S.muted,textAlign:"right"}}>
                        {f.schedBlockMins?fmtMins(f.schedBlockMins):"--"}
                        {f.acType&&<div style={{fontSize:10}}>{f.acType}</div>}
                      </div>
                      <button onClick={()=>deleteFlight(day,fi)} title="Remove flight"
                        style={{width:24,height:24,borderRadius:"50%",background:"none",border:"none",color:S.muted,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:0.5}}>×</button>
                    </div>
                  ))}
        ...
        {/* Bottom buttons */}
        <div style={{flexShrink:0,background:S.surface,borderTop:`1px solid ${S.border}`,padding:"12px 14px",display:"flex",gap:10}}>
          <button onClick={()=>{setParsedRoster(null);setStatus(null);setMsg("");}} style={{flex:1,padding:"12px",borderRadius:14,background:"none",border:`1.5px solid ${S.border}`,color:S.muted,fontSize:13,fontWeight:700,cursor:"pointer"}}>
            Upload Another
          </button>
          <button onClick={()=>setPage&&setPage("active-logs")} style={{flex:2,padding:"12px",borderRadius:14,background:`linear-gradient(135deg,${S.blue},#7C3AED)`,border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
...
function UploadPage({user, onRosterSaved, setPage}) {
  ...
  if(parsedRoster) return(
    <PostUploadVerifyScreen
      ...
      onRosterSaved={onRosterSaved} setParsedRoster={setParsedRoster}
      setStatus={setStatus} setMsg={setMsg} setPage={setPage}
      fmtMins={fmtMins} schedMins={schedMins}
      db_saveRoster={db_saveRoster} db_loadRosters={db_loadRosters}
    />
  );
