...
async function db_saveRoster(userId, roster) {
  if(isConfigured()) {
    ...
    const carryDayNums = new Set(explicitCarry.map((d:any) => d.day));
    const thisDays = cal.filter((d:any) => {
      ...
      const carry = explicitCarry.find((c:any) => c.day === d.day);
      const thisFlightNums = (d.flights||[]).map((f:any)=>f.flightNum).join(",");
      const carryFlightNums = (carry?.flights||[]).map((f:any)=>f.flightNum).join(",");
    ...
    if(carryForwardDays.length === 0) {
      ...
      for(let i = 1; i < cal.length; i++) {
        if(cal[i]?.day < cal[i-1]?.day && cal[i]?.day <= 7 && cal[i-1]?.day >= 20) {
          carryForwardDays = cal.slice(i).filter((d:any) => d.day >= 1 && d.day <= 7);
          break;
        }
      }
      // Also check for days numbered beyond month end (e.g. day:29 in Feb=28 days)
      const highDays = cal.filter((d:any) => d.day > daysInMonth).map((d:any) => ({...d, day: d.day - daysInMonth}));
    ...
    if(carryForwardDays.length > 0) {
      ...
      const validCarry = carryForwardDays.filter((d:any) => d.day >= 1 && d.day <= nextDaysInMonth);
      ...
      if(validCarry.length > 0) {
        ...
        if(nextRoster?.id) {
          ...
          const existingDayNums = new Set((nextRoster.calendar||[]).map((d:any)=>d.day));
          const daysToAdd = validCarry.filter((d:any) => !existingDayNums.has(d.day));
          if(daysToAdd.length > 0) {
            const merged = [...(nextRoster.calendar||[]), ...daysToAdd].sort((a:any,b:any)=>a.day-b.day);
      ...
      <div>
        ...
        {reviewRoster.carryForwardDays?.length > 0 && (
          <div style={{marginBottom:16,padding:"12px 14px",borderRadius:10,background:C.teal+"12",border:`1px solid ${C.teal}44`}}>
            ...
            {reviewRoster.carryForwardDays.map((d:any,i:number)=>(
              <div key={i} style={{fontSize:12,color:C.ink,marginTop:4}}>
                · Day {d.day}: {d.flights?.length>0 ? d.flights.map((f:any)=>`${f.flightNum} ${f.dep}→${f.arr}`).join(", ") : d.dutyCode||"Off"}
              </div>
            ))}
          </div>
        )}
