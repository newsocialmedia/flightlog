...
function Dashboard({user,rosters,tails,setPage,onOpenFlight}) {
  ...
  useEffect(()=>{
    ...
    const tryFetch = (icao: string) => fetch(`https://aviationweather.gov/api/data/metar?ids=${icao}&format=json&hours=2`).then(r=>r.json());
...
function ProfilePage({user, onUserUpdated, setPage}) {
  ...
  async function saveProfile() {
    ...
    try {
      // Use direct fetch to bypass any client-level caching
      const res = await fetch(`${SUPA_URL}/rest/v1/profiles?id=eq.${user.id}`, {
        method:"PATCH",
        headers:{
          "Content-Type":"application/json",
          "apikey":SUPA_ANON,
        ...
        body:JSON.stringify({name, airline_iata:airlineIata||null, airline_name:airlineName||null}),
      });
...
function ExportPage({rosters, tails}) {
  const total = (rosters||[]).reduce((a,r)=>a+(r.calendar||[]).reduce((b,d)=>b+(d.flights||[]).length,0),0);
  const synced = (rosters||[]).reduce((a,r)=>{
    let n=0;
    (r.calendar||[]).forEach((d,di)=>(d.flights||[]).forEach((_,fi)=>{if(tails[`${r.id}-${di}-${fi}`]?.tail)n++;}));
    return a+n;
  },0);
    ...
    <div style={{flex:1,overflowY:"auto",background:C.base}}>
      ...
      <div style={{padding:16}}>
        ...
        <div className="card" style={{marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between"}}>
            <div style={{textAlign:"center",flex:1}}>
              <div style={{fontSize:24,fontWeight:800,color:C.teal}}>{total}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>Total flights</div>
            </div>
            <div style={{width:1,background:C.border}}/>
            <div style={{textAlign:"center",flex:1}}>
              <div style={{fontSize:24,fontWeight:800,color:C.green}}>{synced}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>Tail # synced</div>
            </div>
            <div style={{width:1,background:C.border}}/>
            <div style={{textAlign:"center",flex:1}}>
              <div style={{fontSize:24,fontWeight:800,color:C.ink}}>{(rosters||[]).length}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>Rosters</div>
            </div>
          </div>
        </div>

        <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".5px",marginBottom:10}}>Export formats</div>

        {[
          {
            fmt:"CSV Universal",
            desc:"All fields including actual times, tail numbers, and remarks. Works with Excel and Google Sheets.",
            icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke={C.teal} strokeWidth="2"/><path d="M3 9h18M9 9v12" stroke={C.teal} strokeWidth="2"/></svg>,
            action:()=>csvExport(rosters,tails),
          },
          {
            fmt:"Jeppesen Format",
            desc:"Matches the column layout of the Jeppesen Professional Pilot Logbook exactly.",
            icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="4" y="2" width="14" height="18" rx="2" stroke="#1D4ED8" strokeWidth="2"/><path d="M8 7h8M8 11h6M8 15h4" stroke="#1D4ED8" strokeWidth="2" strokeLinecap="round"/></svg>,
            action:()=>jeppesenExport(rosters,tails),
          },
          {
            fmt:"ASA Standard",
            desc:"Matches the ASA-SP-30 logbook column format for direct import.",
            icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="4" y="2" width="14" height="18" rx="2" stroke="#16A34A" strokeWidth="2"/><path d="M8 7h8M8 11h6M8 15h4" stroke="#16A34A" strokeWidth="2" strokeLinecap="round"/></svg>,
            action:()=>asaExport(rosters,tails),
          },
        ].map(({fmt,desc,icon,action})=>(
          <div key={fmt} className="card" style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
              <div style={{width:40,height:40,borderRadius:10,background:C.panel,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{icon}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:700,color:C.ink}}>{fmt}</div>
                <div style={{fontSize:12,color:C.muted,marginTop:2,lineHeight:1.4}}>{desc}</div>
              </div>
            </div>
            <button
              onClick={action}
              ...
              style={{
                width:"100%",padding:"11px",borderRadius:10,
                background:total===0?C.panel:C.teal,
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 15V4M12 15l-4-4M12 15l4-4" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M4 17v1a3 3 0 003 3h10a3 3 0 003-3v-1" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"/></svg>
              Download {fmt}
            </button>
          </div>
        ))}
        ...
        <div style={{fontSize:11,color:C.muted,textAlign:"center",marginTop:8}}>
          Cancelled flights are excluded from all exports
        </div>
      </div>
    </div>
  );
