  ...
  if(isConfigured()) {
    ...
    if(carryForwardDays.length > 0) {
      ...
      if(validCarry.length > 0) {
        const {data:nextRosterRows} = await sb.from("rosters").select("id,calendar")
          .eq("user_id",userId).eq("year",nextYear).eq("month_num",nextMonth);
        const nextRoster = Array.isArray(nextRosterRows) ? nextRosterRows[0] : null;
    ...
    const {data:existingRows} = await sb.from("rosters")
      .select("id,calendar")
      .eq("user_id", userId)
      .eq("year", roster.year)
